                  ...
                  double * const restrict INT__g_d_s_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_s_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*64);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 140;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_s_s = primwork + 182;
    double * const hrrwork = (double *)(primwork + 210);
    double * const HRR_INT__g_p_s_s = hrrwork + 0;
    double * const HRR_INT__h_p_s_s = hrrwork + 45;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 64 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_s_s = INT__g_d_s_s + real_abcd * 90;
                  ...
                  double * const restrict INT__d_g_s_s)
{
    ...
    int ret = ostei_g_d_s_s(P_tmp, Q, screen_tol, work, INT__d_g_s_s);
    double buffer[90] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__d_g_s_s[q*90+b*6+a*1+c*1+d];

        memcpy(INT__d_g_s_s+q*90, buffer, 90*sizeof(double));
    }
