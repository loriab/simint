                  ...
                  double * const restrict INT__f_f_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_f_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__h_s_g_s = work + (SIMINT_NSHELL_SIMD * 375);
    double * const INT__i_s_g_s = work + (SIMINT_NSHELL_SIMD * 690);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*1110);
    ...
    double * const HRR_INT__f_d_g_s = hrrwork + 450;
    double * const HRR_INT__g_p_g_s = hrrwork + 1350;
    double * const HRR_INT__g_d_g_s = hrrwork + 2025;
    double * const HRR_INT__h_p_g_s = hrrwork + 3375;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 1110 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_f_g_s = INT__f_f_g_s + real_abcd * 1500;
                  ...
                  double * const restrict INT__f_f_s_g)
{
    ...
    int ret = ostei_f_f_g_s(P, Q_tmp, screen_tol, work, INT__f_f_s_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_f_s_g[q*1500+a*150+b*15+d*1+c];

        memcpy(INT__f_f_s_g+q*1500, buffer, 1500*sizeof(double));
    }
