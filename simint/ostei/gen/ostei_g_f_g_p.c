                  ...
                  double * const restrict INT__g_f_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*3600);
    ...
    double * const HRR_INT__h_p_g_s = hrrwork + 10260;
    double * const HRR_INT__h_p_h_s = hrrwork + 11205;
    double * const HRR_INT__h_d_g_s = hrrwork + 12528;
    double * const HRR_INT__h_d_h_s = hrrwork + 14418;
    double * const HRR_INT__i_p_g_s = hrrwork + 17064;
    double * const HRR_INT__i_p_h_s = hrrwork + 18324;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 3600 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_g_p = INT__g_f_g_p + real_abcd * 6750;
                  ...
                  double * const restrict INT__f_g_g_p)
{
    ...
    int ret = ostei_g_f_g_p(P_tmp, Q, screen_tol, work, INT__f_g_g_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__f_g_g_p[q*6750+b*450+a*45+c*3+d];

        memcpy(INT__f_g_g_p+q*6750, buffer, 6750*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_p_g)
{
    ...
    int ret = ostei_g_f_g_p(P, Q_tmp, screen_tol, work, INT__g_f_p_g);
    double buffer[6750] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_f_p_g[q*6750+a*450+b*45+d*3+c];

        memcpy(INT__g_f_p_g+q*6750, buffer, 6750*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_p_g)
{
    ...
    int ret = ostei_g_f_g_p(P_tmp, Q_tmp, screen_tol, work, INT__f_g_p_g);
    double buffer[6750] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_g_p_g[q*6750+b*450+a*45+d*3+c];

        memcpy(INT__f_g_p_g+q*6750, buffer, 6750*sizeof(double));
    }
