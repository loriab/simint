                  ...
                  double * const restrict INT__g_s_d_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_s_d_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*240);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_s_s = primwork + 119;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_p_s = primwork + 169;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_d_s = primwork + 259;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 379;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 439;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_d_s = primwork + 574;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_f_s = primwork + 754;
    double * const hrrwork = (double *)(primwork + 904);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 240 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_s_d_p = INT__g_s_d_p + real_abcd * 270;
                  ...
                  double * const restrict INT__s_g_d_p)
{
    ...
    int ret = ostei_g_s_d_p(P_tmp, Q, screen_tol, work, INT__s_g_d_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__s_g_d_p[q*270+b*18+a*18+c*3+d];

        memcpy(INT__s_g_d_p+q*270, buffer, 270*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_s_p_d)
{
    ...
    int ret = ostei_g_s_d_p(P, Q_tmp, screen_tol, work, INT__g_s_p_d);
    double buffer[270] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__g_s_p_d[q*270+a*18+b*18+d*3+c];

        memcpy(INT__g_s_p_d+q*270, buffer, 270*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_g_p_d)
{
    ...
    int ret = ostei_g_s_d_p(P_tmp, Q_tmp, screen_tol, work, INT__s_g_p_d);
    double buffer[270] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__s_g_p_d[q*270+b*18+a*18+d*3+c];

        memcpy(INT__s_g_p_d+q*270, buffer, 270*sizeof(double));
    }
