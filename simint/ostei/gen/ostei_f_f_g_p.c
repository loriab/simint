                  ...
                  double * const restrict INT__f_f_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_f_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__i_s_g_s = work + (SIMINT_NSHELL_SIMD * 1656);
    double * const INT__i_s_h_s = work + (SIMINT_NSHELL_SIMD * 2076);
    ...
    double * const HRR_INT__f_f_g_s = hrrwork + 3240;
    double * const HRR_INT__f_f_h_s = hrrwork + 4740;
    double * const HRR_INT__g_p_g_s = hrrwork + 6840;
    double * const HRR_INT__g_p_h_s = hrrwork + 7515;
    double * const HRR_INT__g_d_g_s = hrrwork + 8460;
    double * const HRR_INT__g_d_h_s = hrrwork + 9810;
    double * const HRR_INT__h_p_g_s = hrrwork + 11700;
    double * const HRR_INT__h_p_h_s = hrrwork + 12645;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_f_g_p = INT__f_f_g_p + real_abcd * 4500;
                  ...
                  double * const restrict INT__f_f_p_g)
{
    ...
    int ret = ostei_f_f_g_p(P, Q_tmp, screen_tol, work, INT__f_f_p_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_f_p_g[q*4500+a*450+b*45+d*3+c];

        memcpy(INT__f_f_p_g+q*4500, buffer, 4500*sizeof(double));
    }
