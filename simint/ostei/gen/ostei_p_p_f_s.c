                  ...
                  double * const restrict INT__p_p_f_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_f_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__d_s_f_s = work + (SIMINT_NSHELL_SIMD * 30);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_d_s = primwork + 75;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 111;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_f_s = primwork + 171;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_p_f_s = INT__p_p_f_s + real_abcd * 90;
