                  ...
                  double * const restrict INT__g_g_f_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_g_f_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__l_s_f_s = work + (SIMINT_NSHELL_SIMD * 1000);
    ...
    double * const HRR_INT__g_f_f_s = hrrwork + 1350;
    double * const HRR_INT__h_p_f_s = hrrwork + 2850;
    double * const HRR_INT__h_d_f_s = hrrwork + 3480;
    double * const HRR_INT__h_f_f_s = hrrwork + 4740;
    double * const HRR_INT__i_p_f_s = hrrwork + 6840;
    double * const HRR_INT__i_d_f_s = hrrwork + 7680;
    double * const HRR_INT__k_p_f_s = hrrwork + 9360;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_g_f_s = INT__g_g_f_s + real_abcd * 2250;
                  ...
                  double * const restrict INT__g_g_s_f)
{
    ...
    int ret = ostei_g_g_f_s(P, Q_tmp, screen_tol, work, INT__g_g_s_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_g_s_f[q*2250+a*150+b*10+d*1+c];

        memcpy(INT__g_g_s_f+q*2250, buffer, 2250*sizeof(double));
    }
