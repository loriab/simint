                  ...
                  double * const restrict INT__g_f_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__h_s_g_s = work + (SIMINT_NSHELL_SIMD * 225);
    double * const INT__i_s_g_s = work + (SIMINT_NSHELL_SIMD * 540);
    double * const INT__k_s_g_s = work + (SIMINT_NSHELL_SIMD * 960);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*1500);
    ...
    double * const HRR_INT__g_d_g_s = hrrwork + 675;
    double * const HRR_INT__h_p_g_s = hrrwork + 2025;
    double * const HRR_INT__h_d_g_s = hrrwork + 2970;
    double * const HRR_INT__i_p_g_s = hrrwork + 4860;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 1500 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_g_s = INT__g_f_g_s + real_abcd * 2250;
                  ...
                  double * const restrict INT__f_g_g_s)
{
    ...
    int ret = ostei_g_f_g_s(P_tmp, Q, screen_tol, work, INT__f_g_g_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__f_g_g_s[q*2250+b*150+a*15+c*1+d];

        memcpy(INT__f_g_g_s+q*2250, buffer, 2250*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_s_g)
{
    ...
    int ret = ostei_g_f_g_s(P, Q_tmp, screen_tol, work, INT__g_f_s_g);
    double buffer[2250] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_f_s_g[q*2250+a*150+b*15+d*1+c];

        memcpy(INT__g_f_s_g+q*2250, buffer, 2250*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_s_g)
{
    ...
    int ret = ostei_g_f_g_s(P_tmp, Q_tmp, screen_tol, work, INT__f_g_s_g);
    double buffer[2250] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_g_s_g[q*2250+b*150+a*15+d*1+c];

        memcpy(INT__f_g_s_g+q*2250, buffer, 2250*sizeof(double));
    }
