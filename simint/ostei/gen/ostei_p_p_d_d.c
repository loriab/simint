                  ...
                  double * const restrict INT__p_p_d_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_d_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__p_s_d_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__p_s_f_s = work + (SIMINT_NSHELL_SIMD * 18);
    double * const INT__p_s_g_s = work + (SIMINT_NSHELL_SIMD * 48);
    double * const INT__d_s_d_s = work + (SIMINT_NSHELL_SIMD * 93);
    double * const INT__d_s_f_s = work + (SIMINT_NSHELL_SIMD * 129);
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 189);
    ...
    double * const HRR_INT__p_p_d_s = hrrwork + 0;
    double * const HRR_INT__p_p_d_p = hrrwork + 54;
    double * const HRR_INT__p_p_f_s = hrrwork + 216;
    double * const HRR_INT__p_p_f_p = hrrwork + 306;
    double * const HRR_INT__p_p_g_s = hrrwork + 576;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__p_s_d_s = INT__p_s_d_s + abcd * 18;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__p_s_d_s += lastoffset*18;
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(18, PRIM_INT__p_s_d_s, PRIM_PTR_INT__p_s_d_s);
                    ...
                    {
                        contract(18, shelloffsets, PRIM_INT__p_s_d_s, PRIM_PTR_INT__p_s_d_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__p_s_d_s = INT__p_s_d_s + abcd * 18;
                ...
                double * restrict HRR_INT__p_p_d_d = INT__p_p_d_d + real_abcd * 324;

                // form INT__p_p_d_s
                for(iket = 0; iket < 6; ++iket)
                {
                    HRR_INT__p_p_d_s[0 * 6 + iket] = HRR_INT__d_s_d_s[0 * 6 + iket] + ( hAB[0] * HRR_INT__p_s_d_s[0 * 6 + iket] );

                    HRR_INT__p_p_d_s[1 * 6 + iket] = HRR_INT__d_s_d_s[1 * 6 + iket] + ( hAB[1] * HRR_INT__p_s_d_s[0 * 6 + iket] );

                    HRR_INT__p_p_d_s[2 * 6 + iket] = HRR_INT__d_s_d_s[2 * 6 + iket] + ( hAB[2] * HRR_INT__p_s_d_s[0 * 6 + iket] );

                    HRR_INT__p_p_d_s[3 * 6 + iket] = HRR_INT__d_s_d_s[1 * 6 + iket] + ( hAB[0] * HRR_INT__p_s_d_s[1 * 6 + iket] );

                    HRR_INT__p_p_d_s[4 * 6 + iket] = HRR_INT__d_s_d_s[3 * 6 + iket] + ( hAB[1] * HRR_INT__p_s_d_s[1 * 6 + iket] );

                    HRR_INT__p_p_d_s[5 * 6 + iket] = HRR_INT__d_s_d_s[4 * 6 + iket] + ( hAB[2] * HRR_INT__p_s_d_s[1 * 6 + iket] );

                    HRR_INT__p_p_d_s[6 * 6 + iket] = HRR_INT__d_s_d_s[2 * 6 + iket] + ( hAB[0] * HRR_INT__p_s_d_s[2 * 6 + iket] );

                    HRR_INT__p_p_d_s[7 * 6 + iket] = HRR_INT__d_s_d_s[4 * 6 + iket] + ( hAB[1] * HRR_INT__p_s_d_s[2 * 6 + iket] );

                    HRR_INT__p_p_d_s[8 * 6 + iket] = HRR_INT__d_s_d_s[5 * 6 + iket] + ( hAB[2] * HRR_INT__p_s_d_s[2 * 6 + iket] );
                ...
                {
                    HRR_INT__p_p_d_p[ibra * 18 + 0] = HRR_INT__p_p_f_s[ibra * 10 + 0] + ( hCD[0] * HRR_INT__p_p_d_s[ibra * 6 + 0] );

                    HRR_INT__p_p_d_p[ibra * 18 + 1] = HRR_INT__p_p_f_s[ibra * 10 + 1] + ( hCD[1] * HRR_INT__p_p_d_s[ibra * 6 + 0] );

                    HRR_INT__p_p_d_p[ibra * 18 + 2] = HRR_INT__p_p_f_s[ibra * 10 + 2] + ( hCD[2] * HRR_INT__p_p_d_s[ibra * 6 + 0] );

                    HRR_INT__p_p_d_p[ibra * 18 + 3] = HRR_INT__p_p_f_s[ibra * 10 + 1] + ( hCD[0] * HRR_INT__p_p_d_s[ibra * 6 + 1] );

                    HRR_INT__p_p_d_p[ibra * 18 + 4] = HRR_INT__p_p_f_s[ibra * 10 + 3] + ( hCD[1] * HRR_INT__p_p_d_s[ibra * 6 + 1] );

                    HRR_INT__p_p_d_p[ibra * 18 + 5] = HRR_INT__p_p_f_s[ibra * 10 + 4] + ( hCD[2] * HRR_INT__p_p_d_s[ibra * 6 + 1] );

                    HRR_INT__p_p_d_p[ibra * 18 + 6] = HRR_INT__p_p_f_s[ibra * 10 + 2] + ( hCD[0] * HRR_INT__p_p_d_s[ibra * 6 + 2] );

                    HRR_INT__p_p_d_p[ibra * 18 + 7] = HRR_INT__p_p_f_s[ibra * 10 + 4] + ( hCD[1] * HRR_INT__p_p_d_s[ibra * 6 + 2] );

                    HRR_INT__p_p_d_p[ibra * 18 + 8] = HRR_INT__p_p_f_s[ibra * 10 + 5] + ( hCD[2] * HRR_INT__p_p_d_s[ibra * 6 + 2] );

                    HRR_INT__p_p_d_p[ibra * 18 + 9] = HRR_INT__p_p_f_s[ibra * 10 + 3] + ( hCD[0] * HRR_INT__p_p_d_s[ibra * 6 + 3] );

                    HRR_INT__p_p_d_p[ibra * 18 + 10] = HRR_INT__p_p_f_s[ibra * 10 + 6] + ( hCD[1] * HRR_INT__p_p_d_s[ibra * 6 + 3] );

                    HRR_INT__p_p_d_p[ibra * 18 + 11] = HRR_INT__p_p_f_s[ibra * 10 + 7] + ( hCD[2] * HRR_INT__p_p_d_s[ibra * 6 + 3] );

                    HRR_INT__p_p_d_p[ibra * 18 + 12] = HRR_INT__p_p_f_s[ibra * 10 + 4] + ( hCD[0] * HRR_INT__p_p_d_s[ibra * 6 + 4] );

                    HRR_INT__p_p_d_p[ibra * 18 + 13] = HRR_INT__p_p_f_s[ibra * 10 + 7] + ( hCD[1] * HRR_INT__p_p_d_s[ibra * 6 + 4] );

                    HRR_INT__p_p_d_p[ibra * 18 + 14] = HRR_INT__p_p_f_s[ibra * 10 + 8] + ( hCD[2] * HRR_INT__p_p_d_s[ibra * 6 + 4] );

                    HRR_INT__p_p_d_p[ibra * 18 + 15] = HRR_INT__p_p_f_s[ibra * 10 + 5] + ( hCD[0] * HRR_INT__p_p_d_s[ibra * 6 + 5] );

                    HRR_INT__p_p_d_p[ibra * 18 + 16] = HRR_INT__p_p_f_s[ibra * 10 + 8] + ( hCD[1] * HRR_INT__p_p_d_s[ibra * 6 + 5] );

                    HRR_INT__p_p_d_p[ibra * 18 + 17] = HRR_INT__p_p_f_s[ibra * 10 + 9] + ( hCD[2] * HRR_INT__p_p_d_s[ibra * 6 + 5] );
                ...
                for(ibra = 0; ibra < 9; ++ibra)
                {
                    HRR_INT__p_p_d_d[ibra * 36 + 0] = HRR_INT__p_p_f_p[ibra * 30 + 0] + ( hCD[0] * HRR_INT__p_p_d_p[ibra * 18 + 0] );

                    HRR_INT__p_p_d_d[ibra * 36 + 1] = HRR_INT__p_p_f_p[ibra * 30 + 3] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 0] );

                    HRR_INT__p_p_d_d[ibra * 36 + 2] = HRR_INT__p_p_f_p[ibra * 30 + 6] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 0] );

                    HRR_INT__p_p_d_d[ibra * 36 + 3] = HRR_INT__p_p_f_p[ibra * 30 + 4] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 1] );

                    HRR_INT__p_p_d_d[ibra * 36 + 4] = HRR_INT__p_p_f_p[ibra * 30 + 7] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 1] );

                    HRR_INT__p_p_d_d[ibra * 36 + 5] = HRR_INT__p_p_f_p[ibra * 30 + 8] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 2] );

                    HRR_INT__p_p_d_d[ibra * 36 + 6] = HRR_INT__p_p_f_p[ibra * 30 + 3] + ( hCD[0] * HRR_INT__p_p_d_p[ibra * 18 + 3] );

                    HRR_INT__p_p_d_d[ibra * 36 + 7] = HRR_INT__p_p_f_p[ibra * 30 + 9] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 3] );

                    HRR_INT__p_p_d_d[ibra * 36 + 8] = HRR_INT__p_p_f_p[ibra * 30 + 12] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 3] );

                    HRR_INT__p_p_d_d[ibra * 36 + 9] = HRR_INT__p_p_f_p[ibra * 30 + 10] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 4] );

                    HRR_INT__p_p_d_d[ibra * 36 + 10] = HRR_INT__p_p_f_p[ibra * 30 + 13] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 4] );

                    HRR_INT__p_p_d_d[ibra * 36 + 11] = HRR_INT__p_p_f_p[ibra * 30 + 14] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 5] );

                    HRR_INT__p_p_d_d[ibra * 36 + 12] = HRR_INT__p_p_f_p[ibra * 30 + 6] + ( hCD[0] * HRR_INT__p_p_d_p[ibra * 18 + 6] );

                    HRR_INT__p_p_d_d[ibra * 36 + 13] = HRR_INT__p_p_f_p[ibra * 30 + 12] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 6] );

                    HRR_INT__p_p_d_d[ibra * 36 + 14] = HRR_INT__p_p_f_p[ibra * 30 + 15] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 6] );

                    HRR_INT__p_p_d_d[ibra * 36 + 15] = HRR_INT__p_p_f_p[ibra * 30 + 13] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 7] );

                    HRR_INT__p_p_d_d[ibra * 36 + 16] = HRR_INT__p_p_f_p[ibra * 30 + 16] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 7] );

                    HRR_INT__p_p_d_d[ibra * 36 + 17] = HRR_INT__p_p_f_p[ibra * 30 + 17] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 8] );

                    HRR_INT__p_p_d_d[ibra * 36 + 18] = HRR_INT__p_p_f_p[ibra * 30 + 9] + ( hCD[0] * HRR_INT__p_p_d_p[ibra * 18 + 9] );

                    HRR_INT__p_p_d_d[ibra * 36 + 19] = HRR_INT__p_p_f_p[ibra * 30 + 18] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 9] );

                    HRR_INT__p_p_d_d[ibra * 36 + 20] = HRR_INT__p_p_f_p[ibra * 30 + 21] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 9] );

                    HRR_INT__p_p_d_d[ibra * 36 + 21] = HRR_INT__p_p_f_p[ibra * 30 + 19] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 10] );

                    HRR_INT__p_p_d_d[ibra * 36 + 22] = HRR_INT__p_p_f_p[ibra * 30 + 22] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 10] );

                    HRR_INT__p_p_d_d[ibra * 36 + 23] = HRR_INT__p_p_f_p[ibra * 30 + 23] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 11] );

                    HRR_INT__p_p_d_d[ibra * 36 + 24] = HRR_INT__p_p_f_p[ibra * 30 + 12] + ( hCD[0] * HRR_INT__p_p_d_p[ibra * 18 + 12] );

                    HRR_INT__p_p_d_d[ibra * 36 + 25] = HRR_INT__p_p_f_p[ibra * 30 + 21] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 12] );

                    HRR_INT__p_p_d_d[ibra * 36 + 26] = HRR_INT__p_p_f_p[ibra * 30 + 24] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 12] );

                    HRR_INT__p_p_d_d[ibra * 36 + 27] = HRR_INT__p_p_f_p[ibra * 30 + 22] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 13] );

                    HRR_INT__p_p_d_d[ibra * 36 + 28] = HRR_INT__p_p_f_p[ibra * 30 + 25] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 13] );

                    HRR_INT__p_p_d_d[ibra * 36 + 29] = HRR_INT__p_p_f_p[ibra * 30 + 26] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 14] );

                    HRR_INT__p_p_d_d[ibra * 36 + 30] = HRR_INT__p_p_f_p[ibra * 30 + 15] + ( hCD[0] * HRR_INT__p_p_d_p[ibra * 18 + 15] );

                    HRR_INT__p_p_d_d[ibra * 36 + 31] = HRR_INT__p_p_f_p[ibra * 30 + 24] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 15] );

                    HRR_INT__p_p_d_d[ibra * 36 + 32] = HRR_INT__p_p_f_p[ibra * 30 + 27] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 15] );

                    HRR_INT__p_p_d_d[ibra * 36 + 33] = HRR_INT__p_p_f_p[ibra * 30 + 25] + ( hCD[1] * HRR_INT__p_p_d_p[ibra * 18 + 16] );

                    HRR_INT__p_p_d_d[ibra * 36 + 34] = HRR_INT__p_p_f_p[ibra * 30 + 28] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 16] );

                    HRR_INT__p_p_d_d[ibra * 36 + 35] = HRR_INT__p_p_f_p[ibra * 30 + 29] + ( hCD[2] * HRR_INT__p_p_d_p[ibra * 18 + 17] );
