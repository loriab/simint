                  ...
                  double * const restrict INT__g_f_f_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_f_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*2500);
    ...
    double * const HRR_INT__h_p_f_s = hrrwork + 7125;
    double * const HRR_INT__h_p_g_s = hrrwork + 7755;
    double * const HRR_INT__h_d_f_s = hrrwork + 8700;
    double * const HRR_INT__h_d_g_s = hrrwork + 9960;
    double * const HRR_INT__i_p_f_s = hrrwork + 11850;
    double * const HRR_INT__i_p_g_s = hrrwork + 12690;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 2500 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_f_p = INT__g_f_f_p + real_abcd * 4500;
                  ...
                  double * const restrict INT__f_g_f_p)
{
    ...
    int ret = ostei_g_f_f_p(P_tmp, Q, screen_tol, work, INT__f_g_f_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__f_g_f_p[q*4500+b*300+a*30+c*3+d];

        memcpy(INT__f_g_f_p+q*4500, buffer, 4500*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_p_f)
{
    ...
    int ret = ostei_g_f_f_p(P, Q_tmp, screen_tol, work, INT__g_f_p_f);
    double buffer[4500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_f_p_f[q*4500+a*300+b*30+d*3+c];

        memcpy(INT__g_f_p_f+q*4500, buffer, 4500*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_p_f)
{
    ...
    int ret = ostei_g_f_f_p(P_tmp, Q_tmp, screen_tol, work, INT__f_g_p_f);
    double buffer[4500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__f_g_p_f[q*4500+b*300+a*30+d*3+c];

        memcpy(INT__f_g_p_f+q*4500, buffer, 4500*sizeof(double));
    }
