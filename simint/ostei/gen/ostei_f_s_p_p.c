                  ...
                  double * const restrict INT__f_s_p_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_s_p_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_s_p_p = INT__f_s_p_p + real_abcd * 90;

                // form INT__f_s_p_p
                for(ibra = 0; ibra < 10; ++ibra)
                {
                    HRR_INT__f_s_p_p[ibra * 9 + 0] = HRR_INT__f_s_d_s[ibra * 6 + 0] + ( hCD[0] * HRR_INT__f_s_p_s[ibra * 3 + 0] );

                    HRR_INT__f_s_p_p[ibra * 9 + 1] = HRR_INT__f_s_d_s[ibra * 6 + 1] + ( hCD[1] * HRR_INT__f_s_p_s[ibra * 3 + 0] );

                    HRR_INT__f_s_p_p[ibra * 9 + 2] = HRR_INT__f_s_d_s[ibra * 6 + 2] + ( hCD[2] * HRR_INT__f_s_p_s[ibra * 3 + 0] );

                    HRR_INT__f_s_p_p[ibra * 9 + 3] = HRR_INT__f_s_d_s[ibra * 6 + 1] + ( hCD[0] * HRR_INT__f_s_p_s[ibra * 3 + 1] );

                    HRR_INT__f_s_p_p[ibra * 9 + 4] = HRR_INT__f_s_d_s[ibra * 6 + 3] + ( hCD[1] * HRR_INT__f_s_p_s[ibra * 3 + 1] );

                    HRR_INT__f_s_p_p[ibra * 9 + 5] = HRR_INT__f_s_d_s[ibra * 6 + 4] + ( hCD[2] * HRR_INT__f_s_p_s[ibra * 3 + 1] );

                    HRR_INT__f_s_p_p[ibra * 9 + 6] = HRR_INT__f_s_d_s[ibra * 6 + 2] + ( hCD[0] * HRR_INT__f_s_p_s[ibra * 3 + 2] );

                    HRR_INT__f_s_p_p[ibra * 9 + 7] = HRR_INT__f_s_d_s[ibra * 6 + 4] + ( hCD[1] * HRR_INT__f_s_p_s[ibra * 3 + 2] );

                    HRR_INT__f_s_p_p[ibra * 9 + 8] = HRR_INT__f_s_d_s[ibra * 6 + 5] + ( hCD[2] * HRR_INT__f_s_p_s[ibra * 3 + 2] );
