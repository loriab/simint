                  ...
                  double * const restrict INT__g_s_g_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_s_g_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_s_h_p = hrrwork + 2025;
    double * const HRR_INT__g_s_h_d = hrrwork + 2970;
    double * const HRR_INT__g_s_i_p = hrrwork + 4860;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_s_g_f = INT__g_s_g_f + real_abcd * 2250;
                  ...
                  double * const restrict INT__s_g_g_f)
{
    ...
    int ret = ostei_g_s_g_f(P_tmp, Q, screen_tol, work, INT__s_g_g_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__s_g_g_f[q*2250+b*150+a*150+c*10+d];

        memcpy(INT__s_g_g_f+q*2250, buffer, 2250*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_s_f_g)
{
    ...
    int ret = ostei_g_s_g_f(P, Q_tmp, screen_tol, work, INT__g_s_f_g);
    double buffer[2250] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_s_f_g[q*2250+a*150+b*150+d*10+c];

        memcpy(INT__g_s_f_g+q*2250, buffer, 2250*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_g_f_g)
{
    ...
    int ret = ostei_g_s_g_f(P_tmp, Q_tmp, screen_tol, work, INT__s_g_f_g);
    double buffer[2250] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_g_f_g[q*2250+b*150+a*150+d*10+c];

        memcpy(INT__s_g_f_g+q*2250, buffer, 2250*sizeof(double));
    }
