                  ...
                  double * const restrict INT__g_p_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*540);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 540 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_g_s = INT__g_p_g_s + real_abcd * 675;
                  ...
                  double * const restrict INT__p_g_g_s)
{
    ...
    int ret = ostei_g_p_g_s(P_tmp, Q, screen_tol, work, INT__p_g_g_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__p_g_g_s[q*675+b*45+a*15+c*1+d];

        memcpy(INT__p_g_g_s+q*675, buffer, 675*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_s_g)
{
    ...
    int ret = ostei_g_p_g_s(P, Q_tmp, screen_tol, work, INT__g_p_s_g);
    double buffer[675] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_p_s_g[q*675+a*45+b*15+d*1+c];

        memcpy(INT__g_p_s_g+q*675, buffer, 675*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_s_g)
{
    ...
    int ret = ostei_g_p_g_s(P_tmp, Q_tmp, screen_tol, work, INT__p_g_s_g);
    double buffer[675] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_g_s_g[q*675+b*45+a*15+d*1+c];

        memcpy(INT__p_g_s_g+q*675, buffer, 675*sizeof(double));
    }
