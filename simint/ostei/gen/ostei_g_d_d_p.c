                  ...
                  double * const restrict INT__g_d_d_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_d_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_p_s = primwork + 85;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_s_s = primwork + 139;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_p_s = primwork + 209;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_d_s = primwork + 299;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 419;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 509;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_d_s = primwork + 644;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_f_s = primwork + 824;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 974;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_p_s = primwork + 1079;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_d_s = primwork + 1268;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_f_s = primwork + 1520;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_s_s = primwork + 1730;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_p_s = primwork + 1842;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_d_s = primwork + 2094;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_f_s = primwork + 2430;
    ...
    double * const HRR_INT__h_p_d_s = hrrwork + 2160;
    double * const HRR_INT__h_p_f_s = hrrwork + 2538;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_d_p = INT__g_d_d_p + real_abcd * 1620;
                  ...
                  double * const restrict INT__d_g_d_p)
{
    ...
    int ret = ostei_g_d_d_p(P_tmp, Q, screen_tol, work, INT__d_g_d_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__d_g_d_p[q*1620+b*108+a*18+c*3+d];

        memcpy(INT__d_g_d_p+q*1620, buffer, 1620*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_d_p_d)
{
    ...
    int ret = ostei_g_d_d_p(P, Q_tmp, screen_tol, work, INT__g_d_p_d);
    double buffer[1620] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__g_d_p_d[q*1620+a*108+b*18+d*3+c];

        memcpy(INT__g_d_p_d+q*1620, buffer, 1620*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_g_p_d)
{
    ...
    int ret = ostei_g_d_d_p(P_tmp, Q_tmp, screen_tol, work, INT__d_g_p_d);
    double buffer[1620] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__d_g_p_d[q*1620+b*108+a*18+d*3+c];

        memcpy(INT__d_g_p_d+q*1620, buffer, 1620*sizeof(double));
    }
