                  ...
                  double * const restrict INT__d_d_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_d_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__g_s_g_s = work + (SIMINT_NSHELL_SIMD * 1024);
    double * const INT__g_s_h_s = work + (SIMINT_NSHELL_SIMD * 1249);
    double * const INT__g_s_i_s = work + (SIMINT_NSHELL_SIMD * 1564);
    ...
    double * const HRR_INT__d_d_g_s = hrrwork + 1152;
    double * const HRR_INT__d_d_g_p = hrrwork + 1692;
    double * const HRR_INT__d_d_h_s = hrrwork + 3312;
    double * const HRR_INT__d_d_h_p = hrrwork + 4068;
    double * const HRR_INT__d_d_i_s = hrrwork + 6336;
    double * const HRR_INT__f_p_g_s = hrrwork + 7344;
    double * const HRR_INT__f_p_h_s = hrrwork + 7794;
    double * const HRR_INT__f_p_i_s = hrrwork + 8424;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_d_g_d = INT__d_d_g_d + real_abcd * 3240;
                  ...
                  double * const restrict INT__d_d_d_g)
{
    ...
    int ret = ostei_d_d_g_d(P, Q_tmp, screen_tol, work, INT__d_d_d_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_d_d_g[q*3240+a*540+b*90+d*6+c];

        memcpy(INT__d_d_d_g+q*3240, buffer, 3240*sizeof(double));
    }
