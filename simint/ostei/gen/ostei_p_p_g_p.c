                  ...
                  double * const restrict INT__p_p_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 108);
    double * const INT__d_s_h_s = work + (SIMINT_NSHELL_SIMD * 198);
    ...
    double * const HRR_INT__p_p_h_s = hrrwork + 135;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_p_g_p = INT__p_p_g_p + real_abcd * 405;
                  ...
                  double * const restrict INT__p_p_p_g)
{
    ...
    int ret = ostei_p_p_g_p(P, Q_tmp, screen_tol, work, INT__p_p_p_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_p_p_g[q*405+a*135+b*45+d*3+c];

        memcpy(INT__p_p_p_g+q*405, buffer, 405*sizeof(double));
    }
