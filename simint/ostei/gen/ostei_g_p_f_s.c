                  ...
                  double * const restrict INT__g_p_f_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_f_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*360);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 360 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_f_s = INT__g_p_f_s + real_abcd * 450;
                  ...
                  double * const restrict INT__p_g_f_s)
{
    ...
    int ret = ostei_g_p_f_s(P_tmp, Q, screen_tol, work, INT__p_g_f_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__p_g_f_s[q*450+b*30+a*10+c*1+d];

        memcpy(INT__p_g_f_s+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_s_f)
{
    ...
    int ret = ostei_g_p_f_s(P, Q_tmp, screen_tol, work, INT__g_p_s_f);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_p_s_f[q*450+a*30+b*10+d*1+c];

        memcpy(INT__g_p_s_f+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_s_f)
{
    ...
    int ret = ostei_g_p_f_s(P_tmp, Q_tmp, screen_tol, work, INT__p_g_s_f);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__p_g_s_f[q*450+b*30+a*10+d*1+c];

        memcpy(INT__p_g_s_f+q*450, buffer, 450*sizeof(double));
    }
