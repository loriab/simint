                  ...
                  double * const restrict INT__p_p_p_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_p_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__p_s_p_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__p_s_d_s = work + (SIMINT_NSHELL_SIMD * 9);
    double * const INT__d_s_p_s = work + (SIMINT_NSHELL_SIMD * 27);
    double * const INT__d_s_d_s = work + (SIMINT_NSHELL_SIMD * 45);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*81);
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_s_s = primwork + 0;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_p_s = primwork + 5;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_s_s = primwork + 11;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_p_s = primwork + 23;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_d_s = primwork + 41;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_s = primwork + 59;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_p_s = primwork + 77;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_d_s = primwork + 113;
    double * const hrrwork = (double *)(primwork + 149);
    double * const HRR_INT__p_p_p_s = hrrwork + 0;
    double * const HRR_INT__p_p_d_s = hrrwork + 27;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 81 * sizeof(double));
            abcd = 0;
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__p_s_p_s = INT__p_s_p_s + abcd * 9;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__p_s_p_s += lastoffset*9;
                    ...
                    // Forming PRIM_INT__p_s_d_s[1 * 18];
                    for(n = 0; n < 1; ++n)  // loop over orders of auxiliary function
                    {
                        ...
                        PRIM_INT__p_s_d_s[n * 18 + 2] = SIMINT_MUL(Q_PA[2], PRIM_INT__p_s_p_s[n * 9 + 0]);
                        PRIM_INT__p_s_d_s[n * 18 + 2] = SIMINT_FMADD( aoq_PQ[2], PRIM_INT__p_s_p_s[(n+1) * 9 + 0], PRIM_INT__p_s_d_s[n * 18 + 2]);
                        ...
                        PRIM_INT__p_s_d_s[n * 18 + 4] = SIMINT_MUL(Q_PA[2], PRIM_INT__p_s_p_s[n * 9 + 1]);
                        PRIM_INT__p_s_d_s[n * 18 + 4] = SIMINT_FMADD( aoq_PQ[2], PRIM_INT__p_s_p_s[(n+1) * 9 + 1], PRIM_INT__p_s_d_s[n * 18 + 4]);
                        ...
                        PRIM_INT__p_s_d_s[n * 18 + 8] = SIMINT_MUL(Q_PA[2], PRIM_INT__p_s_p_s[n * 9 + 3]);
                        PRIM_INT__p_s_d_s[n * 18 + 8] = SIMINT_FMADD( aoq_PQ[2], PRIM_INT__p_s_p_s[(n+1) * 9 + 3], PRIM_INT__p_s_d_s[n * 18 + 8]);
                        ...
                        PRIM_INT__p_s_d_s[n * 18 + 10] = SIMINT_MUL(Q_PA[2], PRIM_INT__p_s_p_s[n * 9 + 4]);
                        PRIM_INT__p_s_d_s[n * 18 + 10] = SIMINT_FMADD( aoq_PQ[2], PRIM_INT__p_s_p_s[(n+1) * 9 + 4], PRIM_INT__p_s_d_s[n * 18 + 10]);
                        ...
                        PRIM_INT__p_s_d_s[n * 18 + 14] = SIMINT_MUL(Q_PA[2], PRIM_INT__p_s_p_s[n * 9 + 6]);
                        PRIM_INT__p_s_d_s[n * 18 + 14] = SIMINT_FMADD( aoq_PQ[2], PRIM_INT__p_s_p_s[(n+1) * 9 + 6], PRIM_INT__p_s_d_s[n * 18 + 14]);
                        ...
                        PRIM_INT__p_s_d_s[n * 18 + 16] = SIMINT_MUL(Q_PA[2], PRIM_INT__p_s_p_s[n * 9 + 7]);
                        PRIM_INT__p_s_d_s[n * 18 + 16] = SIMINT_FMADD( aoq_PQ[2], PRIM_INT__p_s_p_s[(n+1) * 9 + 7], PRIM_INT__p_s_d_s[n * 18 + 16]);
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(9, PRIM_INT__p_s_p_s, PRIM_PTR_INT__p_s_p_s);
                    ...
                    {
                        contract(9, shelloffsets, PRIM_INT__p_s_p_s, PRIM_PTR_INT__p_s_p_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__p_s_p_s = INT__p_s_p_s + abcd * 9;
                ...
                double * restrict HRR_INT__p_p_p_p = INT__p_p_p_p + real_abcd * 81;

                // form INT__p_p_p_s
                for(iket = 0; iket < 3; ++iket)
                {
                    HRR_INT__p_p_p_s[0 * 3 + iket] = HRR_INT__d_s_p_s[0 * 3 + iket] + ( hAB[0] * HRR_INT__p_s_p_s[0 * 3 + iket] );

                    HRR_INT__p_p_p_s[1 * 3 + iket] = HRR_INT__d_s_p_s[1 * 3 + iket] + ( hAB[1] * HRR_INT__p_s_p_s[0 * 3 + iket] );

                    HRR_INT__p_p_p_s[2 * 3 + iket] = HRR_INT__d_s_p_s[2 * 3 + iket] + ( hAB[2] * HRR_INT__p_s_p_s[0 * 3 + iket] );

                    HRR_INT__p_p_p_s[3 * 3 + iket] = HRR_INT__d_s_p_s[1 * 3 + iket] + ( hAB[0] * HRR_INT__p_s_p_s[1 * 3 + iket] );

                    HRR_INT__p_p_p_s[4 * 3 + iket] = HRR_INT__d_s_p_s[3 * 3 + iket] + ( hAB[1] * HRR_INT__p_s_p_s[1 * 3 + iket] );

                    HRR_INT__p_p_p_s[5 * 3 + iket] = HRR_INT__d_s_p_s[4 * 3 + iket] + ( hAB[2] * HRR_INT__p_s_p_s[1 * 3 + iket] );

                    HRR_INT__p_p_p_s[6 * 3 + iket] = HRR_INT__d_s_p_s[2 * 3 + iket] + ( hAB[0] * HRR_INT__p_s_p_s[2 * 3 + iket] );

                    HRR_INT__p_p_p_s[7 * 3 + iket] = HRR_INT__d_s_p_s[4 * 3 + iket] + ( hAB[1] * HRR_INT__p_s_p_s[2 * 3 + iket] );

                    HRR_INT__p_p_p_s[8 * 3 + iket] = HRR_INT__d_s_p_s[5 * 3 + iket] + ( hAB[2] * HRR_INT__p_s_p_s[2 * 3 + iket] );
                ...
                {
                    HRR_INT__p_p_p_p[ibra * 9 + 0] = HRR_INT__p_p_d_s[ibra * 6 + 0] + ( hCD[0] * HRR_INT__p_p_p_s[ibra * 3 + 0] );

                    HRR_INT__p_p_p_p[ibra * 9 + 1] = HRR_INT__p_p_d_s[ibra * 6 + 1] + ( hCD[1] * HRR_INT__p_p_p_s[ibra * 3 + 0] );

                    HRR_INT__p_p_p_p[ibra * 9 + 2] = HRR_INT__p_p_d_s[ibra * 6 + 2] + ( hCD[2] * HRR_INT__p_p_p_s[ibra * 3 + 0] );

                    HRR_INT__p_p_p_p[ibra * 9 + 3] = HRR_INT__p_p_d_s[ibra * 6 + 1] + ( hCD[0] * HRR_INT__p_p_p_s[ibra * 3 + 1] );

                    HRR_INT__p_p_p_p[ibra * 9 + 4] = HRR_INT__p_p_d_s[ibra * 6 + 3] + ( hCD[1] * HRR_INT__p_p_p_s[ibra * 3 + 1] );

                    HRR_INT__p_p_p_p[ibra * 9 + 5] = HRR_INT__p_p_d_s[ibra * 6 + 4] + ( hCD[2] * HRR_INT__p_p_p_s[ibra * 3 + 1] );

                    HRR_INT__p_p_p_p[ibra * 9 + 6] = HRR_INT__p_p_d_s[ibra * 6 + 2] + ( hCD[0] * HRR_INT__p_p_p_s[ibra * 3 + 2] );

                    HRR_INT__p_p_p_p[ibra * 9 + 7] = HRR_INT__p_p_d_s[ibra * 6 + 4] + ( hCD[1] * HRR_INT__p_p_p_s[ibra * 3 + 2] );

                    HRR_INT__p_p_p_p[ibra * 9 + 8] = HRR_INT__p_p_d_s[ibra * 6 + 5] + ( hCD[2] * HRR_INT__p_p_p_s[ibra * 3 + 2] );
