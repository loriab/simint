                  ...
                  double * const restrict INT__p_s_g_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_s_g_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*300);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_k_s = primwork + 378;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 450;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 495;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 558;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_k_s = primwork + 642;
    double * const hrrwork = (double *)(primwork + 750);
    double * const HRR_INT__p_s_g_p = hrrwork + 0;
    double * const HRR_INT__p_s_g_d = hrrwork + 135;
    double * const HRR_INT__p_s_h_p = hrrwork + 405;
    double * const HRR_INT__p_s_h_d = hrrwork + 594;
    double * const HRR_INT__p_s_i_p = hrrwork + 972;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 300 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_s_g_f = INT__p_s_g_f + real_abcd * 450;
                  ...
                  double * const restrict INT__s_p_g_f)
{
    ...
    int ret = ostei_p_s_g_f(P_tmp, Q, screen_tol, work, INT__s_p_g_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__s_p_g_f[q*450+b*150+a*150+c*10+d];

        memcpy(INT__s_p_g_f+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_s_f_g)
{
    ...
    int ret = ostei_p_s_g_f(P, Q_tmp, screen_tol, work, INT__p_s_f_g);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_s_f_g[q*450+a*150+b*150+d*10+c];

        memcpy(INT__p_s_f_g+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_p_f_g)
{
    ...
    int ret = ostei_p_s_g_f(P_tmp, Q_tmp, screen_tol, work, INT__s_p_f_g);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_p_f_g[q*450+b*150+a*150+d*10+c];

        memcpy(INT__s_p_f_g+q*450, buffer, 450*sizeof(double));
    }
