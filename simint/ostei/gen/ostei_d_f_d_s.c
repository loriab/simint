                  ...
                  double * const restrict INT__d_f_d_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_f_d_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__s_g_d_s = work + (SIMINT_NSHELL_SIMD * 60);
    double * const INT__s_h_d_s = work + (SIMINT_NSHELL_SIMD * 150);
    ...
    double * const HRR_INT__p_g_d_s = hrrwork + 180;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_f_d_s = INT__d_f_d_s + real_abcd * 360;
