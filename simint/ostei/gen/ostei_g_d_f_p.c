                  ...
                  double * const restrict INT__g_d_f_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_f_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__h_p_f_s = hrrwork + 3375;
    double * const HRR_INT__h_p_g_s = hrrwork + 4005;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_f_p = INT__g_d_f_p + real_abcd * 2700;
                  ...
                  double * const restrict INT__d_g_f_p)
{
    ...
    int ret = ostei_g_d_f_p(P_tmp, Q, screen_tol, work, INT__d_g_f_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__d_g_f_p[q*2700+b*180+a*30+c*3+d];

        memcpy(INT__d_g_f_p+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_d_p_f)
{
    ...
    int ret = ostei_g_d_f_p(P, Q_tmp, screen_tol, work, INT__g_d_p_f);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_d_p_f[q*2700+a*180+b*30+d*3+c];

        memcpy(INT__g_d_p_f+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_g_p_f)
{
    ...
    int ret = ostei_g_d_f_p(P_tmp, Q_tmp, screen_tol, work, INT__d_g_p_f);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__d_g_p_f[q*2700+b*180+a*30+d*3+c];

        memcpy(INT__d_g_p_f+q*2700, buffer, 2700*sizeof(double));
    }
