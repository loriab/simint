                  ...
                  double * const restrict INT__d_p_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_p_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_d_s = primwork + 175;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 229;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 319;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_f_s = primwork + 454;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 574;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_g_s = primwork + 754;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_p_g_s = INT__d_p_g_s + real_abcd * 270;
                  ...
                  double * const restrict INT__p_d_g_s)
{
    ...
    int ret = ostei_d_p_g_s(P_tmp, Q, screen_tol, work, INT__p_d_g_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__p_d_g_s[q*270+b*45+a*15+c*1+d];

        memcpy(INT__p_d_g_s+q*270, buffer, 270*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_p_s_g)
{
    ...
    int ret = ostei_d_p_g_s(P, Q_tmp, screen_tol, work, INT__d_p_s_g);
    double buffer[270] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_p_s_g[q*270+a*45+b*15+d*1+c];

        memcpy(INT__d_p_s_g+q*270, buffer, 270*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_d_s_g)
{
    ...
    int ret = ostei_d_p_g_s(P_tmp, Q_tmp, screen_tol, work, INT__p_d_s_g);
    double buffer[270] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_d_s_g[q*270+b*45+a*15+d*1+c];

        memcpy(INT__p_d_s_g+q*270, buffer, 270*sizeof(double));
    }
