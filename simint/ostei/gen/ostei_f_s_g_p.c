                  ...
                  double * const restrict INT__f_s_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_s_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_s_g_p = INT__f_s_g_p + real_abcd * 450;
                  ...
                  double * const restrict INT__s_f_g_p)
{
    ...
    int ret = ostei_f_s_g_p(P_tmp, Q, screen_tol, work, INT__s_f_g_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__s_f_g_p[q*450+b*45+a*45+c*3+d];

        memcpy(INT__s_f_g_p+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_s_p_g)
{
    ...
    int ret = ostei_f_s_g_p(P, Q_tmp, screen_tol, work, INT__f_s_p_g);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_s_p_g[q*450+a*45+b*45+d*3+c];

        memcpy(INT__f_s_p_g+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_f_p_g)
{
    ...
    int ret = ostei_f_s_g_p(P_tmp, Q_tmp, screen_tol, work, INT__s_f_p_g);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_f_p_g[q*450+b*45+a*45+d*3+c];

        memcpy(INT__s_f_p_g+q*450, buffer, 450*sizeof(double));
    }
