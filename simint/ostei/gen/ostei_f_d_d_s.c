                  ...
                  double * const restrict INT__f_d_d_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_d_d_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__g_s_d_s = work + (SIMINT_NSHELL_SIMD * 60);
    double * const INT__h_s_d_s = work + (SIMINT_NSHELL_SIMD * 150);
    ...
    double * const HRR_INT__g_p_d_s = hrrwork + 180;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_d_d_s = INT__f_d_d_s + real_abcd * 360;
