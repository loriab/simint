                  ...
                  double * const restrict INT__f_p_g_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_p_g_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__f_p_g_p = hrrwork + 450;
    double * const HRR_INT__f_p_g_d = hrrwork + 1800;
    double * const HRR_INT__f_p_h_s = hrrwork + 4500;
    double * const HRR_INT__f_p_h_p = hrrwork + 5130;
    double * const HRR_INT__f_p_h_d = hrrwork + 7020;
    double * const HRR_INT__f_p_i_s = hrrwork + 10800;
    double * const HRR_INT__f_p_i_p = hrrwork + 11640;
    double * const HRR_INT__f_p_k_s = hrrwork + 14160;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_p_g_f = INT__f_p_g_f + real_abcd * 4500;
                  ...
                  double * const restrict INT__p_f_g_f)
{
    ...
    int ret = ostei_f_p_g_f(P_tmp, Q, screen_tol, work, INT__p_f_g_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__p_f_g_f[q*4500+b*450+a*150+c*10+d];

        memcpy(INT__p_f_g_f+q*4500, buffer, 4500*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_p_f_g)
{
    ...
    int ret = ostei_f_p_g_f(P, Q_tmp, screen_tol, work, INT__f_p_f_g);
    double buffer[4500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_p_f_g[q*4500+a*450+b*150+d*10+c];

        memcpy(INT__f_p_f_g+q*4500, buffer, 4500*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_f_f_g)
{
    ...
    int ret = ostei_f_p_g_f(P_tmp, Q_tmp, screen_tol, work, INT__p_f_f_g);
    double buffer[4500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_f_f_g[q*4500+b*450+a*150+d*10+c];

        memcpy(INT__p_f_f_g+q*4500, buffer, 4500*sizeof(double));
    }
