                  ...
                  double * const restrict INT__f_d_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_d_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_d_g_s = INT__f_d_g_s + real_abcd * 900;
                  ...
                  double * const restrict INT__d_f_g_s)
{
    ...
    int ret = ostei_f_d_g_s(P_tmp, Q, screen_tol, work, INT__d_f_g_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__d_f_g_s[q*900+b*90+a*15+c*1+d];

        memcpy(INT__d_f_g_s+q*900, buffer, 900*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_d_s_g)
{
    ...
    int ret = ostei_f_d_g_s(P, Q_tmp, screen_tol, work, INT__f_d_s_g);
    double buffer[900] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_d_s_g[q*900+a*90+b*15+d*1+c];

        memcpy(INT__f_d_s_g+q*900, buffer, 900*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_f_s_g)
{
    ...
    int ret = ostei_f_d_g_s(P_tmp, Q_tmp, screen_tol, work, INT__d_f_s_g);
    double buffer[900] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_f_s_g[q*900+b*90+a*15+d*1+c];

        memcpy(INT__d_f_s_g+q*900, buffer, 900*sizeof(double));
    }
