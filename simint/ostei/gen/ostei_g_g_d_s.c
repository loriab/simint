                  ...
                  double * const restrict INT__g_g_d_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_g_d_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__h_s_d_s = work + (SIMINT_NSHELL_SIMD * 90);
    double * const INT__i_s_d_s = work + (SIMINT_NSHELL_SIMD * 216);
    double * const INT__k_s_d_s = work + (SIMINT_NSHELL_SIMD * 384);
    double * const INT__l_s_d_s = work + (SIMINT_NSHELL_SIMD * 600);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*870);
    ...
    double * const HRR_INT__g_d_d_s = hrrwork + 270;
    double * const HRR_INT__g_f_d_s = hrrwork + 810;
    double * const HRR_INT__h_p_d_s = hrrwork + 1710;
    double * const HRR_INT__h_d_d_s = hrrwork + 2088;
    double * const HRR_INT__h_f_d_s = hrrwork + 2844;
    double * const HRR_INT__i_p_d_s = hrrwork + 4104;
    double * const HRR_INT__i_d_d_s = hrrwork + 4608;
    double * const HRR_INT__k_p_d_s = hrrwork + 5616;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 870 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_g_d_s = INT__g_g_d_s + real_abcd * 1350;
                  ...
                  double * const restrict INT__g_g_s_d)
{
    ...
    int ret = ostei_g_g_d_s(P, Q_tmp, screen_tol, work, INT__g_g_s_d);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__g_g_s_d[q*1350+a*90+b*6+d*1+c];

        memcpy(INT__g_g_s_d+q*1350, buffer, 1350*sizeof(double));
    }
