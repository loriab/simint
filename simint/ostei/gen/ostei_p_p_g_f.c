                  ...
                  double * const restrict INT__p_p_g_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_g_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 300);
    double * const INT__d_s_h_s = work + (SIMINT_NSHELL_SIMD * 390);
    double * const INT__d_s_i_s = work + (SIMINT_NSHELL_SIMD * 516);
    double * const INT__d_s_k_s = work + (SIMINT_NSHELL_SIMD * 684);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_k_s = primwork + 462;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 570;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 630;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 720;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 846;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_k_s = primwork + 1014;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 1230;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_h_s = primwork + 1320;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_i_s = primwork + 1446;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_k_s = primwork + 1614;
    double * const hrrwork = (double *)(primwork + 1830);
    ...
    double * const HRR_INT__p_p_h_s = hrrwork + 1350;
    double * const HRR_INT__p_p_h_p = hrrwork + 1539;
    double * const HRR_INT__p_p_h_d = hrrwork + 2106;
    double * const HRR_INT__p_p_i_s = hrrwork + 3240;
    double * const HRR_INT__p_p_i_p = hrrwork + 3492;
    double * const HRR_INT__p_p_k_s = hrrwork + 4248;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_p_g_f = INT__p_p_g_f + real_abcd * 1350;
                  ...
                  double * const restrict INT__p_p_f_g)
{
    ...
    int ret = ostei_p_p_g_f(P, Q_tmp, screen_tol, work, INT__p_p_f_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_p_f_g[q*1350+a*450+b*150+d*10+c];

        memcpy(INT__p_p_f_g+q*1350, buffer, 1350*sizeof(double));
    }
