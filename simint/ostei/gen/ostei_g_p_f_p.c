                  ...
                  double * const restrict INT__g_p_f_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_f_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*900);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 900 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_f_p = INT__g_p_f_p + real_abcd * 1350;
                  ...
                  double * const restrict INT__p_g_f_p)
{
    ...
    int ret = ostei_g_p_f_p(P_tmp, Q, screen_tol, work, INT__p_g_f_p);
    double buffer[1350] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__p_g_f_p[q*1350+b*90+a*30+c*3+d];

        memcpy(INT__p_g_f_p+q*1350, buffer, 1350*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_p_f)
{
    ...
    int ret = ostei_g_p_f_p(P, Q_tmp, screen_tol, work, INT__g_p_p_f);
    double buffer[1350] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_p_p_f[q*1350+a*90+b*30+d*3+c];

        memcpy(INT__g_p_p_f+q*1350, buffer, 1350*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_p_f)
{
    ...
    int ret = ostei_g_p_f_p(P_tmp, Q_tmp, screen_tol, work, INT__p_g_p_f);
    double buffer[1350] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__p_g_p_f[q*1350+b*90+a*30+d*3+c];

        memcpy(INT__p_g_p_f+q*1350, buffer, 1350*sizeof(double));
    }
