                  ...
                  double * const restrict INT__g_s_f_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_s_f_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_s_f_p = INT__g_s_f_p + real_abcd * 450;
                  ...
                  double * const restrict INT__s_g_f_p)
{
    ...
    int ret = ostei_g_s_f_p(P_tmp, Q, screen_tol, work, INT__s_g_f_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__s_g_f_p[q*450+b*30+a*30+c*3+d];

        memcpy(INT__s_g_f_p+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_s_p_f)
{
    ...
    int ret = ostei_g_s_f_p(P, Q_tmp, screen_tol, work, INT__g_s_p_f);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_s_p_f[q*450+a*30+b*30+d*3+c];

        memcpy(INT__g_s_p_f+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_g_p_f)
{
    ...
    int ret = ostei_g_s_f_p(P_tmp, Q_tmp, screen_tol, work, INT__s_g_p_f);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__s_g_p_f[q*450+b*30+a*30+d*3+c];

        memcpy(INT__s_g_p_f+q*450, buffer, 450*sizeof(double));
    }
