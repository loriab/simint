                  ...
                  double * const restrict INT__g_g_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_g_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__l_s_g_s = work + (SIMINT_NSHELL_SIMD * 1500);
    ...
    double * const HRR_INT__g_f_g_s = hrrwork + 2025;
    double * const HRR_INT__h_p_g_s = hrrwork + 4275;
    double * const HRR_INT__h_d_g_s = hrrwork + 5220;
    double * const HRR_INT__h_f_g_s = hrrwork + 7110;
    double * const HRR_INT__i_p_g_s = hrrwork + 10260;
    double * const HRR_INT__i_d_g_s = hrrwork + 11520;
    double * const HRR_INT__k_p_g_s = hrrwork + 14040;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_g_g_s = INT__g_g_g_s + real_abcd * 3375;
                  ...
                  double * const restrict INT__g_g_s_g)
{
    ...
    int ret = ostei_g_g_g_s(P, Q_tmp, screen_tol, work, INT__g_g_s_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_g_s_g[q*3375+a*225+b*15+d*1+c];

        memcpy(INT__g_g_s_g+q*3375, buffer, 3375*sizeof(double));
    }
