                  ...
                  double * const restrict INT__p_d_f_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_d_f_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_d_f_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_d_g_s = work + (SIMINT_NSHELL_SIMD * 60);
    double * const INT__s_d_h_s = work + (SIMINT_NSHELL_SIMD * 150);
    double * const INT__s_d_i_s = work + (SIMINT_NSHELL_SIMD * 276);
    double * const INT__s_f_f_s = work + (SIMINT_NSHELL_SIMD * 444);
    double * const INT__s_f_g_s = work + (SIMINT_NSHELL_SIMD * 544);
    double * const INT__s_f_h_s = work + (SIMINT_NSHELL_SIMD * 694);
    double * const INT__s_f_i_s = work + (SIMINT_NSHELL_SIMD * 904);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_p_s = primwork + 462;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_d_s = primwork + 489;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_f_s = primwork + 543;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_g_s = primwork + 633;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_h_s = primwork + 768;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_i_s = primwork + 957;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_d_s = primwork + 1209;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_f_s = primwork + 1281;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_g_s = primwork + 1401;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_h_s = primwork + 1581;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_i_s = primwork + 1833;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_f_s = primwork + 2169;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_g_s = primwork + 2269;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_h_s = primwork + 2419;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_i_s = primwork + 2629;
    double * const hrrwork = (double *)(primwork + 2909);
    double * const HRR_INT__p_d_f_s = hrrwork + 0;
    double * const HRR_INT__p_d_f_p = hrrwork + 180;
    double * const HRR_INT__p_d_f_d = hrrwork + 720;
    double * const HRR_INT__p_d_g_s = hrrwork + 1800;
    double * const HRR_INT__p_d_g_p = hrrwork + 2070;
    double * const HRR_INT__p_d_g_d = hrrwork + 2880;
    double * const HRR_INT__p_d_h_s = hrrwork + 4500;
    double * const HRR_INT__p_d_h_p = hrrwork + 4878;
    double * const HRR_INT__p_d_i_s = hrrwork + 6012;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_d_h_s = INT__s_d_h_s + abcd * 126;
                double * restrict PRIM_PTR_INT__s_d_i_s = INT__s_d_i_s + abcd * 168;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__s_d_h_s += 126;
                            PRIM_PTR_INT__s_d_i_s += 168;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            ...
                            PRIM_PTR_INT__s_d_h_s += lastoffset*126;
                            PRIM_PTR_INT__s_d_i_s += lastoffset*168;
                    ...
                    if(lastoffset == 0)
                    {
                        ...
                        contract_all(126, PRIM_INT__s_d_h_s, PRIM_PTR_INT__s_d_h_s);
                        contract_all(168, PRIM_INT__s_d_i_s, PRIM_PTR_INT__s_d_i_s);
                    ...
                    else
                    {
                        ...
                        contract(126, shelloffsets, PRIM_INT__s_d_h_s, PRIM_PTR_INT__s_d_h_s);
                        contract(168, shelloffsets, PRIM_INT__s_d_i_s, PRIM_PTR_INT__s_d_i_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double const * restrict HRR_INT__s_d_h_s = INT__s_d_h_s + abcd * 126;
                double const * restrict HRR_INT__s_d_i_s = INT__s_d_i_s + abcd * 168;
                ...
                double * restrict HRR_INT__p_d_f_f = INT__p_d_f_f + real_abcd * 1800;
                ...
                {
                    HRR_INT__p_d_h_s[0 * 21 + iket] = HRR_INT__s_f_h_s[0 * 21 + iket] - ( hAB[0] * HRR_INT__s_d_h_s[0 * 21 + iket] );

                    HRR_INT__p_d_h_s[1 * 21 + iket] = HRR_INT__s_f_h_s[1 * 21 + iket] - ( hAB[0] * HRR_INT__s_d_h_s[1 * 21 + iket] );

                    HRR_INT__p_d_h_s[2 * 21 + iket] = HRR_INT__s_f_h_s[2 * 21 + iket] - ( hAB[0] * HRR_INT__s_d_h_s[2 * 21 + iket] );

                    HRR_INT__p_d_h_s[3 * 21 + iket] = HRR_INT__s_f_h_s[3 * 21 + iket] - ( hAB[0] * HRR_INT__s_d_h_s[3 * 21 + iket] );

                    HRR_INT__p_d_h_s[4 * 21 + iket] = HRR_INT__s_f_h_s[4 * 21 + iket] - ( hAB[0] * HRR_INT__s_d_h_s[4 * 21 + iket] );

                    HRR_INT__p_d_h_s[5 * 21 + iket] = HRR_INT__s_f_h_s[5 * 21 + iket] - ( hAB[0] * HRR_INT__s_d_h_s[5 * 21 + iket] );

                    HRR_INT__p_d_h_s[6 * 21 + iket] = HRR_INT__s_f_h_s[1 * 21 + iket] - ( hAB[1] * HRR_INT__s_d_h_s[0 * 21 + iket] );

                    HRR_INT__p_d_h_s[7 * 21 + iket] = HRR_INT__s_f_h_s[3 * 21 + iket] - ( hAB[1] * HRR_INT__s_d_h_s[1 * 21 + iket] );

                    HRR_INT__p_d_h_s[8 * 21 + iket] = HRR_INT__s_f_h_s[4 * 21 + iket] - ( hAB[1] * HRR_INT__s_d_h_s[2 * 21 + iket] );

                    HRR_INT__p_d_h_s[9 * 21 + iket] = HRR_INT__s_f_h_s[6 * 21 + iket] - ( hAB[1] * HRR_INT__s_d_h_s[3 * 21 + iket] );

                    HRR_INT__p_d_h_s[10 * 21 + iket] = HRR_INT__s_f_h_s[7 * 21 + iket] - ( hAB[1] * HRR_INT__s_d_h_s[4 * 21 + iket] );

                    HRR_INT__p_d_h_s[11 * 21 + iket] = HRR_INT__s_f_h_s[8 * 21 + iket] - ( hAB[1] * HRR_INT__s_d_h_s[5 * 21 + iket] );

                    HRR_INT__p_d_h_s[12 * 21 + iket] = HRR_INT__s_f_h_s[2 * 21 + iket] - ( hAB[2] * HRR_INT__s_d_h_s[0 * 21 + iket] );

                    HRR_INT__p_d_h_s[13 * 21 + iket] = HRR_INT__s_f_h_s[4 * 21 + iket] - ( hAB[2] * HRR_INT__s_d_h_s[1 * 21 + iket] );

                    HRR_INT__p_d_h_s[14 * 21 + iket] = HRR_INT__s_f_h_s[5 * 21 + iket] - ( hAB[2] * HRR_INT__s_d_h_s[2 * 21 + iket] );

                    HRR_INT__p_d_h_s[15 * 21 + iket] = HRR_INT__s_f_h_s[7 * 21 + iket] - ( hAB[2] * HRR_INT__s_d_h_s[3 * 21 + iket] );

                    HRR_INT__p_d_h_s[16 * 21 + iket] = HRR_INT__s_f_h_s[8 * 21 + iket] - ( hAB[2] * HRR_INT__s_d_h_s[4 * 21 + iket] );

                    HRR_INT__p_d_h_s[17 * 21 + iket] = HRR_INT__s_f_h_s[9 * 21 + iket] - ( hAB[2] * HRR_INT__s_d_h_s[5 * 21 + iket] );
                ...
                {
                    HRR_INT__p_d_i_s[0 * 28 + iket] = HRR_INT__s_f_i_s[0 * 28 + iket] - ( hAB[0] * HRR_INT__s_d_i_s[0 * 28 + iket] );

                    HRR_INT__p_d_i_s[1 * 28 + iket] = HRR_INT__s_f_i_s[1 * 28 + iket] - ( hAB[0] * HRR_INT__s_d_i_s[1 * 28 + iket] );

                    HRR_INT__p_d_i_s[2 * 28 + iket] = HRR_INT__s_f_i_s[2 * 28 + iket] - ( hAB[0] * HRR_INT__s_d_i_s[2 * 28 + iket] );

                    HRR_INT__p_d_i_s[3 * 28 + iket] = HRR_INT__s_f_i_s[3 * 28 + iket] - ( hAB[0] * HRR_INT__s_d_i_s[3 * 28 + iket] );

                    HRR_INT__p_d_i_s[4 * 28 + iket] = HRR_INT__s_f_i_s[4 * 28 + iket] - ( hAB[0] * HRR_INT__s_d_i_s[4 * 28 + iket] );

                    HRR_INT__p_d_i_s[5 * 28 + iket] = HRR_INT__s_f_i_s[5 * 28 + iket] - ( hAB[0] * HRR_INT__s_d_i_s[5 * 28 + iket] );

                    HRR_INT__p_d_i_s[6 * 28 + iket] = HRR_INT__s_f_i_s[1 * 28 + iket] - ( hAB[1] * HRR_INT__s_d_i_s[0 * 28 + iket] );

                    HRR_INT__p_d_i_s[7 * 28 + iket] = HRR_INT__s_f_i_s[3 * 28 + iket] - ( hAB[1] * HRR_INT__s_d_i_s[1 * 28 + iket] );

                    HRR_INT__p_d_i_s[8 * 28 + iket] = HRR_INT__s_f_i_s[4 * 28 + iket] - ( hAB[1] * HRR_INT__s_d_i_s[2 * 28 + iket] );

                    HRR_INT__p_d_i_s[9 * 28 + iket] = HRR_INT__s_f_i_s[6 * 28 + iket] - ( hAB[1] * HRR_INT__s_d_i_s[3 * 28 + iket] );

                    HRR_INT__p_d_i_s[10 * 28 + iket] = HRR_INT__s_f_i_s[7 * 28 + iket] - ( hAB[1] * HRR_INT__s_d_i_s[4 * 28 + iket] );

                    HRR_INT__p_d_i_s[11 * 28 + iket] = HRR_INT__s_f_i_s[8 * 28 + iket] - ( hAB[1] * HRR_INT__s_d_i_s[5 * 28 + iket] );

                    HRR_INT__p_d_i_s[12 * 28 + iket] = HRR_INT__s_f_i_s[2 * 28 + iket] - ( hAB[2] * HRR_INT__s_d_i_s[0 * 28 + iket] );

                    HRR_INT__p_d_i_s[13 * 28 + iket] = HRR_INT__s_f_i_s[4 * 28 + iket] - ( hAB[2] * HRR_INT__s_d_i_s[1 * 28 + iket] );

                    HRR_INT__p_d_i_s[14 * 28 + iket] = HRR_INT__s_f_i_s[5 * 28 + iket] - ( hAB[2] * HRR_INT__s_d_i_s[2 * 28 + iket] );

                    HRR_INT__p_d_i_s[15 * 28 + iket] = HRR_INT__s_f_i_s[7 * 28 + iket] - ( hAB[2] * HRR_INT__s_d_i_s[3 * 28 + iket] );

                    HRR_INT__p_d_i_s[16 * 28 + iket] = HRR_INT__s_f_i_s[8 * 28 + iket] - ( hAB[2] * HRR_INT__s_d_i_s[4 * 28 + iket] );

                    HRR_INT__p_d_i_s[17 * 28 + iket] = HRR_INT__s_f_i_s[9 * 28 + iket] - ( hAB[2] * HRR_INT__s_d_i_s[5 * 28 + iket] );
                ...
                for(ibra = 0; ibra < 18; ++ibra)
                {
                    ...
                    HRR_INT__p_d_f_p[ibra * 30 + 1] = HRR_INT__p_d_g_s[ibra * 15 + 1] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 0] );

                    HRR_INT__p_d_f_p[ibra * 30 + 2] = HRR_INT__p_d_g_s[ibra * 15 + 2] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 0] );
                    ...
                    HRR_INT__p_d_f_p[ibra * 30 + 5] = HRR_INT__p_d_g_s[ibra * 15 + 4] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 1] );
                    ...
                    HRR_INT__p_d_f_p[ibra * 30 + 11] = HRR_INT__p_d_g_s[ibra * 15 + 7] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 3] );
                    ...
                    HRR_INT__p_d_f_p[ibra * 30 + 20] = HRR_INT__p_d_g_s[ibra * 15 + 11] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 6] );
