                  ...
                  double * const restrict INT__d_d_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_d_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__f_s_g_s = work + (SIMINT_NSHELL_SIMD * 216);
    double * const INT__f_s_h_s = work + (SIMINT_NSHELL_SIMD * 366);
    double * const INT__g_s_g_s = work + (SIMINT_NSHELL_SIMD * 576);
    double * const INT__g_s_h_s = work + (SIMINT_NSHELL_SIMD * 801);
    ...
    double * const HRR_INT__d_d_g_s = hrrwork + 648;
    double * const HRR_INT__d_d_h_s = hrrwork + 1188;
    double * const HRR_INT__f_p_g_s = hrrwork + 1944;
    double * const HRR_INT__f_p_h_s = hrrwork + 2394;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_d_g_p = INT__d_d_g_p + real_abcd * 1620;
                  ...
                  double * const restrict INT__d_d_p_g)
{
    ...
    int ret = ostei_d_d_g_p(P, Q_tmp, screen_tol, work, INT__d_d_p_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_d_p_g[q*1620+a*270+b*45+d*3+c];

        memcpy(INT__d_d_p_g+q*1620, buffer, 1620*sizeof(double));
    }
