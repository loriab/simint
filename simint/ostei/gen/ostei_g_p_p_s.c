                  ...
                  double * const restrict INT__g_p_p_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_p_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*108);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 95;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 140;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 185;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_p_s = primwork + 227;
    double * const hrrwork = (double *)(primwork + 290);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 108 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_p_s = INT__g_p_p_s + real_abcd * 135;
                  ...
                  double * const restrict INT__p_g_p_s)
{
    ...
    int ret = ostei_g_p_p_s(P_tmp, Q, screen_tol, work, INT__p_g_p_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__p_g_p_s[q*135+b*9+a*3+c*1+d];

        memcpy(INT__p_g_p_s+q*135, buffer, 135*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_s_p)
{
    ...
    int ret = ostei_g_p_p_s(P, Q_tmp, screen_tol, work, INT__g_p_s_p);
    double buffer[135] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__g_p_s_p[q*135+a*9+b*3+d*1+c];

        memcpy(INT__g_p_s_p+q*135, buffer, 135*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_s_p)
{
    ...
    int ret = ostei_g_p_p_s(P_tmp, Q_tmp, screen_tol, work, INT__p_g_s_p);
    double buffer[135] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__p_g_s_p[q*135+b*9+a*3+d*1+c];

        memcpy(INT__p_g_s_p+q*135, buffer, 135*sizeof(double));
    }
