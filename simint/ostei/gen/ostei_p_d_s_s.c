                  ...
                  double * const restrict INT__p_d_s_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_d_s_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_d_s_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_f_s_s = work + (SIMINT_NSHELL_SIMD * 6);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_s_s = primwork + 4;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_s_s = primwork + 13;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_s_s = primwork + 25;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_d_s_s = INT__s_d_s_s + abcd * 6;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__s_d_s_s += lastoffset*6;
                    ...
                    // Forming PRIM_INT__s_f_s_s[1 * 10];
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(6, PRIM_INT__s_d_s_s, PRIM_PTR_INT__s_d_s_s);
                        contract_all(10, PRIM_INT__s_f_s_s, PRIM_PTR_INT__s_f_s_s);
                    }
                    else
                    {
                        contract(6, shelloffsets, PRIM_INT__s_d_s_s, PRIM_PTR_INT__s_d_s_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__s_d_s_s = INT__s_d_s_s + abcd * 6;
                double const * restrict HRR_INT__s_f_s_s = INT__s_f_s_s + abcd * 10;
                double * restrict HRR_INT__p_d_s_s = INT__p_d_s_s + real_abcd * 18;

                // form INT__p_d_s_s
                for(iket = 0; iket < 1; ++iket)
                {
                    HRR_INT__p_d_s_s[0 * 1 + iket] = HRR_INT__s_f_s_s[0 * 1 + iket] - ( hAB[0] * HRR_INT__s_d_s_s[0 * 1 + iket] );

                    HRR_INT__p_d_s_s[1 * 1 + iket] = HRR_INT__s_f_s_s[1 * 1 + iket] - ( hAB[0] * HRR_INT__s_d_s_s[1 * 1 + iket] );

                    HRR_INT__p_d_s_s[2 * 1 + iket] = HRR_INT__s_f_s_s[2 * 1 + iket] - ( hAB[0] * HRR_INT__s_d_s_s[2 * 1 + iket] );

                    HRR_INT__p_d_s_s[3 * 1 + iket] = HRR_INT__s_f_s_s[3 * 1 + iket] - ( hAB[0] * HRR_INT__s_d_s_s[3 * 1 + iket] );

                    HRR_INT__p_d_s_s[4 * 1 + iket] = HRR_INT__s_f_s_s[4 * 1 + iket] - ( hAB[0] * HRR_INT__s_d_s_s[4 * 1 + iket] );

                    HRR_INT__p_d_s_s[5 * 1 + iket] = HRR_INT__s_f_s_s[5 * 1 + iket] - ( hAB[0] * HRR_INT__s_d_s_s[5 * 1 + iket] );

                    HRR_INT__p_d_s_s[6 * 1 + iket] = HRR_INT__s_f_s_s[1 * 1 + iket] - ( hAB[1] * HRR_INT__s_d_s_s[0 * 1 + iket] );

                    HRR_INT__p_d_s_s[7 * 1 + iket] = HRR_INT__s_f_s_s[3 * 1 + iket] - ( hAB[1] * HRR_INT__s_d_s_s[1 * 1 + iket] );

                    HRR_INT__p_d_s_s[8 * 1 + iket] = HRR_INT__s_f_s_s[4 * 1 + iket] - ( hAB[1] * HRR_INT__s_d_s_s[2 * 1 + iket] );

                    HRR_INT__p_d_s_s[9 * 1 + iket] = HRR_INT__s_f_s_s[6 * 1 + iket] - ( hAB[1] * HRR_INT__s_d_s_s[3 * 1 + iket] );

                    HRR_INT__p_d_s_s[10 * 1 + iket] = HRR_INT__s_f_s_s[7 * 1 + iket] - ( hAB[1] * HRR_INT__s_d_s_s[4 * 1 + iket] );

                    HRR_INT__p_d_s_s[11 * 1 + iket] = HRR_INT__s_f_s_s[8 * 1 + iket] - ( hAB[1] * HRR_INT__s_d_s_s[5 * 1 + iket] );

                    HRR_INT__p_d_s_s[12 * 1 + iket] = HRR_INT__s_f_s_s[2 * 1 + iket] - ( hAB[2] * HRR_INT__s_d_s_s[0 * 1 + iket] );

                    HRR_INT__p_d_s_s[13 * 1 + iket] = HRR_INT__s_f_s_s[4 * 1 + iket] - ( hAB[2] * HRR_INT__s_d_s_s[1 * 1 + iket] );

                    HRR_INT__p_d_s_s[14 * 1 + iket] = HRR_INT__s_f_s_s[5 * 1 + iket] - ( hAB[2] * HRR_INT__s_d_s_s[2 * 1 + iket] );

                    HRR_INT__p_d_s_s[15 * 1 + iket] = HRR_INT__s_f_s_s[7 * 1 + iket] - ( hAB[2] * HRR_INT__s_d_s_s[3 * 1 + iket] );

                    HRR_INT__p_d_s_s[16 * 1 + iket] = HRR_INT__s_f_s_s[8 * 1 + iket] - ( hAB[2] * HRR_INT__s_d_s_s[4 * 1 + iket] );

                    HRR_INT__p_d_s_s[17 * 1 + iket] = HRR_INT__s_f_s_s[9 * 1 + iket] - ( hAB[2] * HRR_INT__s_d_s_s[5 * 1 + iket] );
