                  ...
                  double * const restrict INT__g_p_g_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_g_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_p_h_s = hrrwork + 6750;
    double * const HRR_INT__g_p_h_p = hrrwork + 7695;
    double * const HRR_INT__g_p_h_d = hrrwork + 10530;
    double * const HRR_INT__g_p_i_s = hrrwork + 16200;
    double * const HRR_INT__g_p_i_p = hrrwork + 17460;
    double * const HRR_INT__g_p_k_s = hrrwork + 21240;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_g_f = INT__g_p_g_f + real_abcd * 6750;
                  ...
                  double * const restrict INT__p_g_g_f)
{
    ...
    int ret = ostei_g_p_g_f(P_tmp, Q, screen_tol, work, INT__p_g_g_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__p_g_g_f[q*6750+b*450+a*150+c*10+d];

        memcpy(INT__p_g_g_f+q*6750, buffer, 6750*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_f_g)
{
    ...
    int ret = ostei_g_p_g_f(P, Q_tmp, screen_tol, work, INT__g_p_f_g);
    double buffer[6750] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_p_f_g[q*6750+a*450+b*150+d*10+c];

        memcpy(INT__g_p_f_g+q*6750, buffer, 6750*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_f_g)
{
    ...
    int ret = ostei_g_p_g_f(P_tmp, Q_tmp, screen_tol, work, INT__p_g_f_g);
    double buffer[6750] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_g_f_g[q*6750+b*450+a*150+d*10+c];

        memcpy(INT__p_g_f_g+q*6750, buffer, 6750*sizeof(double));
    }
