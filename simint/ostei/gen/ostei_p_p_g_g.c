                  ...
                  double * const restrict INT__p_p_g_g)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_g_g);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__p_s_k_s = work + (SIMINT_NSHELL_SIMD * 192);
    double * const INT__p_s_l_s = work + (SIMINT_NSHELL_SIMD * 300);
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 435);
    double * const INT__d_s_h_s = work + (SIMINT_NSHELL_SIMD * 525);
    double * const INT__d_s_i_s = work + (SIMINT_NSHELL_SIMD * 651);
    double * const INT__d_s_k_s = work + (SIMINT_NSHELL_SIMD * 819);
    double * const INT__d_s_l_s = work + (SIMINT_NSHELL_SIMD * 1035);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*1305);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_k_s = primwork + 546;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_l_s = primwork + 690;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 825;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 885;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 975;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 1101;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_k_s = primwork + 1269;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_l_s = primwork + 1485;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 1755;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_h_s = primwork + 1845;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_i_s = primwork + 1971;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_k_s = primwork + 2139;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_l_s = primwork + 2355;
    double * const hrrwork = (double *)(primwork + 2625);
    double * const HRR_INT__p_p_g_s = hrrwork + 0;
    double * const HRR_INT__p_p_g_p = hrrwork + 135;
    double * const HRR_INT__p_p_g_d = hrrwork + 540;
    double * const HRR_INT__p_p_g_f = hrrwork + 1350;
    double * const HRR_INT__p_p_h_s = hrrwork + 2700;
    double * const HRR_INT__p_p_h_p = hrrwork + 2889;
    double * const HRR_INT__p_p_h_d = hrrwork + 3456;
    double * const HRR_INT__p_p_h_f = hrrwork + 4590;
    double * const HRR_INT__p_p_i_s = hrrwork + 6480;
    double * const HRR_INT__p_p_i_p = hrrwork + 6732;
    double * const HRR_INT__p_p_i_d = hrrwork + 7488;
    double * const HRR_INT__p_p_k_s = hrrwork + 9000;
    double * const HRR_INT__p_p_k_p = hrrwork + 9324;
    double * const HRR_INT__p_p_l_s = hrrwork + 10296;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 1305 * sizeof(double));
            abcd = 0;
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__p_s_k_s = INT__p_s_k_s + abcd * 108;
                double * restrict PRIM_PTR_INT__p_s_l_s = INT__p_s_l_s + abcd * 135;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__p_s_k_s += 108;
                            PRIM_PTR_INT__p_s_l_s += 135;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            ...
                            PRIM_PTR_INT__p_s_k_s += lastoffset*108;
                            PRIM_PTR_INT__p_s_l_s += lastoffset*135;
                    ...
                    if(lastoffset == 0)
                    {
                        ...
                        contract_all(108, PRIM_INT__p_s_k_s, PRIM_PTR_INT__p_s_k_s);
                        contract_all(135, PRIM_INT__p_s_l_s, PRIM_PTR_INT__p_s_l_s);
                    ...
                    else
                    {
                        ...
                        contract(108, shelloffsets, PRIM_INT__p_s_k_s, PRIM_PTR_INT__p_s_k_s);
                        contract(135, shelloffsets, PRIM_INT__p_s_l_s, PRIM_PTR_INT__p_s_l_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double const * restrict HRR_INT__p_s_k_s = INT__p_s_k_s + abcd * 108;
                double const * restrict HRR_INT__p_s_l_s = INT__p_s_l_s + abcd * 135;
                ...
                double * restrict HRR_INT__p_p_g_g = INT__p_p_g_g + real_abcd * 2025;
                ...
                {
                    HRR_INT__p_p_k_s[0 * 36 + iket] = HRR_INT__d_s_k_s[0 * 36 + iket] + ( hAB[0] * HRR_INT__p_s_k_s[0 * 36 + iket] );

                    HRR_INT__p_p_k_s[1 * 36 + iket] = HRR_INT__d_s_k_s[1 * 36 + iket] + ( hAB[1] * HRR_INT__p_s_k_s[0 * 36 + iket] );

                    HRR_INT__p_p_k_s[2 * 36 + iket] = HRR_INT__d_s_k_s[2 * 36 + iket] + ( hAB[2] * HRR_INT__p_s_k_s[0 * 36 + iket] );

                    HRR_INT__p_p_k_s[3 * 36 + iket] = HRR_INT__d_s_k_s[1 * 36 + iket] + ( hAB[0] * HRR_INT__p_s_k_s[1 * 36 + iket] );

                    HRR_INT__p_p_k_s[4 * 36 + iket] = HRR_INT__d_s_k_s[3 * 36 + iket] + ( hAB[1] * HRR_INT__p_s_k_s[1 * 36 + iket] );

                    HRR_INT__p_p_k_s[5 * 36 + iket] = HRR_INT__d_s_k_s[4 * 36 + iket] + ( hAB[2] * HRR_INT__p_s_k_s[1 * 36 + iket] );

                    HRR_INT__p_p_k_s[6 * 36 + iket] = HRR_INT__d_s_k_s[2 * 36 + iket] + ( hAB[0] * HRR_INT__p_s_k_s[2 * 36 + iket] );

                    HRR_INT__p_p_k_s[7 * 36 + iket] = HRR_INT__d_s_k_s[4 * 36 + iket] + ( hAB[1] * HRR_INT__p_s_k_s[2 * 36 + iket] );

                    HRR_INT__p_p_k_s[8 * 36 + iket] = HRR_INT__d_s_k_s[5 * 36 + iket] + ( hAB[2] * HRR_INT__p_s_k_s[2 * 36 + iket] );
                ...
                {
                    HRR_INT__p_p_l_s[0 * 45 + iket] = HRR_INT__d_s_l_s[0 * 45 + iket] + ( hAB[0] * HRR_INT__p_s_l_s[0 * 45 + iket] );

                    HRR_INT__p_p_l_s[1 * 45 + iket] = HRR_INT__d_s_l_s[1 * 45 + iket] + ( hAB[1] * HRR_INT__p_s_l_s[0 * 45 + iket] );

                    HRR_INT__p_p_l_s[2 * 45 + iket] = HRR_INT__d_s_l_s[2 * 45 + iket] + ( hAB[2] * HRR_INT__p_s_l_s[0 * 45 + iket] );

                    HRR_INT__p_p_l_s[3 * 45 + iket] = HRR_INT__d_s_l_s[1 * 45 + iket] + ( hAB[0] * HRR_INT__p_s_l_s[1 * 45 + iket] );

                    HRR_INT__p_p_l_s[4 * 45 + iket] = HRR_INT__d_s_l_s[3 * 45 + iket] + ( hAB[1] * HRR_INT__p_s_l_s[1 * 45 + iket] );

                    HRR_INT__p_p_l_s[5 * 45 + iket] = HRR_INT__d_s_l_s[4 * 45 + iket] + ( hAB[2] * HRR_INT__p_s_l_s[1 * 45 + iket] );

                    HRR_INT__p_p_l_s[6 * 45 + iket] = HRR_INT__d_s_l_s[2 * 45 + iket] + ( hAB[0] * HRR_INT__p_s_l_s[2 * 45 + iket] );

                    HRR_INT__p_p_l_s[7 * 45 + iket] = HRR_INT__d_s_l_s[4 * 45 + iket] + ( hAB[1] * HRR_INT__p_s_l_s[2 * 45 + iket] );

                    HRR_INT__p_p_l_s[8 * 45 + iket] = HRR_INT__d_s_l_s[5 * 45 + iket] + ( hAB[2] * HRR_INT__p_s_l_s[2 * 45 + iket] );
