                  ...
                  double * const restrict INT__p_s_g_g)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_s_g_g);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_l_s = primwork + 570;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 660;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 705;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 768;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_k_s = primwork + 852;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_l_s = primwork + 960;
    double * const hrrwork = (double *)(primwork + 1095);
    double * const HRR_INT__p_s_g_p = hrrwork + 0;
    double * const HRR_INT__p_s_g_d = hrrwork + 135;
    double * const HRR_INT__p_s_g_f = hrrwork + 405;
    double * const HRR_INT__p_s_h_p = hrrwork + 855;
    double * const HRR_INT__p_s_h_d = hrrwork + 1044;
    double * const HRR_INT__p_s_h_f = hrrwork + 1422;
    double * const HRR_INT__p_s_i_p = hrrwork + 2052;
    double * const HRR_INT__p_s_i_d = hrrwork + 2304;
    double * const HRR_INT__p_s_k_p = hrrwork + 2808;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_s_g_g = INT__p_s_g_g + real_abcd * 675;
                  ...
                  double * const restrict INT__s_p_g_g)
{
    ...
    int ret = ostei_p_s_g_g(P_tmp, Q, screen_tol, work, INT__s_p_g_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_p_g_g[q*675+b*225+a*225+c*15+d];

        memcpy(INT__s_p_g_g+q*675, buffer, 675*sizeof(double));
    }
