                  ...
                  double * const restrict INT__s_s_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__s_s_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__s_s_h_p = hrrwork + 45;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__s_s_g_d = INT__s_s_g_d + real_abcd * 90;
                  ...
                  double * const restrict INT__s_s_d_g)
{
    ...
    int ret = ostei_s_s_g_d(P, Q_tmp, screen_tol, work, INT__s_s_d_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_s_d_g[q*90+a*90+b*90+d*6+c];

        memcpy(INT__s_s_d_g+q*90, buffer, 90*sizeof(double));
    }
