                  ...
                  double * const restrict INT__d_p_g_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_p_g_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 744;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 834;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 969;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 1158;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_k_s = primwork + 1410;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_f_s = primwork + 1734;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 1854;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_h_s = primwork + 2034;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_i_s = primwork + 2286;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_k_s = primwork + 2622;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_g_s = primwork + 3054;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_h_s = primwork + 3204;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_i_s = primwork + 3414;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_k_s = primwork + 3694;
    ...
    double * const HRR_INT__d_p_g_d = hrrwork + 1080;
    double * const HRR_INT__d_p_h_s = hrrwork + 2700;
    double * const HRR_INT__d_p_h_p = hrrwork + 3078;
    double * const HRR_INT__d_p_h_d = hrrwork + 4212;
    double * const HRR_INT__d_p_i_s = hrrwork + 6480;
    double * const HRR_INT__d_p_i_p = hrrwork + 6984;
    double * const HRR_INT__d_p_k_s = hrrwork + 8496;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_p_g_f = INT__d_p_g_f + real_abcd * 2700;
                  ...
                  double * const restrict INT__p_d_g_f)
{
    ...
    int ret = ostei_d_p_g_f(P_tmp, Q, screen_tol, work, INT__p_d_g_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__p_d_g_f[q*2700+b*450+a*150+c*10+d];

        memcpy(INT__p_d_g_f+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_p_f_g)
{
    ...
    int ret = ostei_d_p_g_f(P, Q_tmp, screen_tol, work, INT__d_p_f_g);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_p_f_g[q*2700+a*450+b*150+d*10+c];

        memcpy(INT__d_p_f_g+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_d_f_g)
{
    ...
    int ret = ostei_d_p_g_f(P_tmp, Q_tmp, screen_tol, work, INT__p_d_f_g);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_d_f_g[q*2700+b*450+a*150+d*10+c];

        memcpy(INT__p_d_f_g+q*2700, buffer, 2700*sizeof(double));
    }
