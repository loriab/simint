                  ...
                  double * const restrict INT__d_d_p_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_d_p_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__f_s_p_s = work + (SIMINT_NSHELL_SIMD * 18);
    double * const INT__g_s_p_s = work + (SIMINT_NSHELL_SIMD * 48);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_p_s = primwork + 93;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 123;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 153;
    double * const hrrwork = (double *)(primwork + 198);
    double * const HRR_INT__d_p_p_s = hrrwork + 0;
    double * const HRR_INT__f_p_p_s = hrrwork + 54;
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    // Forming PRIM_INT__d_s_p_s[1 * 18];
                    ...
                    // Forming PRIM_INT__f_s_p_s[1 * 30];
                    for(n = 0; n < 1; ++n)  // loop over orders of auxiliary function
                    {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_d_p_s = INT__d_d_p_s + real_abcd * 108;
