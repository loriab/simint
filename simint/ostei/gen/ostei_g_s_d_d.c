                  ...
                  double * const restrict INT__g_s_d_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_s_d_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*465);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_s = primwork + 69;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_p_s = primwork + 111;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_d_s = primwork + 183;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_s_s = primwork + 291;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_p_s = primwork + 351;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_d_s = primwork + 471;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_f_s = primwork + 651;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 851;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 926;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_d_s = primwork + 1106;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_f_s = primwork + 1376;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_g_s = primwork + 1676;
    double * const hrrwork = (double *)(primwork + 1901);
    double * const HRR_INT__g_s_d_p = hrrwork + 0;
    double * const HRR_INT__g_s_f_p = hrrwork + 270;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 465 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_s_d_d = INT__g_s_d_d + real_abcd * 540;

                // form INT__g_s_d_p
                for(ibra = 0; ibra < 15; ++ibra)
                {
                    HRR_INT__g_s_d_p[ibra * 18 + 0] = HRR_INT__g_s_f_s[ibra * 10 + 0] + ( hCD[0] * HRR_INT__g_s_d_s[ibra * 6 + 0] );

                    HRR_INT__g_s_d_p[ibra * 18 + 1] = HRR_INT__g_s_f_s[ibra * 10 + 1] + ( hCD[1] * HRR_INT__g_s_d_s[ibra * 6 + 0] );

                    HRR_INT__g_s_d_p[ibra * 18 + 2] = HRR_INT__g_s_f_s[ibra * 10 + 2] + ( hCD[2] * HRR_INT__g_s_d_s[ibra * 6 + 0] );

                    HRR_INT__g_s_d_p[ibra * 18 + 3] = HRR_INT__g_s_f_s[ibra * 10 + 1] + ( hCD[0] * HRR_INT__g_s_d_s[ibra * 6 + 1] );

                    HRR_INT__g_s_d_p[ibra * 18 + 4] = HRR_INT__g_s_f_s[ibra * 10 + 3] + ( hCD[1] * HRR_INT__g_s_d_s[ibra * 6 + 1] );

                    HRR_INT__g_s_d_p[ibra * 18 + 5] = HRR_INT__g_s_f_s[ibra * 10 + 4] + ( hCD[2] * HRR_INT__g_s_d_s[ibra * 6 + 1] );

                    HRR_INT__g_s_d_p[ibra * 18 + 6] = HRR_INT__g_s_f_s[ibra * 10 + 2] + ( hCD[0] * HRR_INT__g_s_d_s[ibra * 6 + 2] );

                    HRR_INT__g_s_d_p[ibra * 18 + 7] = HRR_INT__g_s_f_s[ibra * 10 + 4] + ( hCD[1] * HRR_INT__g_s_d_s[ibra * 6 + 2] );

                    HRR_INT__g_s_d_p[ibra * 18 + 8] = HRR_INT__g_s_f_s[ibra * 10 + 5] + ( hCD[2] * HRR_INT__g_s_d_s[ibra * 6 + 2] );

                    HRR_INT__g_s_d_p[ibra * 18 + 9] = HRR_INT__g_s_f_s[ibra * 10 + 3] + ( hCD[0] * HRR_INT__g_s_d_s[ibra * 6 + 3] );

                    HRR_INT__g_s_d_p[ibra * 18 + 10] = HRR_INT__g_s_f_s[ibra * 10 + 6] + ( hCD[1] * HRR_INT__g_s_d_s[ibra * 6 + 3] );

                    HRR_INT__g_s_d_p[ibra * 18 + 11] = HRR_INT__g_s_f_s[ibra * 10 + 7] + ( hCD[2] * HRR_INT__g_s_d_s[ibra * 6 + 3] );

                    HRR_INT__g_s_d_p[ibra * 18 + 12] = HRR_INT__g_s_f_s[ibra * 10 + 4] + ( hCD[0] * HRR_INT__g_s_d_s[ibra * 6 + 4] );

                    HRR_INT__g_s_d_p[ibra * 18 + 13] = HRR_INT__g_s_f_s[ibra * 10 + 7] + ( hCD[1] * HRR_INT__g_s_d_s[ibra * 6 + 4] );

                    HRR_INT__g_s_d_p[ibra * 18 + 14] = HRR_INT__g_s_f_s[ibra * 10 + 8] + ( hCD[2] * HRR_INT__g_s_d_s[ibra * 6 + 4] );

                    HRR_INT__g_s_d_p[ibra * 18 + 15] = HRR_INT__g_s_f_s[ibra * 10 + 5] + ( hCD[0] * HRR_INT__g_s_d_s[ibra * 6 + 5] );

                    HRR_INT__g_s_d_p[ibra * 18 + 16] = HRR_INT__g_s_f_s[ibra * 10 + 8] + ( hCD[1] * HRR_INT__g_s_d_s[ibra * 6 + 5] );

                    HRR_INT__g_s_d_p[ibra * 18 + 17] = HRR_INT__g_s_f_s[ibra * 10 + 9] + ( hCD[2] * HRR_INT__g_s_d_s[ibra * 6 + 5] );
                ...
                for(ibra = 0; ibra < 15; ++ibra)
                {
                    HRR_INT__g_s_d_d[ibra * 36 + 0] = HRR_INT__g_s_f_p[ibra * 30 + 0] + ( hCD[0] * HRR_INT__g_s_d_p[ibra * 18 + 0] );

                    HRR_INT__g_s_d_d[ibra * 36 + 1] = HRR_INT__g_s_f_p[ibra * 30 + 3] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 0] );

                    HRR_INT__g_s_d_d[ibra * 36 + 2] = HRR_INT__g_s_f_p[ibra * 30 + 6] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 0] );

                    HRR_INT__g_s_d_d[ibra * 36 + 3] = HRR_INT__g_s_f_p[ibra * 30 + 4] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 1] );

                    HRR_INT__g_s_d_d[ibra * 36 + 4] = HRR_INT__g_s_f_p[ibra * 30 + 7] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 1] );

                    HRR_INT__g_s_d_d[ibra * 36 + 5] = HRR_INT__g_s_f_p[ibra * 30 + 8] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 2] );

                    HRR_INT__g_s_d_d[ibra * 36 + 6] = HRR_INT__g_s_f_p[ibra * 30 + 3] + ( hCD[0] * HRR_INT__g_s_d_p[ibra * 18 + 3] );

                    HRR_INT__g_s_d_d[ibra * 36 + 7] = HRR_INT__g_s_f_p[ibra * 30 + 9] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 3] );

                    HRR_INT__g_s_d_d[ibra * 36 + 8] = HRR_INT__g_s_f_p[ibra * 30 + 12] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 3] );

                    HRR_INT__g_s_d_d[ibra * 36 + 9] = HRR_INT__g_s_f_p[ibra * 30 + 10] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 4] );

                    HRR_INT__g_s_d_d[ibra * 36 + 10] = HRR_INT__g_s_f_p[ibra * 30 + 13] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 4] );

                    HRR_INT__g_s_d_d[ibra * 36 + 11] = HRR_INT__g_s_f_p[ibra * 30 + 14] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 5] );

                    HRR_INT__g_s_d_d[ibra * 36 + 12] = HRR_INT__g_s_f_p[ibra * 30 + 6] + ( hCD[0] * HRR_INT__g_s_d_p[ibra * 18 + 6] );

                    HRR_INT__g_s_d_d[ibra * 36 + 13] = HRR_INT__g_s_f_p[ibra * 30 + 12] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 6] );

                    HRR_INT__g_s_d_d[ibra * 36 + 14] = HRR_INT__g_s_f_p[ibra * 30 + 15] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 6] );

                    HRR_INT__g_s_d_d[ibra * 36 + 15] = HRR_INT__g_s_f_p[ibra * 30 + 13] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 7] );

                    HRR_INT__g_s_d_d[ibra * 36 + 16] = HRR_INT__g_s_f_p[ibra * 30 + 16] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 7] );

                    HRR_INT__g_s_d_d[ibra * 36 + 17] = HRR_INT__g_s_f_p[ibra * 30 + 17] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 8] );

                    HRR_INT__g_s_d_d[ibra * 36 + 18] = HRR_INT__g_s_f_p[ibra * 30 + 9] + ( hCD[0] * HRR_INT__g_s_d_p[ibra * 18 + 9] );

                    HRR_INT__g_s_d_d[ibra * 36 + 19] = HRR_INT__g_s_f_p[ibra * 30 + 18] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 9] );

                    HRR_INT__g_s_d_d[ibra * 36 + 20] = HRR_INT__g_s_f_p[ibra * 30 + 21] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 9] );

                    HRR_INT__g_s_d_d[ibra * 36 + 21] = HRR_INT__g_s_f_p[ibra * 30 + 19] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 10] );

                    HRR_INT__g_s_d_d[ibra * 36 + 22] = HRR_INT__g_s_f_p[ibra * 30 + 22] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 10] );

                    HRR_INT__g_s_d_d[ibra * 36 + 23] = HRR_INT__g_s_f_p[ibra * 30 + 23] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 11] );

                    HRR_INT__g_s_d_d[ibra * 36 + 24] = HRR_INT__g_s_f_p[ibra * 30 + 12] + ( hCD[0] * HRR_INT__g_s_d_p[ibra * 18 + 12] );

                    HRR_INT__g_s_d_d[ibra * 36 + 25] = HRR_INT__g_s_f_p[ibra * 30 + 21] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 12] );

                    HRR_INT__g_s_d_d[ibra * 36 + 26] = HRR_INT__g_s_f_p[ibra * 30 + 24] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 12] );

                    HRR_INT__g_s_d_d[ibra * 36 + 27] = HRR_INT__g_s_f_p[ibra * 30 + 22] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 13] );

                    HRR_INT__g_s_d_d[ibra * 36 + 28] = HRR_INT__g_s_f_p[ibra * 30 + 25] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 13] );

                    HRR_INT__g_s_d_d[ibra * 36 + 29] = HRR_INT__g_s_f_p[ibra * 30 + 26] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 14] );

                    HRR_INT__g_s_d_d[ibra * 36 + 30] = HRR_INT__g_s_f_p[ibra * 30 + 15] + ( hCD[0] * HRR_INT__g_s_d_p[ibra * 18 + 15] );

                    HRR_INT__g_s_d_d[ibra * 36 + 31] = HRR_INT__g_s_f_p[ibra * 30 + 24] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 15] );

                    HRR_INT__g_s_d_d[ibra * 36 + 32] = HRR_INT__g_s_f_p[ibra * 30 + 27] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 15] );

                    HRR_INT__g_s_d_d[ibra * 36 + 33] = HRR_INT__g_s_f_p[ibra * 30 + 25] + ( hCD[1] * HRR_INT__g_s_d_p[ibra * 18 + 16] );

                    HRR_INT__g_s_d_d[ibra * 36 + 34] = HRR_INT__g_s_f_p[ibra * 30 + 28] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 16] );

                    HRR_INT__g_s_d_d[ibra * 36 + 35] = HRR_INT__g_s_f_p[ibra * 30 + 29] + ( hCD[2] * HRR_INT__g_s_d_p[ibra * 18 + 17] );
                  ...
                  double * const restrict INT__s_g_d_d)
{
    ...
    int ret = ostei_g_s_d_d(P_tmp, Q, screen_tol, work, INT__s_g_d_d);
    double buffer[540] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__s_g_d_d[q*540+b*36+a*36+c*6+d];

        memcpy(INT__s_g_d_d+q*540, buffer, 540*sizeof(double));
    }
