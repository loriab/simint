                  ...
                  double * const restrict INT__g_f_d_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_d_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*600);
    ...
    double * const HRR_INT__h_p_d_s = hrrwork + 810;
    double * const HRR_INT__h_d_d_s = hrrwork + 1188;
    double * const HRR_INT__i_p_d_s = hrrwork + 1944;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 600 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_d_s = INT__g_f_d_s + real_abcd * 900;
                  ...
                  double * const restrict INT__f_g_d_s)
{
    ...
    int ret = ostei_g_f_d_s(P_tmp, Q, screen_tol, work, INT__f_g_d_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__f_g_d_s[q*900+b*60+a*6+c*1+d];

        memcpy(INT__f_g_d_s+q*900, buffer, 900*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_s_d)
{
    ...
    int ret = ostei_g_f_d_s(P, Q_tmp, screen_tol, work, INT__g_f_s_d);
    double buffer[900] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__g_f_s_d[q*900+a*60+b*6+d*1+c];

        memcpy(INT__g_f_s_d+q*900, buffer, 900*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_s_d)
{
    ...
    int ret = ostei_g_f_d_s(P_tmp, Q_tmp, screen_tol, work, INT__f_g_s_d);
    double buffer[900] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__f_g_s_d[q*900+b*60+a*6+d*1+c];

        memcpy(INT__f_g_s_d+q*900, buffer, 900*sizeof(double));
    }
