                  ...
                  double * const restrict INT__p_d_d_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_d_d_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_d_d_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_d_f_s = work + (SIMINT_NSHELL_SIMD * 36);
    double * const INT__s_d_g_s = work + (SIMINT_NSHELL_SIMD * 96);
    double * const INT__s_f_d_s = work + (SIMINT_NSHELL_SIMD * 186);
    double * const INT__s_f_f_s = work + (SIMINT_NSHELL_SIMD * 246);
    double * const INT__s_f_g_s = work + (SIMINT_NSHELL_SIMD * 346);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_s_s = primwork + 175;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_p_s = primwork + 184;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_d_s = primwork + 211;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_f_s = primwork + 265;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_g_s = primwork + 355;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_p_s = primwork + 490;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_d_s = primwork + 526;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_f_s = primwork + 598;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_g_s = primwork + 718;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_d_s = primwork + 898;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_f_s = primwork + 958;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_g_s = primwork + 1058;
    double * const hrrwork = (double *)(primwork + 1208);
    double * const HRR_INT__p_d_d_s = hrrwork + 0;
    double * const HRR_INT__p_d_d_p = hrrwork + 108;
    double * const HRR_INT__p_d_f_s = hrrwork + 432;
    double * const HRR_INT__p_d_f_p = hrrwork + 612;
    double * const HRR_INT__p_d_g_s = hrrwork + 1152;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_d_f_s = INT__s_d_f_s + abcd * 60;
                double * restrict PRIM_PTR_INT__s_d_g_s = INT__s_d_g_s + abcd * 90;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__s_d_f_s += 60;
                            PRIM_PTR_INT__s_d_g_s += 90;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            ...
                            PRIM_PTR_INT__s_d_f_s += lastoffset*60;
                            PRIM_PTR_INT__s_d_g_s += lastoffset*90;
                    ...
                    // Forming PRIM_INT__s_d_d_s[2 * 36];
                    for(n = 0; n < 2; ++n)  // loop over orders of auxiliary function
                    {
                        ...
                        PRIM_INT__s_d_d_s[n * 36 + 12] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 0]);
                        PRIM_INT__s_d_d_s[n * 36 + 12] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 0], PRIM_INT__s_d_d_s[n * 36 + 12]);

                        PRIM_INT__s_d_d_s[n * 36 + 13] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 1]);
                        PRIM_INT__s_d_d_s[n * 36 + 13] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 1], PRIM_INT__s_d_d_s[n * 36 + 13]);

                        PRIM_INT__s_d_d_s[n * 36 + 14] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 2]);
                        PRIM_INT__s_d_d_s[n * 36 + 14] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 2], PRIM_INT__s_d_d_s[n * 36 + 14]);
                        PRIM_INT__s_d_d_s[n * 36 + 14] = SIMINT_FMADD( vrr_const_1_over_2pq, PRIM_INT__s_p_p_s[(n+1) * 9 + 0], PRIM_INT__s_d_d_s[n * 36 + 14]);

                        PRIM_INT__s_d_d_s[n * 36 + 15] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 3]);
                        PRIM_INT__s_d_d_s[n * 36 + 15] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 3], PRIM_INT__s_d_d_s[n * 36 + 15]);

                        PRIM_INT__s_d_d_s[n * 36 + 16] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 4]);
                        PRIM_INT__s_d_d_s[n * 36 + 16] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 4], PRIM_INT__s_d_d_s[n * 36 + 16]);
                        PRIM_INT__s_d_d_s[n * 36 + 16] = SIMINT_FMADD( vrr_const_1_over_2pq, PRIM_INT__s_p_p_s[(n+1) * 9 + 1], PRIM_INT__s_d_d_s[n * 36 + 16]);

                        PRIM_INT__s_d_d_s[n * 36 + 17] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 5]);
                        PRIM_INT__s_d_d_s[n * 36 + 17] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 5], PRIM_INT__s_d_d_s[n * 36 + 17]);
                        PRIM_INT__s_d_d_s[n * 36 + 17] = SIMINT_FMADD( vrr_const_2_over_2pq, PRIM_INT__s_p_p_s[(n+1) * 9 + 2], PRIM_INT__s_d_d_s[n * 36 + 17]);
                        ...
                        PRIM_INT__s_d_d_s[n * 36 + 24] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 6]);
                        PRIM_INT__s_d_d_s[n * 36 + 24] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 6], PRIM_INT__s_d_d_s[n * 36 + 24]);

                        PRIM_INT__s_d_d_s[n * 36 + 25] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 7]);
                        PRIM_INT__s_d_d_s[n * 36 + 25] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 7], PRIM_INT__s_d_d_s[n * 36 + 25]);

                        PRIM_INT__s_d_d_s[n * 36 + 26] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 8]);
                        PRIM_INT__s_d_d_s[n * 36 + 26] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 8], PRIM_INT__s_d_d_s[n * 36 + 26]);
                        PRIM_INT__s_d_d_s[n * 36 + 26] = SIMINT_FMADD( vrr_const_1_over_2pq, PRIM_INT__s_p_p_s[(n+1) * 9 + 3], PRIM_INT__s_d_d_s[n * 36 + 26]);

                        PRIM_INT__s_d_d_s[n * 36 + 27] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 9]);
                        PRIM_INT__s_d_d_s[n * 36 + 27] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 9], PRIM_INT__s_d_d_s[n * 36 + 27]);

                        PRIM_INT__s_d_d_s[n * 36 + 28] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 10]);
                        PRIM_INT__s_d_d_s[n * 36 + 28] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 10], PRIM_INT__s_d_d_s[n * 36 + 28]);
                        PRIM_INT__s_d_d_s[n * 36 + 28] = SIMINT_FMADD( vrr_const_1_over_2pq, PRIM_INT__s_p_p_s[(n+1) * 9 + 4], PRIM_INT__s_d_d_s[n * 36 + 28]);

                        PRIM_INT__s_d_d_s[n * 36 + 29] = SIMINT_MUL(P_PB[2], PRIM_INT__s_p_d_s[n * 18 + 11]);
                        PRIM_INT__s_d_d_s[n * 36 + 29] = SIMINT_FMADD( aop_PQ[2], PRIM_INT__s_p_d_s[(n+1) * 18 + 11], PRIM_INT__s_d_d_s[n * 36 + 29]);
                        PRIM_INT__s_d_d_s[n * 36 + 29] = SIMINT_FMADD( vrr_const_2_over_2pq, PRIM_INT__s_p_p_s[(n+1) * 9 + 5], PRIM_INT__s_d_d_s[n * 36 + 29]);
                    ...
                    // Forming PRIM_INT__s_p_f_s[3 * 30];
                    for(n = 0; n < 3; ++n)  // loop over orders of auxiliary function
                    {
                    ...
                    for(n = 0; n < 2; ++n)  // loop over orders of auxiliary function
                    {
                        ...
                        PRIM_INT__s_d_p_s[n * 18 + 3] = SIMINT_MUL(P_PB[1], PRIM_INT__s_p_p_s[n * 9 + 0]);
                        PRIM_INT__s_d_p_s[n * 18 + 3] = SIMINT_FMADD( aop_PQ[1], PRIM_INT__s_p_p_s[(n+1) * 9 + 0], PRIM_INT__s_d_p_s[n * 18 + 3]);

                        PRIM_INT__s_d_p_s[n * 18 + 4] = SIMINT_MUL(P_PB[1], PRIM_INT__s_p_p_s[n * 9 + 1]);
                        PRIM_INT__s_d_p_s[n * 18 + 4] = SIMINT_FMADD( aop_PQ[1], PRIM_INT__s_p_p_s[(n+1) * 9 + 1], PRIM_INT__s_d_p_s[n * 18 + 4]);
                        PRIM_INT__s_d_p_s[n * 18 + 4] = SIMINT_FMADD( vrr_const_1_over_2pq, PRIM_INT__s_p_s_s[(n+1) * 3 + 0], PRIM_INT__s_d_p_s[n * 18 + 4]);

                        PRIM_INT__s_d_p_s[n * 18 + 5] = SIMINT_MUL(P_PB[1], PRIM_INT__s_p_p_s[n * 9 + 2]);
                        PRIM_INT__s_d_p_s[n * 18 + 5] = SIMINT_FMADD( aop_PQ[1], PRIM_INT__s_p_p_s[(n+1) * 9 + 2], PRIM_INT__s_d_p_s[n * 18 + 5]);
                    ...
                    if(lastoffset == 0)
                    {
                        ...
                        contract_all(60, PRIM_INT__s_d_f_s, PRIM_PTR_INT__s_d_f_s);
                        contract_all(90, PRIM_INT__s_d_g_s, PRIM_PTR_INT__s_d_g_s);
                    ...
                    else
                    {
                        ...
                        contract(60, shelloffsets, PRIM_INT__s_d_f_s, PRIM_PTR_INT__s_d_f_s);
                        contract(90, shelloffsets, PRIM_INT__s_d_g_s, PRIM_PTR_INT__s_d_g_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double const * restrict HRR_INT__s_d_f_s = INT__s_d_f_s + abcd * 60;
                double const * restrict HRR_INT__s_d_g_s = INT__s_d_g_s + abcd * 90;
                ...
                double * restrict HRR_INT__p_d_d_d = INT__p_d_d_d + real_abcd * 648;
                ...
                {
                    HRR_INT__p_d_f_s[0 * 10 + iket] = HRR_INT__s_f_f_s[0 * 10 + iket] - ( hAB[0] * HRR_INT__s_d_f_s[0 * 10 + iket] );

                    HRR_INT__p_d_f_s[1 * 10 + iket] = HRR_INT__s_f_f_s[1 * 10 + iket] - ( hAB[0] * HRR_INT__s_d_f_s[1 * 10 + iket] );

                    HRR_INT__p_d_f_s[2 * 10 + iket] = HRR_INT__s_f_f_s[2 * 10 + iket] - ( hAB[0] * HRR_INT__s_d_f_s[2 * 10 + iket] );

                    HRR_INT__p_d_f_s[3 * 10 + iket] = HRR_INT__s_f_f_s[3 * 10 + iket] - ( hAB[0] * HRR_INT__s_d_f_s[3 * 10 + iket] );

                    HRR_INT__p_d_f_s[4 * 10 + iket] = HRR_INT__s_f_f_s[4 * 10 + iket] - ( hAB[0] * HRR_INT__s_d_f_s[4 * 10 + iket] );

                    HRR_INT__p_d_f_s[5 * 10 + iket] = HRR_INT__s_f_f_s[5 * 10 + iket] - ( hAB[0] * HRR_INT__s_d_f_s[5 * 10 + iket] );

                    HRR_INT__p_d_f_s[6 * 10 + iket] = HRR_INT__s_f_f_s[1 * 10 + iket] - ( hAB[1] * HRR_INT__s_d_f_s[0 * 10 + iket] );

                    HRR_INT__p_d_f_s[7 * 10 + iket] = HRR_INT__s_f_f_s[3 * 10 + iket] - ( hAB[1] * HRR_INT__s_d_f_s[1 * 10 + iket] );

                    HRR_INT__p_d_f_s[8 * 10 + iket] = HRR_INT__s_f_f_s[4 * 10 + iket] - ( hAB[1] * HRR_INT__s_d_f_s[2 * 10 + iket] );

                    HRR_INT__p_d_f_s[9 * 10 + iket] = HRR_INT__s_f_f_s[6 * 10 + iket] - ( hAB[1] * HRR_INT__s_d_f_s[3 * 10 + iket] );

                    HRR_INT__p_d_f_s[10 * 10 + iket] = HRR_INT__s_f_f_s[7 * 10 + iket] - ( hAB[1] * HRR_INT__s_d_f_s[4 * 10 + iket] );

                    HRR_INT__p_d_f_s[11 * 10 + iket] = HRR_INT__s_f_f_s[8 * 10 + iket] - ( hAB[1] * HRR_INT__s_d_f_s[5 * 10 + iket] );

                    HRR_INT__p_d_f_s[12 * 10 + iket] = HRR_INT__s_f_f_s[2 * 10 + iket] - ( hAB[2] * HRR_INT__s_d_f_s[0 * 10 + iket] );

                    HRR_INT__p_d_f_s[13 * 10 + iket] = HRR_INT__s_f_f_s[4 * 10 + iket] - ( hAB[2] * HRR_INT__s_d_f_s[1 * 10 + iket] );

                    HRR_INT__p_d_f_s[14 * 10 + iket] = HRR_INT__s_f_f_s[5 * 10 + iket] - ( hAB[2] * HRR_INT__s_d_f_s[2 * 10 + iket] );

                    HRR_INT__p_d_f_s[15 * 10 + iket] = HRR_INT__s_f_f_s[7 * 10 + iket] - ( hAB[2] * HRR_INT__s_d_f_s[3 * 10 + iket] );

                    HRR_INT__p_d_f_s[16 * 10 + iket] = HRR_INT__s_f_f_s[8 * 10 + iket] - ( hAB[2] * HRR_INT__s_d_f_s[4 * 10 + iket] );

                    HRR_INT__p_d_f_s[17 * 10 + iket] = HRR_INT__s_f_f_s[9 * 10 + iket] - ( hAB[2] * HRR_INT__s_d_f_s[5 * 10 + iket] );
                ...
                {
                    HRR_INT__p_d_g_s[0 * 15 + iket] = HRR_INT__s_f_g_s[0 * 15 + iket] - ( hAB[0] * HRR_INT__s_d_g_s[0 * 15 + iket] );

                    HRR_INT__p_d_g_s[1 * 15 + iket] = HRR_INT__s_f_g_s[1 * 15 + iket] - ( hAB[0] * HRR_INT__s_d_g_s[1 * 15 + iket] );

                    HRR_INT__p_d_g_s[2 * 15 + iket] = HRR_INT__s_f_g_s[2 * 15 + iket] - ( hAB[0] * HRR_INT__s_d_g_s[2 * 15 + iket] );

                    HRR_INT__p_d_g_s[3 * 15 + iket] = HRR_INT__s_f_g_s[3 * 15 + iket] - ( hAB[0] * HRR_INT__s_d_g_s[3 * 15 + iket] );

                    HRR_INT__p_d_g_s[4 * 15 + iket] = HRR_INT__s_f_g_s[4 * 15 + iket] - ( hAB[0] * HRR_INT__s_d_g_s[4 * 15 + iket] );

                    HRR_INT__p_d_g_s[5 * 15 + iket] = HRR_INT__s_f_g_s[5 * 15 + iket] - ( hAB[0] * HRR_INT__s_d_g_s[5 * 15 + iket] );

                    HRR_INT__p_d_g_s[6 * 15 + iket] = HRR_INT__s_f_g_s[1 * 15 + iket] - ( hAB[1] * HRR_INT__s_d_g_s[0 * 15 + iket] );

                    HRR_INT__p_d_g_s[7 * 15 + iket] = HRR_INT__s_f_g_s[3 * 15 + iket] - ( hAB[1] * HRR_INT__s_d_g_s[1 * 15 + iket] );

                    HRR_INT__p_d_g_s[8 * 15 + iket] = HRR_INT__s_f_g_s[4 * 15 + iket] - ( hAB[1] * HRR_INT__s_d_g_s[2 * 15 + iket] );

                    HRR_INT__p_d_g_s[9 * 15 + iket] = HRR_INT__s_f_g_s[6 * 15 + iket] - ( hAB[1] * HRR_INT__s_d_g_s[3 * 15 + iket] );

                    HRR_INT__p_d_g_s[10 * 15 + iket] = HRR_INT__s_f_g_s[7 * 15 + iket] - ( hAB[1] * HRR_INT__s_d_g_s[4 * 15 + iket] );

                    HRR_INT__p_d_g_s[11 * 15 + iket] = HRR_INT__s_f_g_s[8 * 15 + iket] - ( hAB[1] * HRR_INT__s_d_g_s[5 * 15 + iket] );

                    HRR_INT__p_d_g_s[12 * 15 + iket] = HRR_INT__s_f_g_s[2 * 15 + iket] - ( hAB[2] * HRR_INT__s_d_g_s[0 * 15 + iket] );

                    HRR_INT__p_d_g_s[13 * 15 + iket] = HRR_INT__s_f_g_s[4 * 15 + iket] - ( hAB[2] * HRR_INT__s_d_g_s[1 * 15 + iket] );

                    HRR_INT__p_d_g_s[14 * 15 + iket] = HRR_INT__s_f_g_s[5 * 15 + iket] - ( hAB[2] * HRR_INT__s_d_g_s[2 * 15 + iket] );

                    HRR_INT__p_d_g_s[15 * 15 + iket] = HRR_INT__s_f_g_s[7 * 15 + iket] - ( hAB[2] * HRR_INT__s_d_g_s[3 * 15 + iket] );

                    HRR_INT__p_d_g_s[16 * 15 + iket] = HRR_INT__s_f_g_s[8 * 15 + iket] - ( hAB[2] * HRR_INT__s_d_g_s[4 * 15 + iket] );

                    HRR_INT__p_d_g_s[17 * 15 + iket] = HRR_INT__s_f_g_s[9 * 15 + iket] - ( hAB[2] * HRR_INT__s_d_g_s[5 * 15 + iket] );
                ...
                {
                    HRR_INT__p_d_d_p[ibra * 18 + 0] = HRR_INT__p_d_f_s[ibra * 10 + 0] + ( hCD[0] * HRR_INT__p_d_d_s[ibra * 6 + 0] );

                    HRR_INT__p_d_d_p[ibra * 18 + 1] = HRR_INT__p_d_f_s[ibra * 10 + 1] + ( hCD[1] * HRR_INT__p_d_d_s[ibra * 6 + 0] );

                    HRR_INT__p_d_d_p[ibra * 18 + 2] = HRR_INT__p_d_f_s[ibra * 10 + 2] + ( hCD[2] * HRR_INT__p_d_d_s[ibra * 6 + 0] );

                    HRR_INT__p_d_d_p[ibra * 18 + 3] = HRR_INT__p_d_f_s[ibra * 10 + 1] + ( hCD[0] * HRR_INT__p_d_d_s[ibra * 6 + 1] );

                    HRR_INT__p_d_d_p[ibra * 18 + 4] = HRR_INT__p_d_f_s[ibra * 10 + 3] + ( hCD[1] * HRR_INT__p_d_d_s[ibra * 6 + 1] );

                    HRR_INT__p_d_d_p[ibra * 18 + 5] = HRR_INT__p_d_f_s[ibra * 10 + 4] + ( hCD[2] * HRR_INT__p_d_d_s[ibra * 6 + 1] );

                    HRR_INT__p_d_d_p[ibra * 18 + 6] = HRR_INT__p_d_f_s[ibra * 10 + 2] + ( hCD[0] * HRR_INT__p_d_d_s[ibra * 6 + 2] );

                    HRR_INT__p_d_d_p[ibra * 18 + 7] = HRR_INT__p_d_f_s[ibra * 10 + 4] + ( hCD[1] * HRR_INT__p_d_d_s[ibra * 6 + 2] );

                    HRR_INT__p_d_d_p[ibra * 18 + 8] = HRR_INT__p_d_f_s[ibra * 10 + 5] + ( hCD[2] * HRR_INT__p_d_d_s[ibra * 6 + 2] );

                    HRR_INT__p_d_d_p[ibra * 18 + 9] = HRR_INT__p_d_f_s[ibra * 10 + 3] + ( hCD[0] * HRR_INT__p_d_d_s[ibra * 6 + 3] );

                    HRR_INT__p_d_d_p[ibra * 18 + 10] = HRR_INT__p_d_f_s[ibra * 10 + 6] + ( hCD[1] * HRR_INT__p_d_d_s[ibra * 6 + 3] );

                    HRR_INT__p_d_d_p[ibra * 18 + 11] = HRR_INT__p_d_f_s[ibra * 10 + 7] + ( hCD[2] * HRR_INT__p_d_d_s[ibra * 6 + 3] );

                    HRR_INT__p_d_d_p[ibra * 18 + 12] = HRR_INT__p_d_f_s[ibra * 10 + 4] + ( hCD[0] * HRR_INT__p_d_d_s[ibra * 6 + 4] );

                    HRR_INT__p_d_d_p[ibra * 18 + 13] = HRR_INT__p_d_f_s[ibra * 10 + 7] + ( hCD[1] * HRR_INT__p_d_d_s[ibra * 6 + 4] );

                    HRR_INT__p_d_d_p[ibra * 18 + 14] = HRR_INT__p_d_f_s[ibra * 10 + 8] + ( hCD[2] * HRR_INT__p_d_d_s[ibra * 6 + 4] );

                    HRR_INT__p_d_d_p[ibra * 18 + 15] = HRR_INT__p_d_f_s[ibra * 10 + 5] + ( hCD[0] * HRR_INT__p_d_d_s[ibra * 6 + 5] );

                    HRR_INT__p_d_d_p[ibra * 18 + 16] = HRR_INT__p_d_f_s[ibra * 10 + 8] + ( hCD[1] * HRR_INT__p_d_d_s[ibra * 6 + 5] );

                    HRR_INT__p_d_d_p[ibra * 18 + 17] = HRR_INT__p_d_f_s[ibra * 10 + 9] + ( hCD[2] * HRR_INT__p_d_d_s[ibra * 6 + 5] );
                ...
                for(ibra = 0; ibra < 18; ++ibra)
                {
                    HRR_INT__p_d_f_p[ibra * 30 + 0] = HRR_INT__p_d_g_s[ibra * 15 + 0] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 0] );

                    HRR_INT__p_d_f_p[ibra * 30 + 3] = HRR_INT__p_d_g_s[ibra * 15 + 1] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 1] );

                    HRR_INT__p_d_f_p[ibra * 30 + 4] = HRR_INT__p_d_g_s[ibra * 15 + 3] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 1] );

                    HRR_INT__p_d_f_p[ibra * 30 + 6] = HRR_INT__p_d_g_s[ibra * 15 + 2] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 2] );

                    HRR_INT__p_d_f_p[ibra * 30 + 7] = HRR_INT__p_d_g_s[ibra * 15 + 4] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 2] );

                    HRR_INT__p_d_f_p[ibra * 30 + 8] = HRR_INT__p_d_g_s[ibra * 15 + 5] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 2] );

                    HRR_INT__p_d_f_p[ibra * 30 + 9] = HRR_INT__p_d_g_s[ibra * 15 + 3] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 3] );

                    HRR_INT__p_d_f_p[ibra * 30 + 10] = HRR_INT__p_d_g_s[ibra * 15 + 6] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 3] );

                    HRR_INT__p_d_f_p[ibra * 30 + 12] = HRR_INT__p_d_g_s[ibra * 15 + 4] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 4] );

                    HRR_INT__p_d_f_p[ibra * 30 + 13] = HRR_INT__p_d_g_s[ibra * 15 + 7] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 4] );

                    HRR_INT__p_d_f_p[ibra * 30 + 14] = HRR_INT__p_d_g_s[ibra * 15 + 8] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 4] );

                    HRR_INT__p_d_f_p[ibra * 30 + 15] = HRR_INT__p_d_g_s[ibra * 15 + 5] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 5] );

                    HRR_INT__p_d_f_p[ibra * 30 + 16] = HRR_INT__p_d_g_s[ibra * 15 + 8] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 5] );

                    HRR_INT__p_d_f_p[ibra * 30 + 17] = HRR_INT__p_d_g_s[ibra * 15 + 9] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 5] );

                    HRR_INT__p_d_f_p[ibra * 30 + 18] = HRR_INT__p_d_g_s[ibra * 15 + 6] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 6] );

                    HRR_INT__p_d_f_p[ibra * 30 + 19] = HRR_INT__p_d_g_s[ibra * 15 + 10] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 6] );

                    HRR_INT__p_d_f_p[ibra * 30 + 21] = HRR_INT__p_d_g_s[ibra * 15 + 7] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 7] );

                    HRR_INT__p_d_f_p[ibra * 30 + 22] = HRR_INT__p_d_g_s[ibra * 15 + 11] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 7] );

                    HRR_INT__p_d_f_p[ibra * 30 + 23] = HRR_INT__p_d_g_s[ibra * 15 + 12] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 7] );

                    HRR_INT__p_d_f_p[ibra * 30 + 24] = HRR_INT__p_d_g_s[ibra * 15 + 8] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 8] );

                    HRR_INT__p_d_f_p[ibra * 30 + 25] = HRR_INT__p_d_g_s[ibra * 15 + 12] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 8] );

                    HRR_INT__p_d_f_p[ibra * 30 + 26] = HRR_INT__p_d_g_s[ibra * 15 + 13] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 8] );

                    HRR_INT__p_d_f_p[ibra * 30 + 27] = HRR_INT__p_d_g_s[ibra * 15 + 9] + ( hCD[0] * HRR_INT__p_d_f_s[ibra * 10 + 9] );

                    HRR_INT__p_d_f_p[ibra * 30 + 28] = HRR_INT__p_d_g_s[ibra * 15 + 13] + ( hCD[1] * HRR_INT__p_d_f_s[ibra * 10 + 9] );

                    HRR_INT__p_d_f_p[ibra * 30 + 29] = HRR_INT__p_d_g_s[ibra * 15 + 14] + ( hCD[2] * HRR_INT__p_d_f_s[ibra * 10 + 9] );
                ...
                for(ibra = 0; ibra < 18; ++ibra)
                {
                    HRR_INT__p_d_d_d[ibra * 36 + 0] = HRR_INT__p_d_f_p[ibra * 30 + 0] + ( hCD[0] * HRR_INT__p_d_d_p[ibra * 18 + 0] );

                    HRR_INT__p_d_d_d[ibra * 36 + 1] = HRR_INT__p_d_f_p[ibra * 30 + 3] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 0] );

                    HRR_INT__p_d_d_d[ibra * 36 + 2] = HRR_INT__p_d_f_p[ibra * 30 + 6] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 0] );

                    HRR_INT__p_d_d_d[ibra * 36 + 3] = HRR_INT__p_d_f_p[ibra * 30 + 4] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 1] );

                    HRR_INT__p_d_d_d[ibra * 36 + 4] = HRR_INT__p_d_f_p[ibra * 30 + 7] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 1] );

                    HRR_INT__p_d_d_d[ibra * 36 + 5] = HRR_INT__p_d_f_p[ibra * 30 + 8] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 2] );

                    HRR_INT__p_d_d_d[ibra * 36 + 6] = HRR_INT__p_d_f_p[ibra * 30 + 3] + ( hCD[0] * HRR_INT__p_d_d_p[ibra * 18 + 3] );

                    HRR_INT__p_d_d_d[ibra * 36 + 7] = HRR_INT__p_d_f_p[ibra * 30 + 9] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 3] );

                    HRR_INT__p_d_d_d[ibra * 36 + 8] = HRR_INT__p_d_f_p[ibra * 30 + 12] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 3] );

                    HRR_INT__p_d_d_d[ibra * 36 + 9] = HRR_INT__p_d_f_p[ibra * 30 + 10] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 4] );

                    HRR_INT__p_d_d_d[ibra * 36 + 10] = HRR_INT__p_d_f_p[ibra * 30 + 13] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 4] );

                    HRR_INT__p_d_d_d[ibra * 36 + 11] = HRR_INT__p_d_f_p[ibra * 30 + 14] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 5] );

                    HRR_INT__p_d_d_d[ibra * 36 + 12] = HRR_INT__p_d_f_p[ibra * 30 + 6] + ( hCD[0] * HRR_INT__p_d_d_p[ibra * 18 + 6] );

                    HRR_INT__p_d_d_d[ibra * 36 + 13] = HRR_INT__p_d_f_p[ibra * 30 + 12] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 6] );

                    HRR_INT__p_d_d_d[ibra * 36 + 14] = HRR_INT__p_d_f_p[ibra * 30 + 15] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 6] );

                    HRR_INT__p_d_d_d[ibra * 36 + 15] = HRR_INT__p_d_f_p[ibra * 30 + 13] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 7] );

                    HRR_INT__p_d_d_d[ibra * 36 + 16] = HRR_INT__p_d_f_p[ibra * 30 + 16] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 7] );

                    HRR_INT__p_d_d_d[ibra * 36 + 17] = HRR_INT__p_d_f_p[ibra * 30 + 17] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 8] );

                    HRR_INT__p_d_d_d[ibra * 36 + 18] = HRR_INT__p_d_f_p[ibra * 30 + 9] + ( hCD[0] * HRR_INT__p_d_d_p[ibra * 18 + 9] );

                    HRR_INT__p_d_d_d[ibra * 36 + 19] = HRR_INT__p_d_f_p[ibra * 30 + 18] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 9] );

                    HRR_INT__p_d_d_d[ibra * 36 + 20] = HRR_INT__p_d_f_p[ibra * 30 + 21] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 9] );

                    HRR_INT__p_d_d_d[ibra * 36 + 21] = HRR_INT__p_d_f_p[ibra * 30 + 19] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 10] );

                    HRR_INT__p_d_d_d[ibra * 36 + 22] = HRR_INT__p_d_f_p[ibra * 30 + 22] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 10] );

                    HRR_INT__p_d_d_d[ibra * 36 + 23] = HRR_INT__p_d_f_p[ibra * 30 + 23] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 11] );

                    HRR_INT__p_d_d_d[ibra * 36 + 24] = HRR_INT__p_d_f_p[ibra * 30 + 12] + ( hCD[0] * HRR_INT__p_d_d_p[ibra * 18 + 12] );

                    HRR_INT__p_d_d_d[ibra * 36 + 25] = HRR_INT__p_d_f_p[ibra * 30 + 21] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 12] );

                    HRR_INT__p_d_d_d[ibra * 36 + 26] = HRR_INT__p_d_f_p[ibra * 30 + 24] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 12] );

                    HRR_INT__p_d_d_d[ibra * 36 + 27] = HRR_INT__p_d_f_p[ibra * 30 + 22] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 13] );

                    HRR_INT__p_d_d_d[ibra * 36 + 28] = HRR_INT__p_d_f_p[ibra * 30 + 25] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 13] );

                    HRR_INT__p_d_d_d[ibra * 36 + 29] = HRR_INT__p_d_f_p[ibra * 30 + 26] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 14] );

                    HRR_INT__p_d_d_d[ibra * 36 + 30] = HRR_INT__p_d_f_p[ibra * 30 + 15] + ( hCD[0] * HRR_INT__p_d_d_p[ibra * 18 + 15] );

                    HRR_INT__p_d_d_d[ibra * 36 + 31] = HRR_INT__p_d_f_p[ibra * 30 + 24] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 15] );

                    HRR_INT__p_d_d_d[ibra * 36 + 32] = HRR_INT__p_d_f_p[ibra * 30 + 27] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 15] );

                    HRR_INT__p_d_d_d[ibra * 36 + 33] = HRR_INT__p_d_f_p[ibra * 30 + 25] + ( hCD[1] * HRR_INT__p_d_d_p[ibra * 18 + 16] );

                    HRR_INT__p_d_d_d[ibra * 36 + 34] = HRR_INT__p_d_f_p[ibra * 30 + 28] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 16] );

                    HRR_INT__p_d_d_d[ibra * 36 + 35] = HRR_INT__p_d_f_p[ibra * 30 + 29] + ( hCD[2] * HRR_INT__p_d_d_p[ibra * 18 + 17] );
