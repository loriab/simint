                  ...
                  double * const restrict INT__g_p_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*2304);
    ...
    double * const HRR_INT__g_p_h_s = hrrwork + 2700;
    double * const HRR_INT__g_p_h_p = hrrwork + 3645;
    double * const HRR_INT__g_p_i_s = hrrwork + 6480;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 2304 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_g_d = INT__g_p_g_d + real_abcd * 4050;
                  ...
                  double * const restrict INT__p_g_g_d)
{
    ...
    int ret = ostei_g_p_g_d(P_tmp, Q, screen_tol, work, INT__p_g_g_d);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__p_g_g_d[q*4050+b*270+a*90+c*6+d];

        memcpy(INT__p_g_g_d+q*4050, buffer, 4050*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_d_g)
{
    ...
    int ret = ostei_g_p_g_d(P, Q_tmp, screen_tol, work, INT__g_p_d_g);
    double buffer[4050] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_p_d_g[q*4050+a*270+b*90+d*6+c];

        memcpy(INT__g_p_d_g+q*4050, buffer, 4050*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_d_g)
{
    ...
    int ret = ostei_g_p_g_d(P_tmp, Q_tmp, screen_tol, work, INT__p_g_d_g);
    double buffer[4050] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_g_d_g[q*4050+b*270+a*90+d*6+c];

        memcpy(INT__p_g_d_g+q*4050, buffer, 4050*sizeof(double));
    }
