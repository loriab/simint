                  ...
                  double * const restrict INT__d_s_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_s_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 378;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 438;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 528;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 654;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 822;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_h_s = primwork + 912;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_i_s = primwork + 1038;
    double * const hrrwork = (double *)(primwork + 1206);
    double * const HRR_INT__d_s_g_p = hrrwork + 0;
    double * const HRR_INT__d_s_h_p = hrrwork + 270;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_s_g_d = INT__d_s_g_d + real_abcd * 540;
                  ...
                  double * const restrict INT__s_d_g_d)
{
    ...
    int ret = ostei_d_s_g_d(P_tmp, Q, screen_tol, work, INT__s_d_g_d);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__s_d_g_d[q*540+b*90+a*90+c*6+d];

        memcpy(INT__s_d_g_d+q*540, buffer, 540*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_s_d_g)
{
    ...
    int ret = ostei_d_s_g_d(P, Q_tmp, screen_tol, work, INT__d_s_d_g);
    double buffer[540] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_s_d_g[q*540+a*90+b*90+d*6+c];

        memcpy(INT__d_s_d_g+q*540, buffer, 540*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_d_d_g)
{
    ...
    int ret = ostei_d_s_g_d(P_tmp, Q_tmp, screen_tol, work, INT__s_d_d_g);
    double buffer[540] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_d_d_g[q*540+b*90+a*90+d*6+c];

        memcpy(INT__s_d_d_g+q*540, buffer, 540*sizeof(double));
    }
