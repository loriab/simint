                  ...
                  double * const restrict INT__g_d_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__h_p_g_s = hrrwork + 4860;
    double * const HRR_INT__h_p_h_s = hrrwork + 5805;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_g_p = INT__g_d_g_p + real_abcd * 4050;
                  ...
                  double * const restrict INT__d_g_g_p)
{
    ...
    int ret = ostei_g_d_g_p(P_tmp, Q, screen_tol, work, INT__d_g_g_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__d_g_g_p[q*4050+b*270+a*45+c*3+d];

        memcpy(INT__d_g_g_p+q*4050, buffer, 4050*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_d_p_g)
{
    ...
    int ret = ostei_g_d_g_p(P, Q_tmp, screen_tol, work, INT__g_d_p_g);
    double buffer[4050] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_d_p_g[q*4050+a*270+b*45+d*3+c];

        memcpy(INT__g_d_p_g+q*4050, buffer, 4050*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_g_p_g)
{
    ...
    int ret = ostei_g_d_g_p(P_tmp, Q_tmp, screen_tol, work, INT__d_g_p_g);
    double buffer[4050] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_g_p_g[q*4050+b*270+a*45+d*3+c];

        memcpy(INT__d_g_p_g+q*4050, buffer, 4050*sizeof(double));
    }
