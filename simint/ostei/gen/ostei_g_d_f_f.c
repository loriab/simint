                  ...
                  double * const restrict INT__g_d_f_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_f_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_d_f_p = hrrwork + 4230;
    double * const HRR_INT__g_d_f_d = hrrwork + 6930;
    double * const HRR_INT__g_d_g_s = hrrwork + 12330;
    double * const HRR_INT__g_d_g_p = hrrwork + 13680;
    double * const HRR_INT__g_d_g_d = hrrwork + 17730;
    double * const HRR_INT__g_d_h_s = hrrwork + 25830;
    double * const HRR_INT__g_d_h_p = hrrwork + 27720;
    double * const HRR_INT__g_d_i_s = hrrwork + 33390;
    double * const HRR_INT__h_p_f_s = hrrwork + 35910;
    double * const HRR_INT__h_p_g_s = hrrwork + 36540;
    double * const HRR_INT__h_p_h_s = hrrwork + 37485;
    double * const HRR_INT__h_p_i_s = hrrwork + 38808;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_f_f = INT__g_d_f_f + real_abcd * 9000;
                  ...
                  double * const restrict INT__d_g_f_f)
{
    ...
    int ret = ostei_g_d_f_f(P_tmp, Q, screen_tol, work, INT__d_g_f_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__d_g_f_f[q*9000+b*600+a*100+c*10+d];

        memcpy(INT__d_g_f_f+q*9000, buffer, 9000*sizeof(double));
    }
