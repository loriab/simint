                  ...
                  double * const restrict INT__p_p_d_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_d_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__p_s_s_f = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__p_s_s_g = work + (SIMINT_NSHELL_SIMD * 30);
    double * const INT__p_s_s_h = work + (SIMINT_NSHELL_SIMD * 75);
    double * const INT__d_s_s_f = work + (SIMINT_NSHELL_SIMD * 138);
    double * const INT__d_s_s_g = work + (SIMINT_NSHELL_SIMD * 198);
    double * const INT__d_s_s_h = work + (SIMINT_NSHELL_SIMD * 288);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*414);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_s_d = primwork + 238;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_s_f = primwork + 274;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_s_g = primwork + 334;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_s_h = primwork + 424;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_f = primwork + 550;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_g = primwork + 610;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_h = primwork + 700;
    double * const hrrwork = (double *)(primwork + 826);
    double * const HRR_INT__p_p_s_f = hrrwork + 0;
    double * const HRR_INT__p_p_s_g = hrrwork + 90;
    double * const HRR_INT__p_p_s_h = hrrwork + 225;
    double * const HRR_INT__p_p_p_f = hrrwork + 414;
    double * const HRR_INT__p_p_p_g = hrrwork + 684;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 414 * sizeof(double));
            abcd = 0;
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__p_s_s_g = INT__p_s_s_g + abcd * 45;
                double * restrict PRIM_PTR_INT__p_s_s_h = INT__p_s_s_h + abcd * 63;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__p_s_s_g += 45;
                            PRIM_PTR_INT__p_s_s_h += 63;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            ...
                            PRIM_PTR_INT__p_s_s_g += lastoffset*45;
                            PRIM_PTR_INT__p_s_s_h += lastoffset*63;
                    ...
                    if(lastoffset == 0)
                    {
                        ...
                        contract_all(45, PRIM_INT__p_s_s_g, PRIM_PTR_INT__p_s_s_g);
                        contract_all(63, PRIM_INT__p_s_s_h, PRIM_PTR_INT__p_s_s_h);
                    ...
                    else
                    {
                        ...
                        contract(45, shelloffsets, PRIM_INT__p_s_s_g, PRIM_PTR_INT__p_s_s_g);
                        contract(63, shelloffsets, PRIM_INT__p_s_s_h, PRIM_PTR_INT__p_s_s_h);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double const * restrict HRR_INT__p_s_s_g = INT__p_s_s_g + abcd * 45;
                double const * restrict HRR_INT__p_s_s_h = INT__p_s_s_h + abcd * 63;
                ...
                double * restrict HRR_INT__p_p_d_f = INT__p_p_d_f + real_abcd * 540;
                ...
                {
                    HRR_INT__p_p_s_g[0 * 15 + iket] = HRR_INT__d_s_s_g[0 * 15 + iket] + ( hAB[0] * HRR_INT__p_s_s_g[0 * 15 + iket] );

                    HRR_INT__p_p_s_g[1 * 15 + iket] = HRR_INT__d_s_s_g[1 * 15 + iket] + ( hAB[1] * HRR_INT__p_s_s_g[0 * 15 + iket] );

                    HRR_INT__p_p_s_g[2 * 15 + iket] = HRR_INT__d_s_s_g[2 * 15 + iket] + ( hAB[2] * HRR_INT__p_s_s_g[0 * 15 + iket] );

                    HRR_INT__p_p_s_g[3 * 15 + iket] = HRR_INT__d_s_s_g[1 * 15 + iket] + ( hAB[0] * HRR_INT__p_s_s_g[1 * 15 + iket] );

                    HRR_INT__p_p_s_g[4 * 15 + iket] = HRR_INT__d_s_s_g[3 * 15 + iket] + ( hAB[1] * HRR_INT__p_s_s_g[1 * 15 + iket] );

                    HRR_INT__p_p_s_g[5 * 15 + iket] = HRR_INT__d_s_s_g[4 * 15 + iket] + ( hAB[2] * HRR_INT__p_s_s_g[1 * 15 + iket] );

                    HRR_INT__p_p_s_g[6 * 15 + iket] = HRR_INT__d_s_s_g[2 * 15 + iket] + ( hAB[0] * HRR_INT__p_s_s_g[2 * 15 + iket] );

                    HRR_INT__p_p_s_g[7 * 15 + iket] = HRR_INT__d_s_s_g[4 * 15 + iket] + ( hAB[1] * HRR_INT__p_s_s_g[2 * 15 + iket] );

                    HRR_INT__p_p_s_g[8 * 15 + iket] = HRR_INT__d_s_s_g[5 * 15 + iket] + ( hAB[2] * HRR_INT__p_s_s_g[2 * 15 + iket] );
                ...
                {
                    HRR_INT__p_p_s_h[0 * 21 + iket] = HRR_INT__d_s_s_h[0 * 21 + iket] + ( hAB[0] * HRR_INT__p_s_s_h[0 * 21 + iket] );

                    HRR_INT__p_p_s_h[1 * 21 + iket] = HRR_INT__d_s_s_h[1 * 21 + iket] + ( hAB[1] * HRR_INT__p_s_s_h[0 * 21 + iket] );

                    HRR_INT__p_p_s_h[2 * 21 + iket] = HRR_INT__d_s_s_h[2 * 21 + iket] + ( hAB[2] * HRR_INT__p_s_s_h[0 * 21 + iket] );

                    HRR_INT__p_p_s_h[3 * 21 + iket] = HRR_INT__d_s_s_h[1 * 21 + iket] + ( hAB[0] * HRR_INT__p_s_s_h[1 * 21 + iket] );

                    HRR_INT__p_p_s_h[4 * 21 + iket] = HRR_INT__d_s_s_h[3 * 21 + iket] + ( hAB[1] * HRR_INT__p_s_s_h[1 * 21 + iket] );

                    HRR_INT__p_p_s_h[5 * 21 + iket] = HRR_INT__d_s_s_h[4 * 21 + iket] + ( hAB[2] * HRR_INT__p_s_s_h[1 * 21 + iket] );

                    HRR_INT__p_p_s_h[6 * 21 + iket] = HRR_INT__d_s_s_h[2 * 21 + iket] + ( hAB[0] * HRR_INT__p_s_s_h[2 * 21 + iket] );

                    HRR_INT__p_p_s_h[7 * 21 + iket] = HRR_INT__d_s_s_h[4 * 21 + iket] + ( hAB[1] * HRR_INT__p_s_s_h[2 * 21 + iket] );

                    HRR_INT__p_p_s_h[8 * 21 + iket] = HRR_INT__d_s_s_h[5 * 21 + iket] + ( hAB[2] * HRR_INT__p_s_s_h[2 * 21 + iket] );
                ...
                {
                    HRR_INT__p_p_p_f[ibra * 30 + 0] = HRR_INT__p_p_s_g[ibra * 15 + 0] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 0] );

                    HRR_INT__p_p_p_f[ibra * 30 + 1] = HRR_INT__p_p_s_g[ibra * 15 + 1] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 1] );

                    HRR_INT__p_p_p_f[ibra * 30 + 2] = HRR_INT__p_p_s_g[ibra * 15 + 2] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 2] );

                    HRR_INT__p_p_p_f[ibra * 30 + 3] = HRR_INT__p_p_s_g[ibra * 15 + 3] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 3] );

                    HRR_INT__p_p_p_f[ibra * 30 + 4] = HRR_INT__p_p_s_g[ibra * 15 + 4] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 4] );

                    HRR_INT__p_p_p_f[ibra * 30 + 5] = HRR_INT__p_p_s_g[ibra * 15 + 5] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 5] );

                    HRR_INT__p_p_p_f[ibra * 30 + 6] = HRR_INT__p_p_s_g[ibra * 15 + 6] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 6] );

                    HRR_INT__p_p_p_f[ibra * 30 + 7] = HRR_INT__p_p_s_g[ibra * 15 + 7] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 7] );

                    HRR_INT__p_p_p_f[ibra * 30 + 8] = HRR_INT__p_p_s_g[ibra * 15 + 8] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 8] );

                    HRR_INT__p_p_p_f[ibra * 30 + 9] = HRR_INT__p_p_s_g[ibra * 15 + 9] - ( hCD[0] * HRR_INT__p_p_s_f[ibra * 10 + 9] );

                    HRR_INT__p_p_p_f[ibra * 30 + 10] = HRR_INT__p_p_s_g[ibra * 15 + 1] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 0] );

                    HRR_INT__p_p_p_f[ibra * 30 + 11] = HRR_INT__p_p_s_g[ibra * 15 + 3] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 1] );

                    HRR_INT__p_p_p_f[ibra * 30 + 12] = HRR_INT__p_p_s_g[ibra * 15 + 4] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 2] );

                    HRR_INT__p_p_p_f[ibra * 30 + 13] = HRR_INT__p_p_s_g[ibra * 15 + 6] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 3] );

                    HRR_INT__p_p_p_f[ibra * 30 + 14] = HRR_INT__p_p_s_g[ibra * 15 + 7] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 4] );

                    HRR_INT__p_p_p_f[ibra * 30 + 15] = HRR_INT__p_p_s_g[ibra * 15 + 8] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 5] );

                    HRR_INT__p_p_p_f[ibra * 30 + 16] = HRR_INT__p_p_s_g[ibra * 15 + 10] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 6] );

                    HRR_INT__p_p_p_f[ibra * 30 + 17] = HRR_INT__p_p_s_g[ibra * 15 + 11] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 7] );

                    HRR_INT__p_p_p_f[ibra * 30 + 18] = HRR_INT__p_p_s_g[ibra * 15 + 12] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 8] );

                    HRR_INT__p_p_p_f[ibra * 30 + 19] = HRR_INT__p_p_s_g[ibra * 15 + 13] - ( hCD[1] * HRR_INT__p_p_s_f[ibra * 10 + 9] );

                    HRR_INT__p_p_p_f[ibra * 30 + 20] = HRR_INT__p_p_s_g[ibra * 15 + 2] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 0] );

                    HRR_INT__p_p_p_f[ibra * 30 + 21] = HRR_INT__p_p_s_g[ibra * 15 + 4] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 1] );

                    HRR_INT__p_p_p_f[ibra * 30 + 22] = HRR_INT__p_p_s_g[ibra * 15 + 5] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 2] );

                    HRR_INT__p_p_p_f[ibra * 30 + 23] = HRR_INT__p_p_s_g[ibra * 15 + 7] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 3] );

                    HRR_INT__p_p_p_f[ibra * 30 + 24] = HRR_INT__p_p_s_g[ibra * 15 + 8] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 4] );

                    HRR_INT__p_p_p_f[ibra * 30 + 25] = HRR_INT__p_p_s_g[ibra * 15 + 9] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 5] );

                    HRR_INT__p_p_p_f[ibra * 30 + 26] = HRR_INT__p_p_s_g[ibra * 15 + 11] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 6] );

                    HRR_INT__p_p_p_f[ibra * 30 + 27] = HRR_INT__p_p_s_g[ibra * 15 + 12] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 7] );

                    HRR_INT__p_p_p_f[ibra * 30 + 28] = HRR_INT__p_p_s_g[ibra * 15 + 13] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 8] );

                    HRR_INT__p_p_p_f[ibra * 30 + 29] = HRR_INT__p_p_s_g[ibra * 15 + 14] - ( hCD[2] * HRR_INT__p_p_s_f[ibra * 10 + 9] );
