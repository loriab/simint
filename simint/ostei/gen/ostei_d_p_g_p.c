                  ...
                  double * const restrict INT__d_p_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_p_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_d_s = primwork + 294;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 348;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 438;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 573;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_f_s = primwork + 762;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 882;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_h_s = primwork + 1062;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_g_s = primwork + 1314;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_h_s = primwork + 1464;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_p_g_p = INT__d_p_g_p + real_abcd * 810;
                  ...
                  double * const restrict INT__p_d_g_p)
{
    ...
    int ret = ostei_d_p_g_p(P_tmp, Q, screen_tol, work, INT__p_d_g_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__p_d_g_p[q*810+b*135+a*45+c*3+d];

        memcpy(INT__p_d_g_p+q*810, buffer, 810*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_p_p_g)
{
    ...
    int ret = ostei_d_p_g_p(P, Q_tmp, screen_tol, work, INT__d_p_p_g);
    double buffer[810] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_p_p_g[q*810+a*135+b*45+d*3+c];

        memcpy(INT__d_p_p_g+q*810, buffer, 810*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_d_p_g)
{
    ...
    int ret = ostei_d_p_g_p(P_tmp, Q_tmp, screen_tol, work, INT__p_d_p_g);
    double buffer[810] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_d_p_g[q*810+b*135+a*45+d*3+c];

        memcpy(INT__p_d_p_g+q*810, buffer, 810*sizeof(double));
    }
