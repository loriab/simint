                  ...
                  double * const restrict INT__p_d_s_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_d_s_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_d_s_p = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_f_s_p = work + (SIMINT_NSHELL_SIMD * 18);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_s_p = primwork + 35;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_s_s = primwork + 53;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_s_p = primwork + 73;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_d_s_p = INT__s_d_s_p + abcd * 18;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__s_d_s_p += lastoffset*18;
                    ...
                    // Forming PRIM_INT__s_d_s_p[1 * 18];
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(18, PRIM_INT__s_d_s_p, PRIM_PTR_INT__s_d_s_p);
                        contract_all(30, PRIM_INT__s_f_s_p, PRIM_PTR_INT__s_f_s_p);
                    }
                    else
                    {
                        contract(18, shelloffsets, PRIM_INT__s_d_s_p, PRIM_PTR_INT__s_d_s_p);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__s_d_s_p = INT__s_d_s_p + abcd * 18;
                double const * restrict HRR_INT__s_f_s_p = INT__s_f_s_p + abcd * 30;
                double * restrict HRR_INT__p_d_s_p = INT__p_d_s_p + real_abcd * 54;

                // form INT__p_d_s_p
                for(iket = 0; iket < 3; ++iket)
                {
                    HRR_INT__p_d_s_p[0 * 3 + iket] = HRR_INT__s_f_s_p[0 * 3 + iket] - ( hAB[0] * HRR_INT__s_d_s_p[0 * 3 + iket] );

                    HRR_INT__p_d_s_p[1 * 3 + iket] = HRR_INT__s_f_s_p[1 * 3 + iket] - ( hAB[0] * HRR_INT__s_d_s_p[1 * 3 + iket] );

                    HRR_INT__p_d_s_p[2 * 3 + iket] = HRR_INT__s_f_s_p[2 * 3 + iket] - ( hAB[0] * HRR_INT__s_d_s_p[2 * 3 + iket] );

                    HRR_INT__p_d_s_p[3 * 3 + iket] = HRR_INT__s_f_s_p[3 * 3 + iket] - ( hAB[0] * HRR_INT__s_d_s_p[3 * 3 + iket] );

                    HRR_INT__p_d_s_p[4 * 3 + iket] = HRR_INT__s_f_s_p[4 * 3 + iket] - ( hAB[0] * HRR_INT__s_d_s_p[4 * 3 + iket] );

                    HRR_INT__p_d_s_p[5 * 3 + iket] = HRR_INT__s_f_s_p[5 * 3 + iket] - ( hAB[0] * HRR_INT__s_d_s_p[5 * 3 + iket] );

                    HRR_INT__p_d_s_p[6 * 3 + iket] = HRR_INT__s_f_s_p[1 * 3 + iket] - ( hAB[1] * HRR_INT__s_d_s_p[0 * 3 + iket] );

                    HRR_INT__p_d_s_p[7 * 3 + iket] = HRR_INT__s_f_s_p[3 * 3 + iket] - ( hAB[1] * HRR_INT__s_d_s_p[1 * 3 + iket] );

                    HRR_INT__p_d_s_p[8 * 3 + iket] = HRR_INT__s_f_s_p[4 * 3 + iket] - ( hAB[1] * HRR_INT__s_d_s_p[2 * 3 + iket] );

                    HRR_INT__p_d_s_p[9 * 3 + iket] = HRR_INT__s_f_s_p[6 * 3 + iket] - ( hAB[1] * HRR_INT__s_d_s_p[3 * 3 + iket] );

                    HRR_INT__p_d_s_p[10 * 3 + iket] = HRR_INT__s_f_s_p[7 * 3 + iket] - ( hAB[1] * HRR_INT__s_d_s_p[4 * 3 + iket] );

                    HRR_INT__p_d_s_p[11 * 3 + iket] = HRR_INT__s_f_s_p[8 * 3 + iket] - ( hAB[1] * HRR_INT__s_d_s_p[5 * 3 + iket] );

                    HRR_INT__p_d_s_p[12 * 3 + iket] = HRR_INT__s_f_s_p[2 * 3 + iket] - ( hAB[2] * HRR_INT__s_d_s_p[0 * 3 + iket] );

                    HRR_INT__p_d_s_p[13 * 3 + iket] = HRR_INT__s_f_s_p[4 * 3 + iket] - ( hAB[2] * HRR_INT__s_d_s_p[1 * 3 + iket] );

                    HRR_INT__p_d_s_p[14 * 3 + iket] = HRR_INT__s_f_s_p[5 * 3 + iket] - ( hAB[2] * HRR_INT__s_d_s_p[2 * 3 + iket] );

                    HRR_INT__p_d_s_p[15 * 3 + iket] = HRR_INT__s_f_s_p[7 * 3 + iket] - ( hAB[2] * HRR_INT__s_d_s_p[3 * 3 + iket] );

                    HRR_INT__p_d_s_p[16 * 3 + iket] = HRR_INT__s_f_s_p[8 * 3 + iket] - ( hAB[2] * HRR_INT__s_d_s_p[4 * 3 + iket] );

                    HRR_INT__p_d_s_p[17 * 3 + iket] = HRR_INT__s_f_s_p[9 * 3 + iket] - ( hAB[2] * HRR_INT__s_d_s_p[5 * 3 + iket] );
