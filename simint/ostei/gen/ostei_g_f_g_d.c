                  ...
                  double * const restrict INT__g_f_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_f_g_p = hrrwork + 10890;
    double * const HRR_INT__g_f_h_s = hrrwork + 17640;
    double * const HRR_INT__g_f_h_p = hrrwork + 20790;
    double * const HRR_INT__g_f_i_s = hrrwork + 30240;
    double * const HRR_INT__h_p_g_s = hrrwork + 34440;
    double * const HRR_INT__h_p_h_s = hrrwork + 35385;
    double * const HRR_INT__h_p_i_s = hrrwork + 36708;
    double * const HRR_INT__h_d_g_s = hrrwork + 38472;
    double * const HRR_INT__h_d_h_s = hrrwork + 40362;
    double * const HRR_INT__h_d_i_s = hrrwork + 43008;
    double * const HRR_INT__i_p_g_s = hrrwork + 46536;
    double * const HRR_INT__i_p_h_s = hrrwork + 47796;
    double * const HRR_INT__i_p_i_s = hrrwork + 49560;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_g_d = INT__g_f_g_d + real_abcd * 13500;
                  ...
                  double * const restrict INT__f_g_g_d)
{
    ...
    int ret = ostei_g_f_g_d(P_tmp, Q, screen_tol, work, INT__f_g_g_d);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__f_g_g_d[q*13500+b*900+a*90+c*6+d];

        memcpy(INT__f_g_g_d+q*13500, buffer, 13500*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_d_g)
{
    ...
    int ret = ostei_g_f_g_d(P, Q_tmp, screen_tol, work, INT__g_f_d_g);
    double buffer[13500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_f_d_g[q*13500+a*900+b*90+d*6+c];

        memcpy(INT__g_f_d_g+q*13500, buffer, 13500*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_d_g)
{
    ...
    int ret = ostei_g_f_g_d(P_tmp, Q_tmp, screen_tol, work, INT__f_g_d_g);
    double buffer[13500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_g_d_g[q*13500+b*900+a*90+d*6+c];

        memcpy(INT__f_g_d_g+q*13500, buffer, 13500*sizeof(double));
    }
