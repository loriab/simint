                  ...
                  double * const restrict INT__s_p_s_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__s_p_s_p);
    memset(INT__s_p_s_p, 0, P.nshell12_clip * Q.nshell12_clip * 9 * sizeof(double));
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_s_p = primwork + 9;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_p_s_p = INT__s_p_s_p + abcd * 9;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__s_p_s_p += lastoffset*9;
                            continue;
                        }
                    }
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(9, PRIM_INT__s_p_s_p, PRIM_PTR_INT__s_p_s_p);
                    }
                    else
                    {
                        contract(9, shelloffsets, PRIM_INT__s_p_s_p, PRIM_PTR_INT__s_p_s_p);
                        PRIM_PTR_INT__s_p_s_p += lastoffset*9;
                    }
