                  ...
                  double * const restrict INT__g_d_p_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_p_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 115;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 175;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 220;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_p_s = primwork + 283;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_s_s = primwork + 346;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_p_s = primwork + 402;
    double * const hrrwork = (double *)(primwork + 486);
    double * const HRR_INT__g_p_p_s = hrrwork + 0;
    double * const HRR_INT__h_p_p_s = hrrwork + 135;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_p_s = INT__g_d_p_s + real_abcd * 270;
                  ...
                  double * const restrict INT__d_g_p_s)
{
    ...
    int ret = ostei_g_d_p_s(P_tmp, Q, screen_tol, work, INT__d_g_p_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__d_g_p_s[q*270+b*18+a*3+c*1+d];

        memcpy(INT__d_g_p_s+q*270, buffer, 270*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_d_s_p)
{
    ...
    int ret = ostei_g_d_p_s(P, Q_tmp, screen_tol, work, INT__g_d_s_p);
    double buffer[270] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__g_d_s_p[q*270+a*18+b*3+d*1+c];

        memcpy(INT__g_d_s_p+q*270, buffer, 270*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_g_s_p)
{
    ...
    int ret = ostei_g_d_p_s(P_tmp, Q_tmp, screen_tol, work, INT__d_g_s_p);
    double buffer[270] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__d_g_s_p[q*270+b*18+a*3+d*1+c];

        memcpy(INT__d_g_s_p+q*270, buffer, 270*sizeof(double));
    }
