                  ...
                  double * const restrict INT__g_f_s_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_s_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 175;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_s_s = primwork + 238;
    SIMINT_DBLTYPE * const restrict PRIM_INT__k_s_s_s = primwork + 294;
    double * const hrrwork = (double *)(primwork + 330);
    double * const HRR_INT__g_p_s_s = hrrwork + 0;
    double * const HRR_INT__g_d_s_s = hrrwork + 45;
    double * const HRR_INT__h_p_s_s = hrrwork + 135;
    double * const HRR_INT__h_d_s_s = hrrwork + 198;
    double * const HRR_INT__i_p_s_s = hrrwork + 324;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_s_s = INT__g_f_s_s + real_abcd * 150;
                  ...
                  double * const restrict INT__f_g_s_s)
{
    ...
    int ret = ostei_g_f_s_s(P_tmp, Q, screen_tol, work, INT__f_g_s_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__f_g_s_s[q*150+b*10+a*1+c*1+d];

        memcpy(INT__f_g_s_s+q*150, buffer, 150*sizeof(double));
    }
