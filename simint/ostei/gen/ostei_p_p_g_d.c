                  ...
                  double * const restrict INT__p_p_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 192);
    double * const INT__d_s_h_s = work + (SIMINT_NSHELL_SIMD * 282);
    double * const INT__d_s_i_s = work + (SIMINT_NSHELL_SIMD * 408);
    ...
    double * const HRR_INT__p_p_h_s = hrrwork + 540;
    double * const HRR_INT__p_p_h_p = hrrwork + 729;
    double * const HRR_INT__p_p_i_s = hrrwork + 1296;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_p_g_d = INT__p_p_g_d + real_abcd * 810;
                  ...
                  double * const restrict INT__p_p_d_g)
{
    ...
    int ret = ostei_p_p_g_d(P, Q_tmp, screen_tol, work, INT__p_p_d_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_p_d_g[q*810+a*270+b*90+d*6+c];

        memcpy(INT__p_p_d_g+q*810, buffer, 810*sizeof(double));
    }
