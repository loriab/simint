                  ...
                  double * const restrict INT__d_d_p_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_d_p_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__d_s_p_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__d_s_d_s = work + (SIMINT_NSHELL_SIMD * 18);
    double * const INT__f_s_p_s = work + (SIMINT_NSHELL_SIMD * 54);
    double * const INT__f_s_d_s = work + (SIMINT_NSHELL_SIMD * 84);
    double * const INT__g_s_p_s = work + (SIMINT_NSHELL_SIMD * 144);
    double * const INT__g_s_d_s = work + (SIMINT_NSHELL_SIMD * 189);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*279);
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_s_s = primwork + 0;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_s_s = primwork + 7;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_p_s = primwork + 25;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_s = primwork + 43;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_p_s = primwork + 73;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_d_s = primwork + 109;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_s_s = primwork + 145;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_p_s = primwork + 185;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_d_s = primwork + 245;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 305;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 350;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_d_s = primwork + 440;
    double * const hrrwork = (double *)(primwork + 530);
    double * const HRR_INT__d_p_p_s = hrrwork + 0;
    double * const HRR_INT__d_p_d_s = hrrwork + 54;
    double * const HRR_INT__d_d_p_s = hrrwork + 162;
    double * const HRR_INT__d_d_d_s = hrrwork + 270;
    double * const HRR_INT__f_p_p_s = hrrwork + 486;
    double * const HRR_INT__f_p_d_s = hrrwork + 576;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 279 * sizeof(double));
            abcd = 0;
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__d_s_p_s = INT__d_s_p_s + abcd * 18;
                double * restrict PRIM_PTR_INT__d_s_d_s = INT__d_s_d_s + abcd * 36;
                double * restrict PRIM_PTR_INT__f_s_p_s = INT__f_s_p_s + abcd * 30;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__d_s_p_s += 18;
                            PRIM_PTR_INT__d_s_d_s += 36;
                            PRIM_PTR_INT__f_s_p_s += 30;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__d_s_p_s += lastoffset*18;
                            PRIM_PTR_INT__d_s_d_s += lastoffset*36;
                            PRIM_PTR_INT__f_s_p_s += lastoffset*30;
                    ...
                    // Forming PRIM_INT__d_s_d_s[1 * 36];
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(18, PRIM_INT__d_s_p_s, PRIM_PTR_INT__d_s_p_s);
                        contract_all(36, PRIM_INT__d_s_d_s, PRIM_PTR_INT__d_s_d_s);
                        contract_all(30, PRIM_INT__f_s_p_s, PRIM_PTR_INT__f_s_p_s);
                    ...
                    {
                        contract(18, shelloffsets, PRIM_INT__d_s_p_s, PRIM_PTR_INT__d_s_p_s);
                        contract(36, shelloffsets, PRIM_INT__d_s_d_s, PRIM_PTR_INT__d_s_d_s);
                        contract(30, shelloffsets, PRIM_INT__f_s_p_s, PRIM_PTR_INT__f_s_p_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__d_s_p_s = INT__d_s_p_s + abcd * 18;
                double const * restrict HRR_INT__d_s_d_s = INT__d_s_d_s + abcd * 36;
                double const * restrict HRR_INT__f_s_p_s = INT__f_s_p_s + abcd * 30;
                ...
                double * restrict HRR_INT__d_d_p_p = INT__d_d_p_p + real_abcd * 324;

                // form INT__d_p_p_s
                for(iket = 0; iket < 3; ++iket)
                {
                    HRR_INT__d_p_p_s[0 * 3 + iket] = HRR_INT__f_s_p_s[0 * 3 + iket] + ( hAB[0] * HRR_INT__d_s_p_s[0 * 3 + iket] );

                    HRR_INT__d_p_p_s[1 * 3 + iket] = HRR_INT__f_s_p_s[1 * 3 + iket] + ( hAB[1] * HRR_INT__d_s_p_s[0 * 3 + iket] );

                    HRR_INT__d_p_p_s[2 * 3 + iket] = HRR_INT__f_s_p_s[2 * 3 + iket] + ( hAB[2] * HRR_INT__d_s_p_s[0 * 3 + iket] );

                    HRR_INT__d_p_p_s[3 * 3 + iket] = HRR_INT__f_s_p_s[1 * 3 + iket] + ( hAB[0] * HRR_INT__d_s_p_s[1 * 3 + iket] );

                    HRR_INT__d_p_p_s[4 * 3 + iket] = HRR_INT__f_s_p_s[3 * 3 + iket] + ( hAB[1] * HRR_INT__d_s_p_s[1 * 3 + iket] );

                    HRR_INT__d_p_p_s[5 * 3 + iket] = HRR_INT__f_s_p_s[4 * 3 + iket] + ( hAB[2] * HRR_INT__d_s_p_s[1 * 3 + iket] );

                    HRR_INT__d_p_p_s[6 * 3 + iket] = HRR_INT__f_s_p_s[2 * 3 + iket] + ( hAB[0] * HRR_INT__d_s_p_s[2 * 3 + iket] );

                    HRR_INT__d_p_p_s[7 * 3 + iket] = HRR_INT__f_s_p_s[4 * 3 + iket] + ( hAB[1] * HRR_INT__d_s_p_s[2 * 3 + iket] );

                    HRR_INT__d_p_p_s[8 * 3 + iket] = HRR_INT__f_s_p_s[5 * 3 + iket] + ( hAB[2] * HRR_INT__d_s_p_s[2 * 3 + iket] );

                    HRR_INT__d_p_p_s[9 * 3 + iket] = HRR_INT__f_s_p_s[3 * 3 + iket] + ( hAB[0] * HRR_INT__d_s_p_s[3 * 3 + iket] );

                    HRR_INT__d_p_p_s[10 * 3 + iket] = HRR_INT__f_s_p_s[6 * 3 + iket] + ( hAB[1] * HRR_INT__d_s_p_s[3 * 3 + iket] );

                    HRR_INT__d_p_p_s[11 * 3 + iket] = HRR_INT__f_s_p_s[7 * 3 + iket] + ( hAB[2] * HRR_INT__d_s_p_s[3 * 3 + iket] );

                    HRR_INT__d_p_p_s[12 * 3 + iket] = HRR_INT__f_s_p_s[4 * 3 + iket] + ( hAB[0] * HRR_INT__d_s_p_s[4 * 3 + iket] );

                    HRR_INT__d_p_p_s[13 * 3 + iket] = HRR_INT__f_s_p_s[7 * 3 + iket] + ( hAB[1] * HRR_INT__d_s_p_s[4 * 3 + iket] );

                    HRR_INT__d_p_p_s[14 * 3 + iket] = HRR_INT__f_s_p_s[8 * 3 + iket] + ( hAB[2] * HRR_INT__d_s_p_s[4 * 3 + iket] );

                    HRR_INT__d_p_p_s[15 * 3 + iket] = HRR_INT__f_s_p_s[5 * 3 + iket] + ( hAB[0] * HRR_INT__d_s_p_s[5 * 3 + iket] );

                    HRR_INT__d_p_p_s[16 * 3 + iket] = HRR_INT__f_s_p_s[8 * 3 + iket] + ( hAB[1] * HRR_INT__d_s_p_s[5 * 3 + iket] );

                    HRR_INT__d_p_p_s[17 * 3 + iket] = HRR_INT__f_s_p_s[9 * 3 + iket] + ( hAB[2] * HRR_INT__d_s_p_s[5 * 3 + iket] );
                ...
                for(iket = 0; iket < 3; ++iket)
                {
                    HRR_INT__f_p_p_s[0 * 3 + iket] = HRR_INT__g_s_p_s[0 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[0 * 3 + iket] );

                    HRR_INT__f_p_p_s[3 * 3 + iket] = HRR_INT__g_s_p_s[1 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[1 * 3 + iket] );

                    HRR_INT__f_p_p_s[4 * 3 + iket] = HRR_INT__g_s_p_s[3 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[1 * 3 + iket] );

                    HRR_INT__f_p_p_s[6 * 3 + iket] = HRR_INT__g_s_p_s[2 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[2 * 3 + iket] );

                    HRR_INT__f_p_p_s[7 * 3 + iket] = HRR_INT__g_s_p_s[4 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[2 * 3 + iket] );

                    HRR_INT__f_p_p_s[8 * 3 + iket] = HRR_INT__g_s_p_s[5 * 3 + iket] + ( hAB[2] * HRR_INT__f_s_p_s[2 * 3 + iket] );

                    HRR_INT__f_p_p_s[9 * 3 + iket] = HRR_INT__g_s_p_s[3 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[3 * 3 + iket] );

                    HRR_INT__f_p_p_s[10 * 3 + iket] = HRR_INT__g_s_p_s[6 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[3 * 3 + iket] );

                    HRR_INT__f_p_p_s[12 * 3 + iket] = HRR_INT__g_s_p_s[4 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[4 * 3 + iket] );

                    HRR_INT__f_p_p_s[13 * 3 + iket] = HRR_INT__g_s_p_s[7 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[4 * 3 + iket] );

                    HRR_INT__f_p_p_s[14 * 3 + iket] = HRR_INT__g_s_p_s[8 * 3 + iket] + ( hAB[2] * HRR_INT__f_s_p_s[4 * 3 + iket] );

                    HRR_INT__f_p_p_s[15 * 3 + iket] = HRR_INT__g_s_p_s[5 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[5 * 3 + iket] );

                    HRR_INT__f_p_p_s[16 * 3 + iket] = HRR_INT__g_s_p_s[8 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[5 * 3 + iket] );

                    HRR_INT__f_p_p_s[17 * 3 + iket] = HRR_INT__g_s_p_s[9 * 3 + iket] + ( hAB[2] * HRR_INT__f_s_p_s[5 * 3 + iket] );

                    HRR_INT__f_p_p_s[18 * 3 + iket] = HRR_INT__g_s_p_s[6 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[6 * 3 + iket] );

                    HRR_INT__f_p_p_s[19 * 3 + iket] = HRR_INT__g_s_p_s[10 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[6 * 3 + iket] );

                    HRR_INT__f_p_p_s[21 * 3 + iket] = HRR_INT__g_s_p_s[7 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[7 * 3 + iket] );

                    HRR_INT__f_p_p_s[22 * 3 + iket] = HRR_INT__g_s_p_s[11 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[7 * 3 + iket] );

                    HRR_INT__f_p_p_s[23 * 3 + iket] = HRR_INT__g_s_p_s[12 * 3 + iket] + ( hAB[2] * HRR_INT__f_s_p_s[7 * 3 + iket] );

                    HRR_INT__f_p_p_s[24 * 3 + iket] = HRR_INT__g_s_p_s[8 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[8 * 3 + iket] );

                    HRR_INT__f_p_p_s[25 * 3 + iket] = HRR_INT__g_s_p_s[12 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[8 * 3 + iket] );

                    HRR_INT__f_p_p_s[26 * 3 + iket] = HRR_INT__g_s_p_s[13 * 3 + iket] + ( hAB[2] * HRR_INT__f_s_p_s[8 * 3 + iket] );

                    HRR_INT__f_p_p_s[27 * 3 + iket] = HRR_INT__g_s_p_s[9 * 3 + iket] + ( hAB[0] * HRR_INT__f_s_p_s[9 * 3 + iket] );

                    HRR_INT__f_p_p_s[28 * 3 + iket] = HRR_INT__g_s_p_s[13 * 3 + iket] + ( hAB[1] * HRR_INT__f_s_p_s[9 * 3 + iket] );

                    HRR_INT__f_p_p_s[29 * 3 + iket] = HRR_INT__g_s_p_s[14 * 3 + iket] + ( hAB[2] * HRR_INT__f_s_p_s[9 * 3 + iket] );
                ...
                for(iket = 0; iket < 3; ++iket)
                {
                    HRR_INT__d_d_p_s[0 * 3 + iket] = HRR_INT__f_p_p_s[0 * 3 + iket] + ( hAB[0] * HRR_INT__d_p_p_s[0 * 3 + iket] );

                    HRR_INT__d_d_p_s[1 * 3 + iket] = HRR_INT__f_p_p_s[3 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[0 * 3 + iket] );

                    HRR_INT__d_d_p_s[2 * 3 + iket] = HRR_INT__f_p_p_s[6 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[0 * 3 + iket] );

                    HRR_INT__d_d_p_s[3 * 3 + iket] = HRR_INT__f_p_p_s[4 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[1 * 3 + iket] );

                    HRR_INT__d_d_p_s[4 * 3 + iket] = HRR_INT__f_p_p_s[7 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[1 * 3 + iket] );

                    HRR_INT__d_d_p_s[5 * 3 + iket] = HRR_INT__f_p_p_s[8 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[2 * 3 + iket] );

                    HRR_INT__d_d_p_s[6 * 3 + iket] = HRR_INT__f_p_p_s[3 * 3 + iket] + ( hAB[0] * HRR_INT__d_p_p_s[3 * 3 + iket] );

                    HRR_INT__d_d_p_s[7 * 3 + iket] = HRR_INT__f_p_p_s[9 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[3 * 3 + iket] );

                    HRR_INT__d_d_p_s[8 * 3 + iket] = HRR_INT__f_p_p_s[12 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[3 * 3 + iket] );

                    HRR_INT__d_d_p_s[9 * 3 + iket] = HRR_INT__f_p_p_s[10 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[4 * 3 + iket] );

                    HRR_INT__d_d_p_s[10 * 3 + iket] = HRR_INT__f_p_p_s[13 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[4 * 3 + iket] );

                    HRR_INT__d_d_p_s[11 * 3 + iket] = HRR_INT__f_p_p_s[14 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[5 * 3 + iket] );

                    HRR_INT__d_d_p_s[12 * 3 + iket] = HRR_INT__f_p_p_s[6 * 3 + iket] + ( hAB[0] * HRR_INT__d_p_p_s[6 * 3 + iket] );

                    HRR_INT__d_d_p_s[13 * 3 + iket] = HRR_INT__f_p_p_s[12 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[6 * 3 + iket] );

                    HRR_INT__d_d_p_s[14 * 3 + iket] = HRR_INT__f_p_p_s[15 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[6 * 3 + iket] );

                    HRR_INT__d_d_p_s[15 * 3 + iket] = HRR_INT__f_p_p_s[13 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[7 * 3 + iket] );

                    HRR_INT__d_d_p_s[16 * 3 + iket] = HRR_INT__f_p_p_s[16 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[7 * 3 + iket] );

                    HRR_INT__d_d_p_s[17 * 3 + iket] = HRR_INT__f_p_p_s[17 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[8 * 3 + iket] );

                    HRR_INT__d_d_p_s[18 * 3 + iket] = HRR_INT__f_p_p_s[9 * 3 + iket] + ( hAB[0] * HRR_INT__d_p_p_s[9 * 3 + iket] );

                    HRR_INT__d_d_p_s[19 * 3 + iket] = HRR_INT__f_p_p_s[18 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[9 * 3 + iket] );

                    HRR_INT__d_d_p_s[20 * 3 + iket] = HRR_INT__f_p_p_s[21 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[9 * 3 + iket] );

                    HRR_INT__d_d_p_s[21 * 3 + iket] = HRR_INT__f_p_p_s[19 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[10 * 3 + iket] );

                    HRR_INT__d_d_p_s[22 * 3 + iket] = HRR_INT__f_p_p_s[22 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[10 * 3 + iket] );

                    HRR_INT__d_d_p_s[23 * 3 + iket] = HRR_INT__f_p_p_s[23 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[11 * 3 + iket] );

                    HRR_INT__d_d_p_s[24 * 3 + iket] = HRR_INT__f_p_p_s[12 * 3 + iket] + ( hAB[0] * HRR_INT__d_p_p_s[12 * 3 + iket] );

                    HRR_INT__d_d_p_s[25 * 3 + iket] = HRR_INT__f_p_p_s[21 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[12 * 3 + iket] );

                    HRR_INT__d_d_p_s[26 * 3 + iket] = HRR_INT__f_p_p_s[24 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[12 * 3 + iket] );

                    HRR_INT__d_d_p_s[27 * 3 + iket] = HRR_INT__f_p_p_s[22 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[13 * 3 + iket] );

                    HRR_INT__d_d_p_s[28 * 3 + iket] = HRR_INT__f_p_p_s[25 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[13 * 3 + iket] );

                    HRR_INT__d_d_p_s[29 * 3 + iket] = HRR_INT__f_p_p_s[26 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[14 * 3 + iket] );

                    HRR_INT__d_d_p_s[30 * 3 + iket] = HRR_INT__f_p_p_s[15 * 3 + iket] + ( hAB[0] * HRR_INT__d_p_p_s[15 * 3 + iket] );

                    HRR_INT__d_d_p_s[31 * 3 + iket] = HRR_INT__f_p_p_s[24 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[15 * 3 + iket] );

                    HRR_INT__d_d_p_s[32 * 3 + iket] = HRR_INT__f_p_p_s[27 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[15 * 3 + iket] );

                    HRR_INT__d_d_p_s[33 * 3 + iket] = HRR_INT__f_p_p_s[25 * 3 + iket] + ( hAB[1] * HRR_INT__d_p_p_s[16 * 3 + iket] );

                    HRR_INT__d_d_p_s[34 * 3 + iket] = HRR_INT__f_p_p_s[28 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[16 * 3 + iket] );

                    HRR_INT__d_d_p_s[35 * 3 + iket] = HRR_INT__f_p_p_s[29 * 3 + iket] + ( hAB[2] * HRR_INT__d_p_p_s[17 * 3 + iket] );
                ...
                {
                    HRR_INT__d_d_p_p[ibra * 9 + 0] = HRR_INT__d_d_d_s[ibra * 6 + 0] + ( hCD[0] * HRR_INT__d_d_p_s[ibra * 3 + 0] );

                    HRR_INT__d_d_p_p[ibra * 9 + 1] = HRR_INT__d_d_d_s[ibra * 6 + 1] + ( hCD[1] * HRR_INT__d_d_p_s[ibra * 3 + 0] );

                    HRR_INT__d_d_p_p[ibra * 9 + 2] = HRR_INT__d_d_d_s[ibra * 6 + 2] + ( hCD[2] * HRR_INT__d_d_p_s[ibra * 3 + 0] );

                    HRR_INT__d_d_p_p[ibra * 9 + 3] = HRR_INT__d_d_d_s[ibra * 6 + 1] + ( hCD[0] * HRR_INT__d_d_p_s[ibra * 3 + 1] );

                    HRR_INT__d_d_p_p[ibra * 9 + 4] = HRR_INT__d_d_d_s[ibra * 6 + 3] + ( hCD[1] * HRR_INT__d_d_p_s[ibra * 3 + 1] );

                    HRR_INT__d_d_p_p[ibra * 9 + 5] = HRR_INT__d_d_d_s[ibra * 6 + 4] + ( hCD[2] * HRR_INT__d_d_p_s[ibra * 3 + 1] );

                    HRR_INT__d_d_p_p[ibra * 9 + 6] = HRR_INT__d_d_d_s[ibra * 6 + 2] + ( hCD[0] * HRR_INT__d_d_p_s[ibra * 3 + 2] );

                    HRR_INT__d_d_p_p[ibra * 9 + 7] = HRR_INT__d_d_d_s[ibra * 6 + 4] + ( hCD[1] * HRR_INT__d_d_p_s[ibra * 3 + 2] );

                    HRR_INT__d_d_p_p[ibra * 9 + 8] = HRR_INT__d_d_d_s[ibra * 6 + 5] + ( hCD[2] * HRR_INT__d_d_p_s[ibra * 3 + 2] );
