                  ...
                  double * const restrict INT__f_p_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_p_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*375);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 375 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_p_g_s = INT__f_p_g_s + real_abcd * 450;
                  ...
                  double * const restrict INT__p_f_g_s)
{
    ...
    int ret = ostei_f_p_g_s(P_tmp, Q, screen_tol, work, INT__p_f_g_s);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__p_f_g_s[q*450+b*45+a*15+c*1+d];

        memcpy(INT__p_f_g_s+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_p_s_g)
{
    ...
    int ret = ostei_f_p_g_s(P, Q_tmp, screen_tol, work, INT__f_p_s_g);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_p_s_g[q*450+a*45+b*15+d*1+c];

        memcpy(INT__f_p_s_g+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_f_s_g)
{
    ...
    int ret = ostei_f_p_g_s(P_tmp, Q_tmp, screen_tol, work, INT__p_f_s_g);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_f_s_g[q*450+b*45+a*15+d*1+c];

        memcpy(INT__p_f_s_g+q*450, buffer, 450*sizeof(double));
    }
