                  ...
                  double * const restrict INT__s_p_d_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__s_p_d_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_p_d_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_p_f_s = work + (SIMINT_NSHELL_SIMD * 18);
    double * const INT__s_p_g_s = work + (SIMINT_NSHELL_SIMD * 48);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*93);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_d_s = primwork + 105;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_f_s = primwork + 123;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_p_g_s = primwork + 153;
    double * const hrrwork = (double *)(primwork + 198);
    double * const HRR_INT__s_p_d_p = hrrwork + 0;
    double * const HRR_INT__s_p_f_p = hrrwork + 54;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 93 * sizeof(double));
            abcd = 0;
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_p_d_s = INT__s_p_d_s + abcd * 18;
                double * restrict PRIM_PTR_INT__s_p_f_s = INT__s_p_f_s + abcd * 30;
                double * restrict PRIM_PTR_INT__s_p_g_s = INT__s_p_g_s + abcd * 45;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__s_p_d_s += 18;
                            PRIM_PTR_INT__s_p_f_s += 30;
                            PRIM_PTR_INT__s_p_g_s += 45;
                        }
                        iprimcd++;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__s_p_d_s += lastoffset*18;
                            PRIM_PTR_INT__s_p_f_s += lastoffset*30;
                            PRIM_PTR_INT__s_p_g_s += lastoffset*45;
                            continue;
                        }
                    }
                    ...
                    // Forming PRIM_INT__s_p_d_s[1 * 18];
                    ...
                    // Forming PRIM_INT__s_p_f_s[1 * 30];
                    for(n = 0; n < 1; ++n)  // loop over orders of auxiliary function
                    {
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(18, PRIM_INT__s_p_d_s, PRIM_PTR_INT__s_p_d_s);
                        contract_all(30, PRIM_INT__s_p_f_s, PRIM_PTR_INT__s_p_f_s);
                        contract_all(45, PRIM_INT__s_p_g_s, PRIM_PTR_INT__s_p_g_s);
                    }
                    else
                    {
                        contract(18, shelloffsets, PRIM_INT__s_p_d_s, PRIM_PTR_INT__s_p_d_s);
                        contract(30, shelloffsets, PRIM_INT__s_p_f_s, PRIM_PTR_INT__s_p_f_s);
                        contract(45, shelloffsets, PRIM_INT__s_p_g_s, PRIM_PTR_INT__s_p_g_s);
                        PRIM_PTR_INT__s_p_d_s += lastoffset*18;
                        PRIM_PTR_INT__s_p_f_s += lastoffset*30;
                        PRIM_PTR_INT__s_p_g_s += lastoffset*45;
                    }
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__s_p_d_s = INT__s_p_d_s + abcd * 18;
                double const * restrict HRR_INT__s_p_f_s = INT__s_p_f_s + abcd * 30;
                double const * restrict HRR_INT__s_p_g_s = INT__s_p_g_s + abcd * 45;
                double * restrict HRR_INT__s_p_d_d = INT__s_p_d_d + real_abcd * 108;

                // form INT__s_p_d_p
                for(ibra = 0; ibra < 3; ++ibra)
                {
                    HRR_INT__s_p_d_p[ibra * 18 + 0] = HRR_INT__s_p_f_s[ibra * 10 + 0] + ( hCD[0] * HRR_INT__s_p_d_s[ibra * 6 + 0] );

                    HRR_INT__s_p_d_p[ibra * 18 + 1] = HRR_INT__s_p_f_s[ibra * 10 + 1] + ( hCD[1] * HRR_INT__s_p_d_s[ibra * 6 + 0] );

                    HRR_INT__s_p_d_p[ibra * 18 + 2] = HRR_INT__s_p_f_s[ibra * 10 + 2] + ( hCD[2] * HRR_INT__s_p_d_s[ibra * 6 + 0] );

                    HRR_INT__s_p_d_p[ibra * 18 + 3] = HRR_INT__s_p_f_s[ibra * 10 + 1] + ( hCD[0] * HRR_INT__s_p_d_s[ibra * 6 + 1] );

                    HRR_INT__s_p_d_p[ibra * 18 + 4] = HRR_INT__s_p_f_s[ibra * 10 + 3] + ( hCD[1] * HRR_INT__s_p_d_s[ibra * 6 + 1] );

                    HRR_INT__s_p_d_p[ibra * 18 + 5] = HRR_INT__s_p_f_s[ibra * 10 + 4] + ( hCD[2] * HRR_INT__s_p_d_s[ibra * 6 + 1] );

                    HRR_INT__s_p_d_p[ibra * 18 + 6] = HRR_INT__s_p_f_s[ibra * 10 + 2] + ( hCD[0] * HRR_INT__s_p_d_s[ibra * 6 + 2] );

                    HRR_INT__s_p_d_p[ibra * 18 + 7] = HRR_INT__s_p_f_s[ibra * 10 + 4] + ( hCD[1] * HRR_INT__s_p_d_s[ibra * 6 + 2] );

                    HRR_INT__s_p_d_p[ibra * 18 + 8] = HRR_INT__s_p_f_s[ibra * 10 + 5] + ( hCD[2] * HRR_INT__s_p_d_s[ibra * 6 + 2] );

                    HRR_INT__s_p_d_p[ibra * 18 + 9] = HRR_INT__s_p_f_s[ibra * 10 + 3] + ( hCD[0] * HRR_INT__s_p_d_s[ibra * 6 + 3] );

                    HRR_INT__s_p_d_p[ibra * 18 + 10] = HRR_INT__s_p_f_s[ibra * 10 + 6] + ( hCD[1] * HRR_INT__s_p_d_s[ibra * 6 + 3] );

                    HRR_INT__s_p_d_p[ibra * 18 + 11] = HRR_INT__s_p_f_s[ibra * 10 + 7] + ( hCD[2] * HRR_INT__s_p_d_s[ibra * 6 + 3] );

                    HRR_INT__s_p_d_p[ibra * 18 + 12] = HRR_INT__s_p_f_s[ibra * 10 + 4] + ( hCD[0] * HRR_INT__s_p_d_s[ibra * 6 + 4] );

                    HRR_INT__s_p_d_p[ibra * 18 + 13] = HRR_INT__s_p_f_s[ibra * 10 + 7] + ( hCD[1] * HRR_INT__s_p_d_s[ibra * 6 + 4] );

                    HRR_INT__s_p_d_p[ibra * 18 + 14] = HRR_INT__s_p_f_s[ibra * 10 + 8] + ( hCD[2] * HRR_INT__s_p_d_s[ibra * 6 + 4] );

                    HRR_INT__s_p_d_p[ibra * 18 + 15] = HRR_INT__s_p_f_s[ibra * 10 + 5] + ( hCD[0] * HRR_INT__s_p_d_s[ibra * 6 + 5] );

                    HRR_INT__s_p_d_p[ibra * 18 + 16] = HRR_INT__s_p_f_s[ibra * 10 + 8] + ( hCD[1] * HRR_INT__s_p_d_s[ibra * 6 + 5] );

                    HRR_INT__s_p_d_p[ibra * 18 + 17] = HRR_INT__s_p_f_s[ibra * 10 + 9] + ( hCD[2] * HRR_INT__s_p_d_s[ibra * 6 + 5] );
                ...
                for(ibra = 0; ibra < 3; ++ibra)
                {
                    HRR_INT__s_p_f_p[ibra * 30 + 0] = HRR_INT__s_p_g_s[ibra * 15 + 0] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 0] );

                    HRR_INT__s_p_f_p[ibra * 30 + 3] = HRR_INT__s_p_g_s[ibra * 15 + 1] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 1] );

                    HRR_INT__s_p_f_p[ibra * 30 + 4] = HRR_INT__s_p_g_s[ibra * 15 + 3] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 1] );

                    HRR_INT__s_p_f_p[ibra * 30 + 6] = HRR_INT__s_p_g_s[ibra * 15 + 2] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 2] );

                    HRR_INT__s_p_f_p[ibra * 30 + 7] = HRR_INT__s_p_g_s[ibra * 15 + 4] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 2] );

                    HRR_INT__s_p_f_p[ibra * 30 + 8] = HRR_INT__s_p_g_s[ibra * 15 + 5] + ( hCD[2] * HRR_INT__s_p_f_s[ibra * 10 + 2] );

                    HRR_INT__s_p_f_p[ibra * 30 + 9] = HRR_INT__s_p_g_s[ibra * 15 + 3] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 3] );

                    HRR_INT__s_p_f_p[ibra * 30 + 10] = HRR_INT__s_p_g_s[ibra * 15 + 6] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 3] );

                    HRR_INT__s_p_f_p[ibra * 30 + 12] = HRR_INT__s_p_g_s[ibra * 15 + 4] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 4] );

                    HRR_INT__s_p_f_p[ibra * 30 + 13] = HRR_INT__s_p_g_s[ibra * 15 + 7] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 4] );

                    HRR_INT__s_p_f_p[ibra * 30 + 14] = HRR_INT__s_p_g_s[ibra * 15 + 8] + ( hCD[2] * HRR_INT__s_p_f_s[ibra * 10 + 4] );

                    HRR_INT__s_p_f_p[ibra * 30 + 15] = HRR_INT__s_p_g_s[ibra * 15 + 5] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 5] );

                    HRR_INT__s_p_f_p[ibra * 30 + 16] = HRR_INT__s_p_g_s[ibra * 15 + 8] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 5] );

                    HRR_INT__s_p_f_p[ibra * 30 + 17] = HRR_INT__s_p_g_s[ibra * 15 + 9] + ( hCD[2] * HRR_INT__s_p_f_s[ibra * 10 + 5] );

                    HRR_INT__s_p_f_p[ibra * 30 + 18] = HRR_INT__s_p_g_s[ibra * 15 + 6] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 6] );

                    HRR_INT__s_p_f_p[ibra * 30 + 19] = HRR_INT__s_p_g_s[ibra * 15 + 10] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 6] );

                    HRR_INT__s_p_f_p[ibra * 30 + 21] = HRR_INT__s_p_g_s[ibra * 15 + 7] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 7] );

                    HRR_INT__s_p_f_p[ibra * 30 + 22] = HRR_INT__s_p_g_s[ibra * 15 + 11] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 7] );

                    HRR_INT__s_p_f_p[ibra * 30 + 23] = HRR_INT__s_p_g_s[ibra * 15 + 12] + ( hCD[2] * HRR_INT__s_p_f_s[ibra * 10 + 7] );

                    HRR_INT__s_p_f_p[ibra * 30 + 24] = HRR_INT__s_p_g_s[ibra * 15 + 8] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 8] );

                    HRR_INT__s_p_f_p[ibra * 30 + 25] = HRR_INT__s_p_g_s[ibra * 15 + 12] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 8] );

                    HRR_INT__s_p_f_p[ibra * 30 + 26] = HRR_INT__s_p_g_s[ibra * 15 + 13] + ( hCD[2] * HRR_INT__s_p_f_s[ibra * 10 + 8] );

                    HRR_INT__s_p_f_p[ibra * 30 + 27] = HRR_INT__s_p_g_s[ibra * 15 + 9] + ( hCD[0] * HRR_INT__s_p_f_s[ibra * 10 + 9] );

                    HRR_INT__s_p_f_p[ibra * 30 + 28] = HRR_INT__s_p_g_s[ibra * 15 + 13] + ( hCD[1] * HRR_INT__s_p_f_s[ibra * 10 + 9] );

                    HRR_INT__s_p_f_p[ibra * 30 + 29] = HRR_INT__s_p_g_s[ibra * 15 + 14] + ( hCD[2] * HRR_INT__s_p_f_s[ibra * 10 + 9] );
                ...
                for(ibra = 0; ibra < 3; ++ibra)
                {
                    HRR_INT__s_p_d_d[ibra * 36 + 0] = HRR_INT__s_p_f_p[ibra * 30 + 0] + ( hCD[0] * HRR_INT__s_p_d_p[ibra * 18 + 0] );

                    HRR_INT__s_p_d_d[ibra * 36 + 1] = HRR_INT__s_p_f_p[ibra * 30 + 3] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 0] );

                    HRR_INT__s_p_d_d[ibra * 36 + 2] = HRR_INT__s_p_f_p[ibra * 30 + 6] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 0] );

                    HRR_INT__s_p_d_d[ibra * 36 + 3] = HRR_INT__s_p_f_p[ibra * 30 + 4] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 1] );

                    HRR_INT__s_p_d_d[ibra * 36 + 4] = HRR_INT__s_p_f_p[ibra * 30 + 7] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 1] );

                    HRR_INT__s_p_d_d[ibra * 36 + 5] = HRR_INT__s_p_f_p[ibra * 30 + 8] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 2] );

                    HRR_INT__s_p_d_d[ibra * 36 + 6] = HRR_INT__s_p_f_p[ibra * 30 + 3] + ( hCD[0] * HRR_INT__s_p_d_p[ibra * 18 + 3] );

                    HRR_INT__s_p_d_d[ibra * 36 + 7] = HRR_INT__s_p_f_p[ibra * 30 + 9] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 3] );

                    HRR_INT__s_p_d_d[ibra * 36 + 8] = HRR_INT__s_p_f_p[ibra * 30 + 12] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 3] );

                    HRR_INT__s_p_d_d[ibra * 36 + 9] = HRR_INT__s_p_f_p[ibra * 30 + 10] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 4] );

                    HRR_INT__s_p_d_d[ibra * 36 + 10] = HRR_INT__s_p_f_p[ibra * 30 + 13] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 4] );

                    HRR_INT__s_p_d_d[ibra * 36 + 11] = HRR_INT__s_p_f_p[ibra * 30 + 14] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 5] );

                    HRR_INT__s_p_d_d[ibra * 36 + 12] = HRR_INT__s_p_f_p[ibra * 30 + 6] + ( hCD[0] * HRR_INT__s_p_d_p[ibra * 18 + 6] );

                    HRR_INT__s_p_d_d[ibra * 36 + 13] = HRR_INT__s_p_f_p[ibra * 30 + 12] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 6] );

                    HRR_INT__s_p_d_d[ibra * 36 + 14] = HRR_INT__s_p_f_p[ibra * 30 + 15] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 6] );

                    HRR_INT__s_p_d_d[ibra * 36 + 15] = HRR_INT__s_p_f_p[ibra * 30 + 13] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 7] );

                    HRR_INT__s_p_d_d[ibra * 36 + 16] = HRR_INT__s_p_f_p[ibra * 30 + 16] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 7] );

                    HRR_INT__s_p_d_d[ibra * 36 + 17] = HRR_INT__s_p_f_p[ibra * 30 + 17] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 8] );

                    HRR_INT__s_p_d_d[ibra * 36 + 18] = HRR_INT__s_p_f_p[ibra * 30 + 9] + ( hCD[0] * HRR_INT__s_p_d_p[ibra * 18 + 9] );

                    HRR_INT__s_p_d_d[ibra * 36 + 19] = HRR_INT__s_p_f_p[ibra * 30 + 18] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 9] );

                    HRR_INT__s_p_d_d[ibra * 36 + 20] = HRR_INT__s_p_f_p[ibra * 30 + 21] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 9] );

                    HRR_INT__s_p_d_d[ibra * 36 + 21] = HRR_INT__s_p_f_p[ibra * 30 + 19] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 10] );

                    HRR_INT__s_p_d_d[ibra * 36 + 22] = HRR_INT__s_p_f_p[ibra * 30 + 22] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 10] );

                    HRR_INT__s_p_d_d[ibra * 36 + 23] = HRR_INT__s_p_f_p[ibra * 30 + 23] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 11] );

                    HRR_INT__s_p_d_d[ibra * 36 + 24] = HRR_INT__s_p_f_p[ibra * 30 + 12] + ( hCD[0] * HRR_INT__s_p_d_p[ibra * 18 + 12] );

                    HRR_INT__s_p_d_d[ibra * 36 + 25] = HRR_INT__s_p_f_p[ibra * 30 + 21] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 12] );

                    HRR_INT__s_p_d_d[ibra * 36 + 26] = HRR_INT__s_p_f_p[ibra * 30 + 24] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 12] );

                    HRR_INT__s_p_d_d[ibra * 36 + 27] = HRR_INT__s_p_f_p[ibra * 30 + 22] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 13] );

                    HRR_INT__s_p_d_d[ibra * 36 + 28] = HRR_INT__s_p_f_p[ibra * 30 + 25] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 13] );

                    HRR_INT__s_p_d_d[ibra * 36 + 29] = HRR_INT__s_p_f_p[ibra * 30 + 26] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 14] );

                    HRR_INT__s_p_d_d[ibra * 36 + 30] = HRR_INT__s_p_f_p[ibra * 30 + 15] + ( hCD[0] * HRR_INT__s_p_d_p[ibra * 18 + 15] );

                    HRR_INT__s_p_d_d[ibra * 36 + 31] = HRR_INT__s_p_f_p[ibra * 30 + 24] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 15] );

                    HRR_INT__s_p_d_d[ibra * 36 + 32] = HRR_INT__s_p_f_p[ibra * 30 + 27] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 15] );

                    HRR_INT__s_p_d_d[ibra * 36 + 33] = HRR_INT__s_p_f_p[ibra * 30 + 25] + ( hCD[1] * HRR_INT__s_p_d_p[ibra * 18 + 16] );

                    HRR_INT__s_p_d_d[ibra * 36 + 34] = HRR_INT__s_p_f_p[ibra * 30 + 28] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 16] );

                    HRR_INT__s_p_d_d[ibra * 36 + 35] = HRR_INT__s_p_f_p[ibra * 30 + 29] + ( hCD[2] * HRR_INT__s_p_d_p[ibra * 18 + 17] );
