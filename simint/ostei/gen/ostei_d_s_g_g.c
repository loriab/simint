                  ...
                  double * const restrict INT__d_s_g_g)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_s_g_g);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__d_s_g_p = hrrwork + 0;
    double * const HRR_INT__d_s_g_d = hrrwork + 270;
    double * const HRR_INT__d_s_g_f = hrrwork + 810;
    double * const HRR_INT__d_s_h_p = hrrwork + 1710;
    double * const HRR_INT__d_s_h_d = hrrwork + 2088;
    double * const HRR_INT__d_s_h_f = hrrwork + 2844;
    double * const HRR_INT__d_s_i_p = hrrwork + 4104;
    double * const HRR_INT__d_s_i_d = hrrwork + 4608;
    double * const HRR_INT__d_s_k_p = hrrwork + 5616;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_s_g_g = INT__d_s_g_g + real_abcd * 1350;
                  ...
                  double * const restrict INT__s_d_g_g)
{
    ...
    int ret = ostei_d_s_g_g(P_tmp, Q, screen_tol, work, INT__s_d_g_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_d_g_g[q*1350+b*225+a*225+c*15+d];

        memcpy(INT__s_d_g_g+q*1350, buffer, 1350*sizeof(double));
    }
