                  ...
                  double * const restrict INT__p_p_f_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_f_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__d_s_f_s = work + (SIMINT_NSHELL_SIMD * 75);
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 135);
    ...
    double * const HRR_INT__p_p_g_s = hrrwork + 90;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_p_f_p = INT__p_p_f_p + real_abcd * 270;
