                  ...
                  double * const restrict INT__p_s_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_s_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 182;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 227;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_s_g_p = INT__p_s_g_p + real_abcd * 135;
                  ...
                  double * const restrict INT__s_p_g_p)
{
    ...
    int ret = ostei_p_s_g_p(P_tmp, Q, screen_tol, work, INT__s_p_g_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__s_p_g_p[q*135+b*45+a*45+c*3+d];

        memcpy(INT__s_p_g_p+q*135, buffer, 135*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_s_p_g)
{
    ...
    int ret = ostei_p_s_g_p(P, Q_tmp, screen_tol, work, INT__p_s_p_g);
    double buffer[135] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_s_p_g[q*135+a*45+b*45+d*3+c];

        memcpy(INT__p_s_p_g+q*135, buffer, 135*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_p_p_g)
{
    ...
    int ret = ostei_p_s_g_p(P_tmp, Q_tmp, screen_tol, work, INT__s_p_p_g);
    double buffer[135] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_p_p_g[q*135+b*45+a*45+d*3+c];

        memcpy(INT__s_p_p_g+q*135, buffer, 135*sizeof(double));
    }
