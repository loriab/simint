                  ...
                  double * const restrict INT__g_d_d_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_d_d_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*384);
    ...
    double * const HRR_INT__h_p_d_s = hrrwork + 270;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 384 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_d_d_s = INT__g_d_d_s + real_abcd * 540;
                  ...
                  double * const restrict INT__d_g_d_s)
{
    ...
    int ret = ostei_g_d_d_s(P_tmp, Q, screen_tol, work, INT__d_g_d_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__d_g_d_s[q*540+b*36+a*6+c*1+d];

        memcpy(INT__d_g_d_s+q*540, buffer, 540*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_d_s_d)
{
    ...
    int ret = ostei_g_d_d_s(P, Q_tmp, screen_tol, work, INT__g_d_s_d);
    double buffer[540] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__g_d_s_d[q*540+a*36+b*6+d*1+c];

        memcpy(INT__g_d_s_d+q*540, buffer, 540*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_g_s_d)
{
    ...
    int ret = ostei_g_d_d_s(P_tmp, Q_tmp, screen_tol, work, INT__d_g_s_d);
    double buffer[540] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__d_g_s_d[q*540+b*36+a*6+d*1+c];

        memcpy(INT__d_g_s_d+q*540, buffer, 540*sizeof(double));
    }
