                  ...
                  double * const restrict INT__f_p_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_p_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_p_s = primwork + 546;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_d_s = primwork + 582;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_f_s = primwork + 654;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_g_s = primwork + 774;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_h_s = primwork + 954;
    SIMINT_DBLTYPE * const restrict PRIM_INT__p_s_i_s = primwork + 1206;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_d_s = primwork + 1542;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_f_s = primwork + 1650;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_g_s = primwork + 1830;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_h_s = primwork + 2100;
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_i_s = primwork + 2478;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_f_s = primwork + 2982;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_g_s = primwork + 3182;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_h_s = primwork + 3482;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_i_s = primwork + 3902;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_g_s = primwork + 4462;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_h_s = primwork + 4687;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_i_s = primwork + 5002;
    ...
    double * const HRR_INT__f_p_h_s = hrrwork + 1800;
    double * const HRR_INT__f_p_h_p = hrrwork + 2430;
    double * const HRR_INT__f_p_i_s = hrrwork + 4320;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_p_g_d = INT__f_p_g_d + real_abcd * 2700;
                  ...
                  double * const restrict INT__p_f_g_d)
{
    ...
    int ret = ostei_f_p_g_d(P_tmp, Q, screen_tol, work, INT__p_f_g_d);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__p_f_g_d[q*2700+b*270+a*90+c*6+d];

        memcpy(INT__p_f_g_d+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_p_d_g)
{
    ...
    int ret = ostei_f_p_g_d(P, Q_tmp, screen_tol, work, INT__f_p_d_g);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_p_d_g[q*2700+a*270+b*90+d*6+c];

        memcpy(INT__f_p_d_g+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_f_d_g)
{
    ...
    int ret = ostei_f_p_g_d(P_tmp, Q_tmp, screen_tol, work, INT__p_f_d_g);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_f_d_g[q*2700+b*270+a*90+d*6+c];

        memcpy(INT__p_f_d_g+q*2700, buffer, 2700*sizeof(double));
    }
