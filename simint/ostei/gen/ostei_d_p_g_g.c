                  ...
                  double * const restrict INT__d_p_g_g)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_p_g_g);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__d_p_g_f = hrrwork + 2700;
    double * const HRR_INT__d_p_h_s = hrrwork + 5400;
    double * const HRR_INT__d_p_h_p = hrrwork + 5778;
    double * const HRR_INT__d_p_h_d = hrrwork + 6912;
    double * const HRR_INT__d_p_h_f = hrrwork + 9180;
    double * const HRR_INT__d_p_i_s = hrrwork + 12960;
    double * const HRR_INT__d_p_i_p = hrrwork + 13464;
    double * const HRR_INT__d_p_i_d = hrrwork + 14976;
    double * const HRR_INT__d_p_k_s = hrrwork + 18000;
    double * const HRR_INT__d_p_k_p = hrrwork + 18648;
    double * const HRR_INT__d_p_l_s = hrrwork + 20592;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_p_g_g = INT__d_p_g_g + real_abcd * 4050;
                  ...
                  double * const restrict INT__p_d_g_g)
{
    ...
    int ret = ostei_d_p_g_g(P_tmp, Q, screen_tol, work, INT__p_d_g_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_d_g_g[q*4050+b*675+a*225+c*15+d];

        memcpy(INT__p_d_g_g+q*4050, buffer, 4050*sizeof(double));
    }
