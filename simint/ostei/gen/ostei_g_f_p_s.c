                  ...
                  double * const restrict INT__g_f_p_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_p_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 210;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 255;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_p_s = primwork + 339;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_s_s = primwork + 402;
    SIMINT_DBLTYPE * const restrict PRIM_INT__i_s_p_s = primwork + 486;
    SIMINT_DBLTYPE * const restrict PRIM_INT__k_s_s_s = primwork + 570;
    SIMINT_DBLTYPE * const restrict PRIM_INT__k_s_p_s = primwork + 642;
    double * const hrrwork = (double *)(primwork + 750);
    double * const HRR_INT__g_p_p_s = hrrwork + 0;
    double * const HRR_INT__g_d_p_s = hrrwork + 135;
    double * const HRR_INT__h_p_p_s = hrrwork + 405;
    double * const HRR_INT__h_d_p_s = hrrwork + 594;
    double * const HRR_INT__i_p_p_s = hrrwork + 972;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_p_s = INT__g_f_p_s + real_abcd * 450;
                  ...
                  double * const restrict INT__f_g_p_s)
{
    ...
    int ret = ostei_g_f_p_s(P_tmp, Q, screen_tol, work, INT__f_g_p_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__f_g_p_s[q*450+b*30+a*3+c*1+d];

        memcpy(INT__f_g_p_s+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_s_p)
{
    ...
    int ret = ostei_g_f_p_s(P, Q_tmp, screen_tol, work, INT__g_f_s_p);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__g_f_s_p[q*450+a*30+b*3+d*1+c];

        memcpy(INT__g_f_s_p+q*450, buffer, 450*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_s_p)
{
    ...
    int ret = ostei_g_f_p_s(P_tmp, Q_tmp, screen_tol, work, INT__f_g_s_p);
    double buffer[450] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__f_g_s_p[q*450+b*30+a*3+d*1+c];

        memcpy(INT__f_g_s_p+q*450, buffer, 450*sizeof(double));
    }
