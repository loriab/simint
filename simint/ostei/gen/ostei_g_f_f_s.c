                  ...
                  double * const restrict INT__g_f_f_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_f_f_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__h_s_f_s = work + (SIMINT_NSHELL_SIMD * 150);
    double * const INT__i_s_f_s = work + (SIMINT_NSHELL_SIMD * 360);
    double * const INT__k_s_f_s = work + (SIMINT_NSHELL_SIMD * 640);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*1000);
    ...
    double * const HRR_INT__g_d_f_s = hrrwork + 450;
    double * const HRR_INT__h_p_f_s = hrrwork + 1350;
    double * const HRR_INT__h_d_f_s = hrrwork + 1980;
    double * const HRR_INT__i_p_f_s = hrrwork + 3240;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 1000 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_f_f_s = INT__g_f_f_s + real_abcd * 1500;
                  ...
                  double * const restrict INT__f_g_f_s)
{
    ...
    int ret = ostei_g_f_f_s(P_tmp, Q, screen_tol, work, INT__f_g_f_s);
    double buffer[1500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__f_g_f_s[q*1500+b*100+a*10+c*1+d];

        memcpy(INT__f_g_f_s+q*1500, buffer, 1500*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_f_s_f)
{
    ...
    int ret = ostei_g_f_f_s(P, Q_tmp, screen_tol, work, INT__g_f_s_f);
    double buffer[1500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__g_f_s_f[q*1500+a*100+b*10+d*1+c];

        memcpy(INT__g_f_s_f+q*1500, buffer, 1500*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_g_s_f)
{
    ...
    int ret = ostei_g_f_f_s(P_tmp, Q_tmp, screen_tol, work, INT__f_g_s_f);
    double buffer[1500] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__f_g_s_f[q*1500+b*100+a*10+d*1+c];

        memcpy(INT__f_g_s_f+q*1500, buffer, 1500*sizeof(double));
    }
