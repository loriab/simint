                  ...
                  double * const restrict INT__p_p_f_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__p_p_f_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__d_s_f_s = work + (SIMINT_NSHELL_SIMD * 138);
    double * const INT__d_s_g_s = work + (SIMINT_NSHELL_SIMD * 198);
    double * const INT__d_s_h_s = work + (SIMINT_NSHELL_SIMD * 288);
    ...
    double * const HRR_INT__p_p_g_s = hrrwork + 360;
    double * const HRR_INT__p_p_g_p = hrrwork + 495;
    double * const HRR_INT__p_p_h_s = hrrwork + 900;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__p_p_f_d = INT__p_p_f_d + real_abcd * 540;
