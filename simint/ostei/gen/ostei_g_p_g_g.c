                  ...
                  double * const restrict INT__g_p_g_g)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_g_g);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_p_g_p = hrrwork + 675;
    double * const HRR_INT__g_p_g_d = hrrwork + 2700;
    double * const HRR_INT__g_p_g_f = hrrwork + 6750;
    double * const HRR_INT__g_p_h_s = hrrwork + 13500;
    double * const HRR_INT__g_p_h_p = hrrwork + 14445;
    double * const HRR_INT__g_p_h_d = hrrwork + 17280;
    double * const HRR_INT__g_p_h_f = hrrwork + 22950;
    double * const HRR_INT__g_p_i_s = hrrwork + 32400;
    double * const HRR_INT__g_p_i_p = hrrwork + 33660;
    double * const HRR_INT__g_p_i_d = hrrwork + 37440;
    double * const HRR_INT__g_p_k_s = hrrwork + 45000;
    double * const HRR_INT__g_p_k_p = hrrwork + 46620;
    double * const HRR_INT__g_p_l_s = hrrwork + 51480;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_g_g = INT__g_p_g_g + real_abcd * 10125;
                  ...
                  double * const restrict INT__p_g_g_g)
{
    ...
    int ret = ostei_g_p_g_g(P_tmp, Q, screen_tol, work, INT__p_g_g_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__p_g_g_g[q*10125+b*675+a*225+c*15+d];

        memcpy(INT__p_g_g_g+q*10125, buffer, 10125*sizeof(double));
    }
