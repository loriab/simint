                  ...
                  double * const restrict INT__d_d_g_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_d_g_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__f_s_g_s = work + (SIMINT_NSHELL_SIMD * 90);
    ...
    double * const HRR_INT__f_p_g_s = hrrwork + 270;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_d_g_s = INT__d_d_g_s + real_abcd * 540;
                  ...
                  double * const restrict INT__d_d_s_g)
{
    ...
    int ret = ostei_d_d_g_s(P, Q_tmp, screen_tol, work, INT__d_d_s_g);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_d_s_g[q*540+a*90+b*15+d*1+c];

        memcpy(INT__d_d_s_g+q*540, buffer, 540*sizeof(double));
    }
