                  ...
                  double * const restrict INT__d_f_s_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_f_s_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_f_s_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_g_s_s = work + (SIMINT_NSHELL_SIMD * 10);
    double * const INT__s_h_s_s = work + (SIMINT_NSHELL_SIMD * 25);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_d_s_s = primwork + 21;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_f_s_s = primwork + 45;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_g_s_s = primwork + 75;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_h_s_s = primwork + 105;
    double * const hrrwork = (double *)(primwork + 126);
    double * const HRR_INT__p_f_s_s = hrrwork + 0;
    double * const HRR_INT__p_g_s_s = hrrwork + 30;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_f_s_s = INT__s_f_s_s + abcd * 10;
                double * restrict PRIM_PTR_INT__s_g_s_s = INT__s_g_s_s + abcd * 15;
                double * restrict PRIM_PTR_INT__s_h_s_s = INT__s_h_s_s + abcd * 21;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__s_f_s_s += 10;
                            PRIM_PTR_INT__s_g_s_s += 15;
                            PRIM_PTR_INT__s_h_s_s += 21;
                        }
                        iprimcd++;
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__s_f_s_s += lastoffset*10;
                            PRIM_PTR_INT__s_g_s_s += lastoffset*15;
                            PRIM_PTR_INT__s_h_s_s += lastoffset*21;
                            continue;
                        }
                    }
                    ...
                    // Forming PRIM_INT__s_g_s_s[2 * 15];
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(10, PRIM_INT__s_f_s_s, PRIM_PTR_INT__s_f_s_s);
                        contract_all(15, PRIM_INT__s_g_s_s, PRIM_PTR_INT__s_g_s_s);
                        contract_all(21, PRIM_INT__s_h_s_s, PRIM_PTR_INT__s_h_s_s);
                    }
                    else
                    {
                        contract(10, shelloffsets, PRIM_INT__s_f_s_s, PRIM_PTR_INT__s_f_s_s);
                        contract(15, shelloffsets, PRIM_INT__s_g_s_s, PRIM_PTR_INT__s_g_s_s);
                        contract(21, shelloffsets, PRIM_INT__s_h_s_s, PRIM_PTR_INT__s_h_s_s);
                        PRIM_PTR_INT__s_f_s_s += lastoffset*10;
                        PRIM_PTR_INT__s_g_s_s += lastoffset*15;
                        PRIM_PTR_INT__s_h_s_s += lastoffset*21;
                    }
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__s_f_s_s = INT__s_f_s_s + abcd * 10;
                double const * restrict HRR_INT__s_g_s_s = INT__s_g_s_s + abcd * 15;
                double const * restrict HRR_INT__s_h_s_s = INT__s_h_s_s + abcd * 21;
                double * restrict HRR_INT__d_f_s_s = INT__d_f_s_s + real_abcd * 60;

                // form INT__p_f_s_s
                for(iket = 0; iket < 1; ++iket)
                {
                    HRR_INT__p_f_s_s[0 * 1 + iket] = HRR_INT__s_g_s_s[0 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[0 * 1 + iket] );

                    HRR_INT__p_f_s_s[1 * 1 + iket] = HRR_INT__s_g_s_s[1 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[1 * 1 + iket] );

                    HRR_INT__p_f_s_s[2 * 1 + iket] = HRR_INT__s_g_s_s[2 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[2 * 1 + iket] );

                    HRR_INT__p_f_s_s[3 * 1 + iket] = HRR_INT__s_g_s_s[3 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[3 * 1 + iket] );

                    HRR_INT__p_f_s_s[4 * 1 + iket] = HRR_INT__s_g_s_s[4 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[4 * 1 + iket] );

                    HRR_INT__p_f_s_s[5 * 1 + iket] = HRR_INT__s_g_s_s[5 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[5 * 1 + iket] );

                    HRR_INT__p_f_s_s[6 * 1 + iket] = HRR_INT__s_g_s_s[6 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[6 * 1 + iket] );

                    HRR_INT__p_f_s_s[7 * 1 + iket] = HRR_INT__s_g_s_s[7 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[7 * 1 + iket] );

                    HRR_INT__p_f_s_s[8 * 1 + iket] = HRR_INT__s_g_s_s[8 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[8 * 1 + iket] );

                    HRR_INT__p_f_s_s[9 * 1 + iket] = HRR_INT__s_g_s_s[9 * 1 + iket] - ( hAB[0] * HRR_INT__s_f_s_s[9 * 1 + iket] );

                    HRR_INT__p_f_s_s[10 * 1 + iket] = HRR_INT__s_g_s_s[1 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[0 * 1 + iket] );

                    HRR_INT__p_f_s_s[11 * 1 + iket] = HRR_INT__s_g_s_s[3 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[1 * 1 + iket] );

                    HRR_INT__p_f_s_s[12 * 1 + iket] = HRR_INT__s_g_s_s[4 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[2 * 1 + iket] );

                    HRR_INT__p_f_s_s[13 * 1 + iket] = HRR_INT__s_g_s_s[6 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[3 * 1 + iket] );

                    HRR_INT__p_f_s_s[14 * 1 + iket] = HRR_INT__s_g_s_s[7 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[4 * 1 + iket] );

                    HRR_INT__p_f_s_s[15 * 1 + iket] = HRR_INT__s_g_s_s[8 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[5 * 1 + iket] );

                    HRR_INT__p_f_s_s[16 * 1 + iket] = HRR_INT__s_g_s_s[10 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[6 * 1 + iket] );

                    HRR_INT__p_f_s_s[17 * 1 + iket] = HRR_INT__s_g_s_s[11 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[7 * 1 + iket] );

                    HRR_INT__p_f_s_s[18 * 1 + iket] = HRR_INT__s_g_s_s[12 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[8 * 1 + iket] );

                    HRR_INT__p_f_s_s[19 * 1 + iket] = HRR_INT__s_g_s_s[13 * 1 + iket] - ( hAB[1] * HRR_INT__s_f_s_s[9 * 1 + iket] );

                    HRR_INT__p_f_s_s[20 * 1 + iket] = HRR_INT__s_g_s_s[2 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[0 * 1 + iket] );

                    HRR_INT__p_f_s_s[21 * 1 + iket] = HRR_INT__s_g_s_s[4 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[1 * 1 + iket] );

                    HRR_INT__p_f_s_s[22 * 1 + iket] = HRR_INT__s_g_s_s[5 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[2 * 1 + iket] );

                    HRR_INT__p_f_s_s[23 * 1 + iket] = HRR_INT__s_g_s_s[7 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[3 * 1 + iket] );

                    HRR_INT__p_f_s_s[24 * 1 + iket] = HRR_INT__s_g_s_s[8 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[4 * 1 + iket] );

                    HRR_INT__p_f_s_s[25 * 1 + iket] = HRR_INT__s_g_s_s[9 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[5 * 1 + iket] );

                    HRR_INT__p_f_s_s[26 * 1 + iket] = HRR_INT__s_g_s_s[11 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[6 * 1 + iket] );

                    HRR_INT__p_f_s_s[27 * 1 + iket] = HRR_INT__s_g_s_s[12 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[7 * 1 + iket] );

                    HRR_INT__p_f_s_s[28 * 1 + iket] = HRR_INT__s_g_s_s[13 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[8 * 1 + iket] );

                    HRR_INT__p_f_s_s[29 * 1 + iket] = HRR_INT__s_g_s_s[14 * 1 + iket] - ( hAB[2] * HRR_INT__s_f_s_s[9 * 1 + iket] );
