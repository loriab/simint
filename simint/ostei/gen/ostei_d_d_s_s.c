                  ...
                  double * const restrict INT__d_d_s_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_d_s_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__d_s_s_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__f_s_s_s = work + (SIMINT_NSHELL_SIMD * 6);
    double * const INT__g_s_s_s = work + (SIMINT_NSHELL_SIMD * 16);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__d_s_s_s = primwork + 17;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_s_s = primwork + 35;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 55;
    double * const hrrwork = (double *)(primwork + 70);
    double * const HRR_INT__d_p_s_s = hrrwork + 0;
    double * const HRR_INT__f_p_s_s = hrrwork + 18;
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    // Forming PRIM_INT__f_s_s_s[2 * 10];
                    ...
                    // Forming PRIM_INT__g_s_s_s[1 * 15];
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_d_s_s = INT__d_d_s_s + real_abcd * 36;

                // form INT__d_p_s_s
                for(iket = 0; iket < 1; ++iket)
                {
                    HRR_INT__d_p_s_s[0 * 1 + iket] = HRR_INT__f_s_s_s[0 * 1 + iket] + ( hAB[0] * HRR_INT__d_s_s_s[0 * 1 + iket] );

                    HRR_INT__d_p_s_s[1 * 1 + iket] = HRR_INT__f_s_s_s[1 * 1 + iket] + ( hAB[1] * HRR_INT__d_s_s_s[0 * 1 + iket] );

                    HRR_INT__d_p_s_s[2 * 1 + iket] = HRR_INT__f_s_s_s[2 * 1 + iket] + ( hAB[2] * HRR_INT__d_s_s_s[0 * 1 + iket] );

                    HRR_INT__d_p_s_s[3 * 1 + iket] = HRR_INT__f_s_s_s[1 * 1 + iket] + ( hAB[0] * HRR_INT__d_s_s_s[1 * 1 + iket] );

                    HRR_INT__d_p_s_s[4 * 1 + iket] = HRR_INT__f_s_s_s[3 * 1 + iket] + ( hAB[1] * HRR_INT__d_s_s_s[1 * 1 + iket] );

                    HRR_INT__d_p_s_s[5 * 1 + iket] = HRR_INT__f_s_s_s[4 * 1 + iket] + ( hAB[2] * HRR_INT__d_s_s_s[1 * 1 + iket] );

                    HRR_INT__d_p_s_s[6 * 1 + iket] = HRR_INT__f_s_s_s[2 * 1 + iket] + ( hAB[0] * HRR_INT__d_s_s_s[2 * 1 + iket] );

                    HRR_INT__d_p_s_s[7 * 1 + iket] = HRR_INT__f_s_s_s[4 * 1 + iket] + ( hAB[1] * HRR_INT__d_s_s_s[2 * 1 + iket] );

                    HRR_INT__d_p_s_s[8 * 1 + iket] = HRR_INT__f_s_s_s[5 * 1 + iket] + ( hAB[2] * HRR_INT__d_s_s_s[2 * 1 + iket] );

                    HRR_INT__d_p_s_s[9 * 1 + iket] = HRR_INT__f_s_s_s[3 * 1 + iket] + ( hAB[0] * HRR_INT__d_s_s_s[3 * 1 + iket] );

                    HRR_INT__d_p_s_s[10 * 1 + iket] = HRR_INT__f_s_s_s[6 * 1 + iket] + ( hAB[1] * HRR_INT__d_s_s_s[3 * 1 + iket] );

                    HRR_INT__d_p_s_s[11 * 1 + iket] = HRR_INT__f_s_s_s[7 * 1 + iket] + ( hAB[2] * HRR_INT__d_s_s_s[3 * 1 + iket] );

                    HRR_INT__d_p_s_s[12 * 1 + iket] = HRR_INT__f_s_s_s[4 * 1 + iket] + ( hAB[0] * HRR_INT__d_s_s_s[4 * 1 + iket] );

                    HRR_INT__d_p_s_s[13 * 1 + iket] = HRR_INT__f_s_s_s[7 * 1 + iket] + ( hAB[1] * HRR_INT__d_s_s_s[4 * 1 + iket] );

                    HRR_INT__d_p_s_s[14 * 1 + iket] = HRR_INT__f_s_s_s[8 * 1 + iket] + ( hAB[2] * HRR_INT__d_s_s_s[4 * 1 + iket] );

                    HRR_INT__d_p_s_s[15 * 1 + iket] = HRR_INT__f_s_s_s[5 * 1 + iket] + ( hAB[0] * HRR_INT__d_s_s_s[5 * 1 + iket] );

                    HRR_INT__d_p_s_s[16 * 1 + iket] = HRR_INT__f_s_s_s[8 * 1 + iket] + ( hAB[1] * HRR_INT__d_s_s_s[5 * 1 + iket] );

                    HRR_INT__d_p_s_s[17 * 1 + iket] = HRR_INT__f_s_s_s[9 * 1 + iket] + ( hAB[2] * HRR_INT__d_s_s_s[5 * 1 + iket] );
                ...
                for(iket = 0; iket < 1; ++iket)
                {
                    HRR_INT__d_d_s_s[0 * 1 + iket] = HRR_INT__f_p_s_s[0 * 1 + iket] + ( hAB[0] * HRR_INT__d_p_s_s[0 * 1 + iket] );

                    HRR_INT__d_d_s_s[1 * 1 + iket] = HRR_INT__f_p_s_s[3 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[0 * 1 + iket] );

                    HRR_INT__d_d_s_s[2 * 1 + iket] = HRR_INT__f_p_s_s[6 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[0 * 1 + iket] );

                    HRR_INT__d_d_s_s[3 * 1 + iket] = HRR_INT__f_p_s_s[4 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[1 * 1 + iket] );

                    HRR_INT__d_d_s_s[4 * 1 + iket] = HRR_INT__f_p_s_s[7 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[1 * 1 + iket] );

                    HRR_INT__d_d_s_s[5 * 1 + iket] = HRR_INT__f_p_s_s[8 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[2 * 1 + iket] );

                    HRR_INT__d_d_s_s[6 * 1 + iket] = HRR_INT__f_p_s_s[3 * 1 + iket] + ( hAB[0] * HRR_INT__d_p_s_s[3 * 1 + iket] );

                    HRR_INT__d_d_s_s[7 * 1 + iket] = HRR_INT__f_p_s_s[9 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[3 * 1 + iket] );

                    HRR_INT__d_d_s_s[8 * 1 + iket] = HRR_INT__f_p_s_s[12 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[3 * 1 + iket] );

                    HRR_INT__d_d_s_s[9 * 1 + iket] = HRR_INT__f_p_s_s[10 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[4 * 1 + iket] );

                    HRR_INT__d_d_s_s[10 * 1 + iket] = HRR_INT__f_p_s_s[13 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[4 * 1 + iket] );

                    HRR_INT__d_d_s_s[11 * 1 + iket] = HRR_INT__f_p_s_s[14 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[5 * 1 + iket] );

                    HRR_INT__d_d_s_s[12 * 1 + iket] = HRR_INT__f_p_s_s[6 * 1 + iket] + ( hAB[0] * HRR_INT__d_p_s_s[6 * 1 + iket] );

                    HRR_INT__d_d_s_s[13 * 1 + iket] = HRR_INT__f_p_s_s[12 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[6 * 1 + iket] );

                    HRR_INT__d_d_s_s[14 * 1 + iket] = HRR_INT__f_p_s_s[15 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[6 * 1 + iket] );

                    HRR_INT__d_d_s_s[15 * 1 + iket] = HRR_INT__f_p_s_s[13 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[7 * 1 + iket] );

                    HRR_INT__d_d_s_s[16 * 1 + iket] = HRR_INT__f_p_s_s[16 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[7 * 1 + iket] );

                    HRR_INT__d_d_s_s[17 * 1 + iket] = HRR_INT__f_p_s_s[17 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[8 * 1 + iket] );

                    HRR_INT__d_d_s_s[18 * 1 + iket] = HRR_INT__f_p_s_s[9 * 1 + iket] + ( hAB[0] * HRR_INT__d_p_s_s[9 * 1 + iket] );

                    HRR_INT__d_d_s_s[19 * 1 + iket] = HRR_INT__f_p_s_s[18 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[9 * 1 + iket] );

                    HRR_INT__d_d_s_s[20 * 1 + iket] = HRR_INT__f_p_s_s[21 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[9 * 1 + iket] );

                    HRR_INT__d_d_s_s[21 * 1 + iket] = HRR_INT__f_p_s_s[19 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[10 * 1 + iket] );

                    HRR_INT__d_d_s_s[22 * 1 + iket] = HRR_INT__f_p_s_s[22 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[10 * 1 + iket] );

                    HRR_INT__d_d_s_s[23 * 1 + iket] = HRR_INT__f_p_s_s[23 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[11 * 1 + iket] );

                    HRR_INT__d_d_s_s[24 * 1 + iket] = HRR_INT__f_p_s_s[12 * 1 + iket] + ( hAB[0] * HRR_INT__d_p_s_s[12 * 1 + iket] );

                    HRR_INT__d_d_s_s[25 * 1 + iket] = HRR_INT__f_p_s_s[21 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[12 * 1 + iket] );

                    HRR_INT__d_d_s_s[26 * 1 + iket] = HRR_INT__f_p_s_s[24 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[12 * 1 + iket] );

                    HRR_INT__d_d_s_s[27 * 1 + iket] = HRR_INT__f_p_s_s[22 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[13 * 1 + iket] );

                    HRR_INT__d_d_s_s[28 * 1 + iket] = HRR_INT__f_p_s_s[25 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[13 * 1 + iket] );

                    HRR_INT__d_d_s_s[29 * 1 + iket] = HRR_INT__f_p_s_s[26 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[14 * 1 + iket] );

                    HRR_INT__d_d_s_s[30 * 1 + iket] = HRR_INT__f_p_s_s[15 * 1 + iket] + ( hAB[0] * HRR_INT__d_p_s_s[15 * 1 + iket] );

                    HRR_INT__d_d_s_s[31 * 1 + iket] = HRR_INT__f_p_s_s[24 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[15 * 1 + iket] );

                    HRR_INT__d_d_s_s[32 * 1 + iket] = HRR_INT__f_p_s_s[27 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[15 * 1 + iket] );

                    HRR_INT__d_d_s_s[33 * 1 + iket] = HRR_INT__f_p_s_s[25 * 1 + iket] + ( hAB[1] * HRR_INT__d_p_s_s[16 * 1 + iket] );

                    HRR_INT__d_d_s_s[34 * 1 + iket] = HRR_INT__f_p_s_s[28 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[16 * 1 + iket] );

                    HRR_INT__d_d_s_s[35 * 1 + iket] = HRR_INT__f_p_s_s[29 * 1 + iket] + ( hAB[2] * HRR_INT__d_p_s_s[17 * 1 + iket] );
