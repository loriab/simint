                  ...
                  double * const restrict INT__s_s_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__s_s_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*36);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 36 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__s_s_g_p = INT__s_s_g_p + real_abcd * 45;
                  ...
                  double * const restrict INT__s_s_p_g)
{
    ...
    int ret = ostei_s_s_g_p(P, Q_tmp, screen_tol, work, INT__s_s_p_g);
    double buffer[45] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_s_p_g[q*45+a*45+b*45+d*3+c];

        memcpy(INT__s_s_p_g+q*45, buffer, 45*sizeof(double));
    }
