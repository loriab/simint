                  ...
                  double * const restrict INT__g_p_d_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_d_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*576);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_s_s = primwork + 129;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_p_s = primwork + 189;
    SIMINT_DBLTYPE * const restrict PRIM_INT__f_s_d_s = primwork + 279;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 399;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 474;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_d_s = primwork + 609;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_f_s = primwork + 789;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 939;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_p_s = primwork + 1023;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_d_s = primwork + 1212;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_f_s = primwork + 1464;
    double * const hrrwork = (double *)(primwork + 1674);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 576 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_d_p = INT__g_p_d_p + real_abcd * 810;
                  ...
                  double * const restrict INT__p_g_d_p)
{
    ...
    int ret = ostei_g_p_d_p(P_tmp, Q, screen_tol, work, INT__p_g_d_p);
    double buffer[810] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__p_g_d_p[q*810+b*54+a*18+c*3+d];

        memcpy(INT__p_g_d_p+q*810, buffer, 810*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_p_p_d)
{
    ...
    int ret = ostei_g_p_d_p(P, Q_tmp, screen_tol, work, INT__g_p_p_d);
    double buffer[810] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__g_p_p_d[q*810+a*54+b*18+d*3+c];

        memcpy(INT__g_p_p_d+q*810, buffer, 810*sizeof(double));
    }
                  ...
                  double * const restrict INT__p_g_p_d)
{
    ...
    int ret = ostei_g_p_d_p(P_tmp, Q_tmp, screen_tol, work, INT__p_g_p_d);
    double buffer[810] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__p_g_p_d[q*810+b*54+a*18+d*3+c];

        memcpy(INT__p_g_p_d+q*810, buffer, 810*sizeof(double));
    }
