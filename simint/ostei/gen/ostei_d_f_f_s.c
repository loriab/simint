                  ...
                  double * const restrict INT__d_f_f_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__d_f_f_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__s_g_f_s = work + (SIMINT_NSHELL_SIMD * 100);
    double * const INT__s_h_f_s = work + (SIMINT_NSHELL_SIMD * 250);
    ...
    double * const HRR_INT__p_g_f_s = hrrwork + 300;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__d_f_f_s = INT__d_f_f_s + real_abcd * 600;
