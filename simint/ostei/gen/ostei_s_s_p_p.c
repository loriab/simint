                  ...
                  double * const restrict INT__s_s_p_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__s_s_p_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    // partition workspace
    double * const INT__s_s_p_s = work + (SIMINT_NSHELL_SIMD * 0);
    double * const INT__s_s_d_s = work + (SIMINT_NSHELL_SIMD * 3);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_p_s = primwork + 3;
    SIMINT_DBLTYPE * const restrict PRIM_INT__s_s_d_s = primwork + 9;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__s_s_p_s = INT__s_s_p_s + abcd * 3;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if(check_screen)
                    {
                        ...
                        if(vmax < screen_tol)
                        {
                            PRIM_PTR_INT__s_s_p_s += lastoffset*3;
                    ...
                    if(lastoffset == 0)
                    {
                        contract_all(3, PRIM_INT__s_s_p_s, PRIM_PTR_INT__s_s_p_s);
                        contract_all(6, PRIM_INT__s_s_d_s, PRIM_PTR_INT__s_s_d_s);
                    }
                    else
                    {
                        contract(3, shelloffsets, PRIM_INT__s_s_p_s, PRIM_PTR_INT__s_s_p_s);
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                // set up HRR pointers
                double const * restrict HRR_INT__s_s_p_s = INT__s_s_p_s + abcd * 3;
                double const * restrict HRR_INT__s_s_d_s = INT__s_s_d_s + abcd * 6;
                double * restrict HRR_INT__s_s_p_p = INT__s_s_p_p + real_abcd * 9;

                // form INT__s_s_p_p
                for(ibra = 0; ibra < 1; ++ibra)
                {
                    HRR_INT__s_s_p_p[ibra * 9 + 0] = HRR_INT__s_s_d_s[ibra * 6 + 0] + ( hCD[0] * HRR_INT__s_s_p_s[ibra * 3 + 0] );

                    HRR_INT__s_s_p_p[ibra * 9 + 1] = HRR_INT__s_s_d_s[ibra * 6 + 1] + ( hCD[1] * HRR_INT__s_s_p_s[ibra * 3 + 0] );

                    HRR_INT__s_s_p_p[ibra * 9 + 2] = HRR_INT__s_s_d_s[ibra * 6 + 2] + ( hCD[2] * HRR_INT__s_s_p_s[ibra * 3 + 0] );

                    HRR_INT__s_s_p_p[ibra * 9 + 3] = HRR_INT__s_s_d_s[ibra * 6 + 1] + ( hCD[0] * HRR_INT__s_s_p_s[ibra * 3 + 1] );

                    HRR_INT__s_s_p_p[ibra * 9 + 4] = HRR_INT__s_s_d_s[ibra * 6 + 3] + ( hCD[1] * HRR_INT__s_s_p_s[ibra * 3 + 1] );

                    HRR_INT__s_s_p_p[ibra * 9 + 5] = HRR_INT__s_s_d_s[ibra * 6 + 4] + ( hCD[2] * HRR_INT__s_s_p_s[ibra * 3 + 1] );

                    HRR_INT__s_s_p_p[ibra * 9 + 6] = HRR_INT__s_s_d_s[ibra * 6 + 2] + ( hCD[0] * HRR_INT__s_s_p_s[ibra * 3 + 2] );

                    HRR_INT__s_s_p_p[ibra * 9 + 7] = HRR_INT__s_s_d_s[ibra * 6 + 4] + ( hCD[1] * HRR_INT__s_s_p_s[ibra * 3 + 2] );

                    HRR_INT__s_s_p_p[ibra * 9 + 8] = HRR_INT__s_s_d_s[ibra * 6 + 5] + ( hCD[2] * HRR_INT__s_s_p_s[ibra * 3 + 2] );
