                  ...
                  double * const restrict INT__g_g_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_g_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__h_s_g_s = work + (SIMINT_NSHELL_SIMD * 540);
    double * const INT__h_s_h_s = work + (SIMINT_NSHELL_SIMD * 855);
    double * const INT__i_s_g_s = work + (SIMINT_NSHELL_SIMD * 1296);
    double * const INT__i_s_h_s = work + (SIMINT_NSHELL_SIMD * 1716);
    double * const INT__k_s_g_s = work + (SIMINT_NSHELL_SIMD * 2304);
    double * const INT__k_s_h_s = work + (SIMINT_NSHELL_SIMD * 2844);
    double * const INT__l_s_g_s = work + (SIMINT_NSHELL_SIMD * 3600);
    double * const INT__l_s_h_s = work + (SIMINT_NSHELL_SIMD * 4275);
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*5220);
    ...
    double * const HRR_INT__g_d_g_s = hrrwork + 1620;
    double * const HRR_INT__g_d_h_s = hrrwork + 2970;
    double * const HRR_INT__g_f_g_s = hrrwork + 4860;
    double * const HRR_INT__g_f_h_s = hrrwork + 7110;
    double * const HRR_INT__g_g_g_s = hrrwork + 10260;
    double * const HRR_INT__g_g_h_s = hrrwork + 13635;
    double * const HRR_INT__h_p_g_s = hrrwork + 18360;
    double * const HRR_INT__h_p_h_s = hrrwork + 19305;
    double * const HRR_INT__h_d_g_s = hrrwork + 20628;
    double * const HRR_INT__h_d_h_s = hrrwork + 22518;
    double * const HRR_INT__h_f_g_s = hrrwork + 25164;
    double * const HRR_INT__h_f_h_s = hrrwork + 28314;
    double * const HRR_INT__i_p_g_s = hrrwork + 32724;
    double * const HRR_INT__i_p_h_s = hrrwork + 33984;
    double * const HRR_INT__i_d_g_s = hrrwork + 35748;
    double * const HRR_INT__i_d_h_s = hrrwork + 38268;
    double * const HRR_INT__k_p_g_s = hrrwork + 41796;
    double * const HRR_INT__k_p_h_s = hrrwork + 43416;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 5220 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_g_g_p = INT__g_g_g_p + real_abcd * 10125;
                  ...
                  double * const restrict INT__g_g_p_g)
{
    ...
    int ret = ostei_g_g_g_p(P, Q_tmp, screen_tol, work, INT__g_g_p_g);
    double buffer[10125] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_g_p_g[q*10125+a*675+b*45+d*3+c];

        memcpy(INT__g_g_p_g+q*10125, buffer, 10125*sizeof(double));
    }
