                  ...
                  double * const restrict INT__g_p_p_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_p_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    SIMINT_DBLTYPE * const primwork = (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD*324);
    ...
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_s_s = primwork + 175;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_p_s = primwork + 235;
    SIMINT_DBLTYPE * const restrict PRIM_INT__g_s_d_s = primwork + 325;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_s_s = primwork + 415;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_p_s = primwork + 478;
    SIMINT_DBLTYPE * const restrict PRIM_INT__h_s_d_s = primwork + 604;
    double * const hrrwork = (double *)(primwork + 730);
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            memset(work, 0, SIMINT_NSHELL_SIMD * 324 * sizeof(double));
            abcd = 0;
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_p_p = INT__g_p_p_p + real_abcd * 405;
                ...
                {
                    HRR_INT__g_p_p_p[ibra * 9 + 0] = HRR_INT__g_p_d_s[ibra * 6 + 0] + ( hCD[0] * HRR_INT__g_p_p_s[ibra * 3 + 0] );

                    HRR_INT__g_p_p_p[ibra * 9 + 1] = HRR_INT__g_p_d_s[ibra * 6 + 1] + ( hCD[1] * HRR_INT__g_p_p_s[ibra * 3 + 0] );

                    HRR_INT__g_p_p_p[ibra * 9 + 2] = HRR_INT__g_p_d_s[ibra * 6 + 2] + ( hCD[2] * HRR_INT__g_p_p_s[ibra * 3 + 0] );

                    HRR_INT__g_p_p_p[ibra * 9 + 3] = HRR_INT__g_p_d_s[ibra * 6 + 1] + ( hCD[0] * HRR_INT__g_p_p_s[ibra * 3 + 1] );

                    HRR_INT__g_p_p_p[ibra * 9 + 4] = HRR_INT__g_p_d_s[ibra * 6 + 3] + ( hCD[1] * HRR_INT__g_p_p_s[ibra * 3 + 1] );

                    HRR_INT__g_p_p_p[ibra * 9 + 5] = HRR_INT__g_p_d_s[ibra * 6 + 4] + ( hCD[2] * HRR_INT__g_p_p_s[ibra * 3 + 1] );

                    HRR_INT__g_p_p_p[ibra * 9 + 6] = HRR_INT__g_p_d_s[ibra * 6 + 2] + ( hCD[0] * HRR_INT__g_p_p_s[ibra * 3 + 2] );

                    HRR_INT__g_p_p_p[ibra * 9 + 7] = HRR_INT__g_p_d_s[ibra * 6 + 4] + ( hCD[1] * HRR_INT__g_p_p_s[ibra * 3 + 2] );

                    HRR_INT__g_p_p_p[ibra * 9 + 8] = HRR_INT__g_p_d_s[ibra * 6 + 5] + ( hCD[2] * HRR_INT__g_p_p_s[ibra * 3 + 2] );
                  ...
                  double * const restrict INT__p_g_p_p)
{
    ...
    int ret = ostei_g_p_p_p(P_tmp, Q, screen_tol, work, INT__p_g_p_p);
    double buffer[405] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__p_g_p_p[q*405+b*27+a*9+c*3+d];

        memcpy(INT__p_g_p_p+q*405, buffer, 405*sizeof(double));
    }
