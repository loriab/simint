                  ...
                  double * const restrict INT__g_s_f_f)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_s_f_f);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_s_f_d = hrrwork + 450;
    double * const HRR_INT__g_s_g_p = hrrwork + 1350;
    double * const HRR_INT__g_s_g_d = hrrwork + 2025;
    double * const HRR_INT__g_s_h_p = hrrwork + 3375;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_s_f_f = INT__g_s_f_f + real_abcd * 1500;
                  ...
                  double * const restrict INT__s_g_f_f)
{
    ...
    int ret = ostei_g_s_f_f(P_tmp, Q, screen_tol, work, INT__s_g_f_f);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 10; ++c)
        for(int d = 0; d < 10; ++d)
            buffer[idx++] = INT__s_g_f_f[q*1500+b*100+a*100+c*10+d];

        memcpy(INT__s_g_f_f+q*1500, buffer, 1500*sizeof(double));
    }
