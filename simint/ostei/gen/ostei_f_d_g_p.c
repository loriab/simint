                  ...
                  double * const restrict INT__f_d_g_p)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__f_d_g_p);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const INT__g_s_g_s = work + (SIMINT_NSHELL_SIMD * 360);
    double * const INT__g_s_h_s = work + (SIMINT_NSHELL_SIMD * 585);
    ...
    double * const HRR_INT__f_d_g_s = hrrwork + 1080;
    double * const HRR_INT__f_d_h_s = hrrwork + 1980;
    double * const HRR_INT__g_p_g_s = hrrwork + 3240;
    double * const HRR_INT__g_p_h_s = hrrwork + 3915;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__f_d_g_p = INT__f_d_g_p + real_abcd * 2700;
                  ...
                  double * const restrict INT__d_f_g_p)
{
    ...
    int ret = ostei_f_d_g_p(P_tmp, Q, screen_tol, work, INT__d_f_g_p);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 3; ++d)
            buffer[idx++] = INT__d_f_g_p[q*2700+b*270+a*45+c*3+d];

        memcpy(INT__d_f_g_p+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__f_d_p_g)
{
    ...
    int ret = ostei_f_d_g_p(P, Q_tmp, screen_tol, work, INT__f_d_p_g);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__f_d_p_g[q*2700+a*270+b*45+d*3+c];

        memcpy(INT__f_d_p_g+q*2700, buffer, 2700*sizeof(double));
    }
                  ...
                  double * const restrict INT__d_f_p_g)
{
    ...
    int ret = ostei_f_d_g_p(P_tmp, Q_tmp, screen_tol, work, INT__d_f_p_g);
    double buffer[2700] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 3; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__d_f_p_g[q*2700+b*270+a*45+d*3+c];

        memcpy(INT__d_f_p_g+q*2700, buffer, 2700*sizeof(double));
    }
