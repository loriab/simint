                  ...
                  double * const restrict INT__g_p_s_s)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_p_s_s);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_p_s_s = INT__g_p_s_s + real_abcd * 45;
                  ...
                  double * const restrict INT__p_g_s_s)
{
    ...
    int ret = ostei_g_p_s_s(P_tmp, Q, screen_tol, work, INT__p_g_s_s);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 1; ++c)
        for(int d = 0; d < 1; ++d)
            buffer[idx++] = INT__p_g_s_s[q*45+b*3+a*1+c*1+d];

        memcpy(INT__p_g_s_s+q*45, buffer, 45*sizeof(double));
    }
