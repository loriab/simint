                  ...
                  double * const restrict INT__g_s_g_d)
{
    ...
    SIMINT_ASSUME_ALIGN_DBL(INT__g_s_g_d);
    int ab, cd, abcd;
    int istart, jstart;
    ...
    double * const HRR_INT__g_s_h_p = hrrwork + 675;
    ...
    for(ab = 0; ab < P.nshell12_clip; ++ab)
    {
        ...
        for(cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD)
        {
            ...
            for(abcd = 0; abcd < nshellbatch; ++abcd, ++real_abcd)
            {
                ...
                double * restrict HRR_INT__g_s_g_d = INT__g_s_g_d + real_abcd * 1350;
                  ...
                  double * const restrict INT__s_g_g_d)
{
    ...
    int ret = ostei_g_s_g_d(P_tmp, Q, screen_tol, work, INT__s_g_g_d);
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 15; ++c)
        for(int d = 0; d < 6; ++d)
            buffer[idx++] = INT__s_g_g_d[q*1350+b*90+a*90+c*6+d];

        memcpy(INT__s_g_g_d+q*1350, buffer, 1350*sizeof(double));
    }
                  ...
                  double * const restrict INT__g_s_d_g)
{
    ...
    int ret = ostei_g_s_g_d(P, Q_tmp, screen_tol, work, INT__g_s_d_g);
    double buffer[1350] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__g_s_d_g[q*1350+a*90+b*90+d*6+c];

        memcpy(INT__g_s_d_g+q*1350, buffer, 1350*sizeof(double));
    }
                  ...
                  double * const restrict INT__s_g_d_g)
{
    ...
    int ret = ostei_g_s_g_d(P_tmp, Q_tmp, screen_tol, work, INT__s_g_d_g);
    double buffer[1350] SIMINT_ALIGN_ARRAY_DBL;
    ...
    for(int q = 0; q < ret; q++)
    {
        ...
        for(int c = 0; c < 6; ++c)
        for(int d = 0; d < 15; ++d)
            buffer[idx++] = INT__s_g_d_g[q*1350+b*90+a*90+d*6+c];

        memcpy(INT__s_g_d_g+q*1350, buffer, 1350*sizeof(double));
    }
